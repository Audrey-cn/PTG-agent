...
class PrometheusREPL:
    ...
    def __init__(self):
        self.session = None
        self.history_path = Path.home() / ".prometheus_history"
        self.running = False
  ...
  clear / cls             清空屏幕
    ...
    def handle_command(self, command: str) -> bool:
        ...
        if cmd == "append" and len(parts) >= 3:
            seed_path = parts[1]
            note = parts[2]
            self.run_tool("append_historical_note", json.dumps({"seed_path": seed_path, "note": note}))
            return True
        
        if cmd == "inspect" and len(parts) >= 2:
            seed_path = parts[1]
            detail = parts[2] if len(parts) >= 3 else "basic"
            self.run_tool("inspect_seed", json.dumps({"seed_path": seed_path, "detail_level": detail}))
            return True
...
class PrometheusCompleter(Completer):
    ...
    def __init__(self):
        self.commands = [
            "help", "exit", "quit", "tools", "tool", "run",
            "stamp", "trace", "append", "inspect", "stamps",
            "clear", "cls"
        ]
