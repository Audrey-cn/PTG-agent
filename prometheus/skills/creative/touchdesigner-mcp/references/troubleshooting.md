...
`$PROMETHEUS_HOME/config.yaml` (defaults to `~/.prometheus/config.yaml` when `PROMETHEUS_HOME` is unset)
...
Restart the Prometheus session for changes to take effect. The MCP connection is
