...
metadata:
  prometheus:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, prometheus-video]
...
```
Prometheus Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${PROMETHEUS_HOME:-$HOME/.prometheus}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Prometheus config (if missing)
...
3. **Restart Prometheus session** to pick up the new MCP server
