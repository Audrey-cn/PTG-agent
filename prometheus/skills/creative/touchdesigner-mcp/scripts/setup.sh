...
PROMETHEUS_HOME_DIR="${PROMETHEUS_HOME:-$HOME/.prometheus}"
PROMETHEUS_CFG="${PROMETHEUS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Prometheus config has twozero_td MCP entry ──
if [[ ! -f "$PROMETHEUS_CFG" ]]; then
    echo -e " ${FAIL} Prometheus config not found at ${PROMETHEUS_CFG}"
    manual_steps+=("Create ${PROMETHEUS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$PROMETHEUS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Prometheus config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Prometheus config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$PROMETHEUS_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${PROMETHEUS_CFG} manually"); }
    manual_steps+=("Restart Prometheus session to pick up config change")
fi
