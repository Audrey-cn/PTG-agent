#!/usr/bin/env python3
"""Google Workspace API CLI for Prometheus Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Prometheus-facing JSON contract and falls back to the Python client
...
from _prometheus_home import get_prometheus_home
...
def _gws_binary() -> str | None:
    override = os.getenv("PROMETHEUS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Prometheus Agent")
