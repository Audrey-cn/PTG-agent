#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Prometheus Agent.
...
CLIENT_SECRET_PATH = PROMETHEUS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = PROMETHEUS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Prometheus profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'prometheus[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Prometheus home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Prometheus")
