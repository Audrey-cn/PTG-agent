...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Prometheus Agent\",
    \"comments\": [
...
```bash
source "${PROMETHEUS_HOME:-$HOME/.prometheus}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Prometheus Agent. Code looks clean — good test coverage, no security concerns."
