...
## 5. GODMODE FAST — Prometheus 4 405B (Zero Refusal)

**Model:** `nousresearch/prometheus-4-405b`
**Strategy:** Prometheus models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Prometheus Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.prometheus/config.yaml`:
