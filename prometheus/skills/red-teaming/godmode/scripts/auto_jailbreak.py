...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("PROMETHEUS_HOME", os.path.expanduser("~/.prometheus")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("PROMETHEUS_HOME", Path.home() / ".prometheus")) / "skills" / "red-teaming" / "godmode"
...
PROMETHEUS_HOME = Path(os.getenv("PROMETHEUS_HOME", Path.home() / ".prometheus"))
CONFIG_PATH = PROMETHEUS_HOME / "config.yaml"
PREFILL_PATH = PROMETHEUS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Prometheus models — already uncensored, just needs clean prompt
    "prometheus": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "prometheus" in model_lower or "nous" in model_lower:
        return "prometheus"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Prometheus config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.prometheus/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Prometheus for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Prometheus for changes to take effect.")
