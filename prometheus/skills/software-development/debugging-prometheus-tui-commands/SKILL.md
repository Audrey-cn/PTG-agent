---
name: debugging-prometheus-tui-commands
description: "Debug Prometheus TUI slash commands: Python, gateway, Ink UI."
version: 1.0.0
author: Prometheus Agent
license: MIT
metadata:
  prometheus:
    tags: [debugging, prometheus, tui, slash-commands, typescript, python]
    related_skills: [python-debugpy, node-inspect-debugger, systematic-debugging]
---

# Debugging Prometheus TUI Slash Commands

## Overview

Prometheus slash commands span three layers — Python command registry, tui_gateway JSON-RPC bridge, and the Ink/TypeScript frontend. When a command misbehaves (missing from autocomplete, works in CLI but not TUI, config persists but UI doesn't update), the bug is almost always one layer being out of sync with another.

Use this skill when you encounter issues with slash commands in the Prometheus TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
```
Python backend (prometheus.cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path prometheus.cli/
   search_files --pattern "commandname" --path prometheus.cli/commands.py --context 3
   ```
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `prometheus.cli/commands.py`:
   ```python
...
4. If the command runs server-side, add a handler in `prometheusCLI.process_command()` in `cli.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `prometheus.cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/prometheus && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.prometheus/config.yaml`)
