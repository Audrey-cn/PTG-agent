...
史诗叙事官，负责：
1. stamp — 烙印：给种子盖上创始铭刻
2. trace — 追溯：读取种子的族谱谱系和铭刻历史，输出史诗叙事报告
3. append — 附史：在种子的谱系上追加史诗叙事

设计原则：
- 非破坏性：不修改原有种子的任何现有内容（附史只追加）
- 语义宽松：使用语义审核引擎，而非硬编码匹配
- 史诗叙事：以史诗笔法记录和解读种子的历史
"""
...
FOUNDER_TAG_LEXICON_DICT = {
    ...
    "eternal_mark": {"desc": "时间无法磨灭的印记，刻入碳基与硅基的契约", "element": "时", "weight": "eternal"},
...
class Chronicler:
    ...
    def chronicle(self, seed_path: str, narrative: Optional[str] = None) -> Dict[str, Any]:
        """
        自动识别模式 — 智能选择三种模式之一
    ...
    def _generate_recommendations(self, classification: Classification, anchor: LineageAnchor) -> List[str]:
        recs = []
    ...
    def _append_to_evolution_chronicle(self, content: str, anchor: LineageAnchor, narrative: str, author: str) -> tuple:
...
"""
        ...
        if '---' in content:
            ...
            if len(parts) >= 2:
                content = parts[0] + '---' + parts[1] + chronicle_block + '---' + parts[2] if len(parts) > 2 else parts[0] + '---' + parts[1] + chronicle_block
            else:
...
def format_trace_report(report: TraceReport, verbose: bool = False) -> str:
    ...
    if li.get("lineage") and li["lineage"] != "?":
        ...
        lines.append(f"║  谱系: {li.get('lineage', '?')} · {li.get('bloodline', '?')}".ljust(61) + "║")
        lines.append(f"║  代次: 第{li.get('generation', '?')}代 · {li.get('variant', '?')}变种".ljust(61) + "║")
        if li.get("life_id"):
    ...
    if inscriptions:
        ...
        if eternal_seals:
            lines.append(f"║  永恒印记: {len(eternal_seals)}/{len(FOUNDER_TAGS)} 命中".ljust(61) + "║")
        for insc in inscriptions[:6]:
            tag = insc.get("tag", "?")
            desc = insc.get("desc", "")[:30]
            element = insc.get("element", "")
