...
class CircuitFactory:
    ...
    def create_task_decider() -> GeneticCircuit:
        ...
        circuit = GeneticCircuit(
            circuit_id="task_decider", name="任务决策回路", description="根据输入条件选择执行任务"
        )
    ...
    def create_feedback_loop() -> GeneticCircuit:
        """反馈调节回路"""
        circuit = GeneticCircuit(
            circuit_id="feedback_loop", name="反馈调节回路", description="负反馈调节回路"
        )
...
class CircuitEngine:
    ...
    def list_circuits(self) -> list[dict[str, str]]:
        """列出所有可用回路"""
        return [
            {"circuit_id": c.circuit_id, "name": c.name, "description": c.description}
