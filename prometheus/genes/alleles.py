...
class AllelicLocus:
    ...
    def get_active_allele(self) -> Allele | None:
        """获取当前激活的等位基因"""
    ...
    def get_default_allele(self) -> Allele | None:
        """获取默认等位基因"""
        for a in self.alleles:
            if a.is_default:
                return a
...
class AlleleManager:
    ...
    def _load_catalog(self) -> dict:
        with open(ALLELES_CATALOG) as f:
            return json.load(f)
    ...
    def register_allele(self, gene_id: str, gene_name: str, allele: Allele) -> dict:
        ...
        if gene_id not in catalog["loci"]:
            catalog["loci"][gene_id] = AllelicLocus(gene_id=gene_id, gene_name=gene_name).to_dict()
    ...
    def switch_allele(self, seed_path: str, gene_id: str, allele_id: str, reason: str = "") -> dict:
        """切换种子的等位基因版本
        ...
        locus.switch_history.append(
            {"from": from_allele, "to": allele_id, "time": locus.last_switched, "reason": reason}
        )
    ...
    def _update_seed_allele(self, seed_path: str, gene_id: str, allele_id: str, allele: Allele):
        """更新种子文件中的等位基因配置"""
    ...
    standard_alleles = [
        {
            ...
            "alleles": [
                Allele(
                    allele_id="v1-standard",
                    name="标准解析器",
                    description="完整解析，支持所有字段",
                    is_default=True,
                ),
                Allele(
                    allele_id="v2-fast",
                    name="快速解析器",
                    description="跳过非必要字段，提升速度",
                    allele_type="dominant",
                ),
                Allele(
                    allele_id="v3-strict",
                    name="严格解析器",
                    description="完整校验，适合生产环境",
                    allele_type="recessive",
                ),
            ],
        },
        {
            ...
            "alleles": [
                ...
                Allele(
                    allele_id="v2-deep",
                    name="深度分析器",
                    description="更深入的分析，耗时更长",
                    allele_type="recessive",
                ),
                Allele(
                    allele_id="v3-quick",
                    name="快速分析器",
                    description="快速扫描，适合批量处理",
                    allele_type="dominant",
                ),
            ],
        },
        {
            ...
            "alleles": [
                Allele(
                    allele_id="v1-standard",
                    name="标准审计器",
                    description="四层纵深防御审计",
                    is_default=True,
                ),
                Allele(
                    allele_id="v2-light",
                    name="轻量审计器",
                    description="快速审计，跳过部分检查",
                    allele_type="dominant",
                ),
                Allele(
                    allele_id="v3-paranoid",
                    name="严格审计器",
                    description="最严格的审计，所有检查",
                    allele_type="recessive",
                ),
            ],
        },
...
""")
    ...
    if "gene_id" in alleles_data:
        ...
        for allele in alleles_data.get("alleles", []):
            ...
            print(
                f"║   {active_marker} {default_marker} [{type_label}] {allele['allele_id']:<15} {allele['name']}"
            )
...
""")
    ...
    if action == "init":
        ...
        if "error" in result:
            ...
            print(
                f"   当前激活: {result['active_allele']['allele_id'] if result['active_allele'] else '无'}"
            )
