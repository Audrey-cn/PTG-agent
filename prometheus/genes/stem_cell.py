...
class StemCell:
    ...
    def is_regenerable(self) -> bool:
        """是否可用于再生"""
        return self.pluripotency_score > 0.3 and self.state != DifferentiationState.SENESCENT
...
class StemCellNursery:
    ...
    def prepare_stem_cell(
        self, gene_id: str, blueprint: dict[str, Any], pluripotency_score: float = None
    ) -> StemCell:
        """从现有基因制备干细胞
    ...
    def dedifferentiate(self, gene_id: str, current_blueprint: dict[str, Any]) -> StemCell:
        ...
        Returns:
            新的干细胞
        """
        return self.prepare_stem_cell(gene_id, current_blueprint, pluripotency_score=0.85)
    ...
    def prune_senescent(self) -> int:
        ...
        self.stem_cells = {
            sid: sc
            for sid, sc in self.stem_cells.items()
