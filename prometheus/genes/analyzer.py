...
碳基生物学对照：
- 基因库(GeneLibrary)：对应种群基因库，存储所有可用等位基因
- 健康度审计(GeneHealthAuditor)：对应DNA损伤检测与修复评估
- 基因融合分析(GeneFusionAnalyzer)：对应同源重组可行性分析
- 外来基因拆解(ForeignGeneExtractor)：对应水平基因转移的识别与整合
"""
import os
import json
import re
...
class GeneLibrary:
    ...
    def find_gene(self, gene_id: str) -> Optional[dict]:
        """查找基因定义"""
...
class GeneHealthAuditor:
    ...
    def audit_seed(self, seed_data: dict) -> dict:
        ...
        对应碳基生物学：全基因组扫描
        """
        dna = seed_data.get('dna_encoding', {}) if isinstance(seed_data.get('dna_encoding'), dict) else {}
    ...
    def _check_carbon(self, loci: list, seed_data: dict) -> dict:
        ...
        has_ttg_markers = bool(genea.get('lineage_laws') or genea.get('bloodline_registry') or genea.get('tag_lexicon'))
        
        if has_founder:
            required_tags = ["audrey_001x", "transcend_binary", "human_genesis",
    ...
    def _check_redundancy(self, loci: list) -> dict:
        ...
        for i, (id1, n1) in enumerate(gene_names):
            for id2, n2 in gene_names[i+1:]:
                common_words = set(n1.replace('器', '').replace('者', '').split()) & \
                              set(n2.replace('器', '').replace('者', '').split())
...
class GeneFusionAnalyzer:
    ...
    def analyze_fusion(self, genes_a: list, genes_b: list,
                       name_a: str = "种子A", name_b: str = "种子B") -> dict:
        """分析两套基因融合的可行性
        ...
        for cap in capabilities:
            if cap in cap_to_gene:
                suggestions.append({
                    ...
                    "action": "insert" if cap_to_gene[cap].startswith("G1") or cap_to_gene[cap].startswith("G2") else "consider"
                })
