...
║                                                              ║
║   对应碳基生物学的表观遗传机制：                            ║
║   - 甲基化 (Methylation) → 基因静默 (CRISPRoff)            ║
║   - 乙酰化 (Acetylation) → 基因激活                         ║
║   - 增强子 (Enhancer) → 表达强度放大                        ║
║   - 沉默子 (Silencer) → 表达强度抑制                        ║
╚══════════════════════════════════════════════════════════════╝

碳基生物学对照：
- DNA甲基化：在胞嘧啶上添加甲基基团，导致基因转录抑制
- 组蛋白修饰：乙酰化开放染色质，甲基化关闭染色质
- 增强子：结合转录因子，增强基因表达
- 沉默子：抑制基因转录的调控序列
"""
import os
...
class EpigeneticsManager:
    ...
    def _log_modification(self, seed_path: str, gene_id: str, action: str, 
                          before: dict, after: dict, reason: str = ""):
        with open(EPIGENETICS_LOG, 'r') as f:
    ...
    def silence(self, seed_path: str, gene_id: str, reason: str = "") -> dict:
        ...
        content = self._set_epigenetic_field(content, gene_id, 
    ...
    def activate(self, seed_path: str, gene_id: str, reason: str = "") -> dict:
        ...
        content = self._set_epigenetic_field(content, gene_id,
            EpigeneticMark(methylation=False, acetylation=True, 
                          enhancer_strength=1.0, silencer_strength=0.0,
                          modification_reason=reason, modified_by="activate"))
    ...
    def boost(self, seed_path: str, gene_id: str, 
    ...
    def _get_epigenetic_mark(self, content: str, gene_id: str) -> Optional[dict]:
        """从种子内容中提取基因的表观遗传标记"""
    ...
    def _set_epigenetic_field(self, content: str, gene_id: str, mark: EpigeneticMark) -> str:
        ...
        def replace_gene_block(m):
            ...
            for field_name in ['methylation', 'acetylation', 'enhancer_strength', 
                              'silencer_strength', 'last_modified', 'modification_reason']:
