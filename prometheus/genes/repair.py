...
"""
    ...
    def _rollback_snapshot(self, seed_path: str, damage: DamageReport) -> RepairResult:
        ...
        """
        snapshots = sorted(
            [f for f in os.listdir(SNAPSHOT_DIR) if f.endswith(".ttg")], reverse=True
        )
...
"""
                ...
                if "tags:" in content:
                    ...
                    if tags_match:
                        ...
                        content = (
                            content[: tags_match.start()] + new_tags + content[tags_match.end() :]
                        )
    ...
    def _get_gene_from_bank(self, gene_id: str) -> dict | None:
        """从基因库获取基因定义"""
...
""")
    ...
    if not damages:
        ...
        for d in damages:
            severity_icon = (
                "🔴" if d.severity == "CRITICAL" else "🟠" if d.severity == "HIGH" else "🟡"
            )
