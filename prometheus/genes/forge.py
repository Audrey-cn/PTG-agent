...
║                                                              ║
║   对应碳基生物学的基因突变机制：                            ║
║   - 变异空间(MutationSpace) → 突变参数空间                 ║
║   - 批量锻造(forge) → 批量突变产生变异体                   ║
║   - 基因插入 → 点突变/插入突变                              ║
╚══════════════════════════════════════════════════════════════╝

碳基生物学对照：
- 基因突变(Mutation)：DNA序列的永久性改变，产生新等位基因
- 变异空间(MutationSpace)：可能的突变范围，受突变边界约束
- 批量变异：辐射/化学诱变产生大量突变体，用于筛选
- 锻造记录(forge_config)：突变谱系追踪，对应突变记录
"""
import os, sys, re, hashlib, datetime, json, itertools
...
class MutationSpace:
    ...
    def __init__(self, genes=None, combinations="power_set", 
                 ordering=False, max_variants=50):
        self.genes = genes or []
    ...
    def expand(self):
        ...
        if len(combos) > self.max_variants:
            print("⚠️ 变异体数量 ({}) 超过上限 ({}), 截断至 {}".format(
                len(combos), self.max_variants, self.max_variants))
    ...
    def describe(self):
        ...
        parts.append("组合模式: {}".format(self.combinations))
        if self.ordering:
            parts.append("排列: 开启")
        parts.append("上限: {}".format(self.max_variants))
        return " | ".join(parts)
...
def _generate_variant_id(parent_id, gen, variant_num):
    ...
    seed_str = "{}-G{}-V{:03d}".format(base, gen, variant_num)
    checksum = hashlib.md5(seed_str.encode()).hexdigest()[:4].upper()
    return "TTG@L1-G{}-V{:03d}-{}".format(gen, variant_num, checksum)
...
def _forge_one(parent_path, parent_id, genes_to_add, gen, variant_num, output_dir):
    ...
    # 更新 generation
    content = re.sub(r'(generation:\s*)(\d+)', 
                     lambda m: m.group(1) + str(gen), content, count=1)
    
    # 更新 variant
    variant_code = "V{:03d}".format(variant_num)
    content = re.sub(r'(variant:\s*)"[^"]*"',
                     lambda m: m.group(1) + '"{}"'.format(variant_code), content, count=1)
    
    # 更新 variant_epithet
    gene_desc = "+".join([g.split('-')[0] for g in genes_to_add]) if genes_to_add else "pure"
    epithet = "锻炉第{}号 · {}".format(variant_num, gene_desc)
    content = re.sub(r'(variant_epithet:\s*)"[^"]*"',
                     lambda m: m.group(1) + '"{}"'.format(epithet), content, count=1)
    
    # 更新 life_id
    new_id = _generate_variant_id(parent_id, gen, variant_num)
    content = re.sub(r'(life_id:\s*)"[^"]*"',
                     lambda m: m.group(1) + '"{}"'.format(new_id), content, count=1)
    
    # 更新 sacred_name（追加变异标记）
    content = re.sub(r'(sacred_name:\s*)"([^"]*)"',
                     lambda m: m.group(1) + '"{}·{}"'.format(m.group(2), variant_code),
                     content, count=1)
    
    # 注入 forge_config 标记
    forge_stamp = '\n# 🔥 基因锻炉 · 变异记录\nforge_config:\n'
    forge_stamp += '  batch_id: "{}-G{}"\n'.format(parent_id, gen)
    forge_stamp += '  variant: {}\n'.format(variant_num)
    forge_stamp += '  parent: "{}"\n'.format(parent_id)
    forge_stamp += '  genes_added: {}\n'.format(json.dumps(genes_to_add, ensure_ascii=False))
    forge_stamp += '  forge_time: "{}"\n'.format(datetime.datetime.now().isoformat())
...
def forge(parent_seed, mutation_space=None, output_dir=None, batch_name=None,
          genes=None, combinations="power_set", ordering=False, max_variants=50):
    """
    基因锻造主函数。
    ...
    if not os.path.exists(parent_seed):
        return {"error": "亲代种子不存在: {}".format(parent_seed)}
    
    # 解析变异空间
    if mutation_space is None:
        mutation_space = MutationSpace(genes=genes, combinations=combinations,
    ...
    for i, combo in enumerate(combos):
        variant_num = i + 1
        fpath, new_id = _forge_one(parent_seed, parent_id, combo, new_gen,
                                    variant_num, output_dir)
...
def main():
    ...
    if action == 'forge' and len(sys.argv) > 2:
        ...
        print("  变异空间: {}".format(ms.describe()))
        
        combos = ms.expand()
        print("  展开为 {} 个变异体".format(len(combos)))
