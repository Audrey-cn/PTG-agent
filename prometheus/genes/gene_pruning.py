...
"""
╔══════════════════════════════════════════════════════════════╗
║   🧬 基因表达剪枝 · Gene Expression Pruning                 ║
║                                                              ║
║   神经网络剪枝 · Neural Network Pruning for Genes          ║
║                                                              ║
║   对应碳基生物学：基因选择性表达、剪接变体、细胞分化     ║
║   动态启用/禁用基因，优化计算资源，类似神经网络剪枝       ║
╚══════════════════════════════════════════════════════════════╝

碳基生物学对照：
- 选择性剪接：同一基因产生不同mRNA变体
- 细胞分化：不同细胞表达不同基因
- 剪枝：去除非必要连接/基因
- 稀疏表达：只激活必要基因
- 发育调控：从全能到特化的基因表达变化
...
class GeneExpressionPruner:
    ...
    def add_connection(self, from_gene: str, to_gene: str,
                     strength: float = 0.5,
                     relation_type: str = "regulatory"):
        """添加基因连接"""
    ...
    def prune_by_usage(self, min_usage: int = 3) -> int:
        ...
        for gene_id, node in self.nodes.items():
            if (not node.is_essential and 
                node.usage_count < min_usage and
                node.is_active):
                node.deactivate()
                pruned_count += 1
        return pruned_count
    ...
    def prune_by_activity(self, inactivity_threshold: int = 10) -> int:
        ...
        for gene_id, node in self.nodes.items():
            try:
                ...
                if (not node.is_essential and
                    days_inactive > inactivity_threshold and
                    node.is_active):
                    node.deactivate()
                    pruned_count += 1
            except:
...
def print_pruning_dashboard(pruner: GeneExpressionPruner):
    ...
    print(f"\n╔══════════════════════════════════════════════════════════════╗")
    print(f"║   ✂️  基因表达剪枝 · Gene Expression Pruning Dashboard    ║")
    print(f"╠══════════════════════════════════════════════════════════════╣")
