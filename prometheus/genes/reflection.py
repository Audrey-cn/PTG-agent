...
class ObservationCollector:
    ...
    def _cleanup(self):
        """清理过期记录"""
        cutoff = (
            datetime.datetime.now() - datetime.timedelta(days=OBSERVATION_RETENTION_DAYS)
        ).isoformat()
...
class SelfReflection:
    ...
    def observe_tool_call(
        self, tool: str, success: bool, duration_ms: int = 0, detail: str = ""
    ) -> str:
        """记录工具调用"""
    ...
    def observe_seed_op(self, op: str, seed: str, success: bool, detail: str = "") -> str:
        """记录种子操作"""
        return self.collector.record(
...
def _analyze_state_health(transitions: list, current_state: str, entered_at: str = None) -> dict:
    """分析状态机健康度（独立函数，无状态）。
    ...
    if current_state == "reflecting" and entered_at:
        ...
        try:
            ...
            if duration_ms > reflecting_limit_ms:
                ...
                issues.append(
                    f"在 REFLECTING 状态停留过久: {duration_ms}ms (上限 {reflecting_limit_ms}ms)"
                )
        except (ValueError, TypeError):
            pass
...
""")
    ...
    if action == "observe" and len(extra_args) >= 2:
        ...
        obs_id = sr.collector.record(
            event_type, op_name, result=result, detail=detail, duration_ms=duration
        )
