...
║                                                              ║
║   对应碳基生物学的信号通路概念：                            ║
║   - 信号通路 (Pathway)：基因间的信号传递链                  ║
║   - 级联反应 (Cascade)：一个触发引发连锁响应                ║
║   - 反馈调节 (Feedback)：正反馈/负反馈循环                  ║
╚══════════════════════════════════════════════════════════════╝

碳基生物学对照：
- 信号通路：细胞内信号分子传递的路径，如MAPK通路、Wnt通路
- 级联反应：一个信号分子激活下一个，形成放大效应
- 正反馈：输出增强输入，形成自我放大
- 负反馈：输出抑制输入，形成稳态调节
"""
import os
...
class PathwayManager:
    ...
    def _load_catalog(self) -> dict:
        with open(PATHWAYS_CATALOG, 'r') as f:
            return json.load(f)
    ...
    def _log_execution(self, pathway_id: str, trigger_gene: str, trigger_event: str,
                       results: list):
        with open(PATHWAYS_LOG, 'r') as f:
    ...
    def register_pathway(self, pathway: SignalPathway) -> dict:
        ...
        if pathway.pathway_id in catalog["pathways"]:
            return {
                "success": False,
                "message": f"信号通路 {pathway.pathway_id} 已存在"
            }
    ...
    def trigger(self, seed_path: str, gene_id: str, event: str, 
    ...
    def _execute_cascade(self, seed_path: str, pathway: SignalPathway, 
    ...
    def _apply_feedback(self, seed_path: str, pathway: SignalPathway, 
                        last_step: PathwayStep):
        """应用反馈调节
        ...
        if pathway.feedback_type == "positive":
            mgr.boost(seed_path, pathway.feedback_target, enhancer=1.2,
                     reason="positive_feedback")
        elif pathway.feedback_type == "negative":
            mgr.boost(seed_path, pathway.feedback_target, silencer=0.3,
