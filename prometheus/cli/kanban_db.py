...
The board lives at ``$PROMETHEUS_HOME/kanban.db`` (profile-agnostic on purpose:
...
coding workloads.  See ``docs/prometheus-kanban-v1-spec.pdf`` for the full
...
def kanban_db_path() -> Path:
    """Return the path to ``kanban.db`` inside the active PROMETHEUS_HOME."""
    from prometheus.constants_core import get_prometheus_home

    return get_prometheus_home() / "kanban.db"
...
def workspaces_root() -> Path:
    ...
    return get_prometheus_home() / "kanban" / "workspaces"
...
class Task:
    ...
    def from_row(cls, row: sqlite3.Row) -> Task:
        ...
        return cls(
            ...
            current_run_id=(row["current_run_id"] if "current_run_id" in keys else None),
...
def init_db(db_path: Path | None = None) -> Path:
    ...
    Kept as a public entry point so CLI ``prometheus kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "spawn_failures" not in cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN spawn_failures INTEGER NOT NULL DEFAULT 0")
...
def create_task(
    ...
    *,
    ...
    each name to ``prometheus --skills ...`` alongside the built-in
    ...
    if workspace_kind not in VALID_WORKSPACE_KINDS:
        raise ValueError(
            f"workspace_kind must be one of {sorted(VALID_WORKSPACE_KINDS)}, got {workspace_kind!r}"
        )
    ...
    # `prometheus --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``prometheus kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``prometheus kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``prometheus kanban complete <id>``) works without requiring
...
def resolve_workspace(task: Task) -> Path:
    ...
    - ``scratch``: a fresh dir under ``$PROMETHEUS_HOME/kanban/workspaces/<id>/``.
    - ``dir:<path>``: the path stored in ``workspace_path``.  Created
      ...
      root.  Users who want a PROMETHEUS_HOME-relative workspace should
...
def _record_spawn_failure(
    ...
    *,
    ...
    with write_txn(conn):
        row = conn.execute(
            "SELECT spawn_failures FROM tasks WHERE id = ?",
            (task_id,),
        ).fetchone()
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``prometheus kanban
...
def _clear_spawn_failures(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    with write_txn(conn):
        conn.execute(
            "UPDATE tasks SET spawn_failures = 0, last_spawn_error = NULL WHERE id = ?",
            (task_id,),
        )
...
def _default_spawn(task: Task, workspace: str) -> int | None:
    """Fire-and-forget ``prometheus -p <profile> chat -q ...`` subprocess.
    ...
    env = dict(os.environ)
    if task.tenant:
        env["PROMETHEUS_TENANT"] = task.tenant
    env["PROMETHEUS_KANBAN_TASK"] = task.id
    env["PROMETHEUS_KANBAN_WORKSPACE"] = workspace
    # PROMETHEUS_PROFILE is the author the kanban_comment tool defaults to.
    # `prometheus -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["PROMETHEUS_PROFILE"] = task.assignee
    ...
    # Redirect output to a per-task log under PROMETHEUS_HOME/kanban/logs/.
    from prometheus.constants_core import get_prometheus_home

    log_dir = get_prometheus_home() / "kanban" / "logs"
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`prometheus` executable not found on PATH. "
            "Install Prometheus Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``prometheus kanban daemon`` is systemd-friendly.
...
def board_stats(conn: sqlite3.Connection) -> dict:
    ...
    now = int(time.time())
    oldest_ready_age = (
        (now - int(oldest_row["ts"])) if oldest_row and oldest_row["ts"] is not None else None
    )
...
def worker_log_path(task_id: str) -> Path:
    ...
    return get_prometheus_home() / "kanban" / "logs" / f"{task_id}.log"
...
def list_profiles_on_disk() -> list[str]:
    ...
    Reads ``~/.prometheus/profiles/`` directly so this module has no import
    dependency on ``prometheus_cli.profiles`` (which pulls in a large chunk
    ...
    """
    try:
        home = Path.home() / ".prometheus" / "profiles"
    except Exception:
        return []
    if not home.is_dir():
        return []
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``prometheus kanban assignees`` for the terminal.
