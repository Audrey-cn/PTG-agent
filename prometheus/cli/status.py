...
def redact_key(key: str) -> str:
    ...
    from prometheus.agent.redact import mask_secret
...
def show_status(args):
    ...
    nous_label = (
        "logged in"
        if nous_logged_in
        else "not logged in (run: prometheus auth add nous --type oauth)"
    )
    ...
    if _effective_provider_label() == "LM Studio":
        from prometheus.cli.models import probe_lmstudio_models
    ...
    if terminal_env == "ssh":
        ...
        runtime = (
            os.getenv("TERMINAL_VERCEL_RUNTIME") or terminal_cfg.get("vercel_runtime") or "node24"
        )
        ...
        sdk_label = (
            "installed" if sdk_ok else "missing (install: pip install 'prometheus-agent[vercel]')"
        )
        ...
        print(
            f"  Persistence:  {'snapshot filesystem' if persist_enabled else 'ephemeral filesystem'}"
        )
        print(
            "  Processes:    live processes do not survive cleanup, snapshots, or sandbox recreation"
        )
    ...
    try:
        from prometheus.cli.gateway import _format_gateway_pids, get_gateway_runtime_snapshot
