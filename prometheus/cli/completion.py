"""Shell completion script generation for prometheus CLI."""
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f'                    COMPREPLY=($(compgen -W "$(_prometheus_profiles)" -- "$cur"))\n'
                f"                    return\n"
                f"                    ;;\n"
                f"                *)\n"
    ...
    return f"""# Prometheus Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef prometheus
# Prometheus Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(prometheus completion zsh)"
...
_prometheus() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_prometheus_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'prometheus command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: List[str] = [
        "# Prometheus Agent fish completion",
        "# Add to your config:",
        "#   prometheus completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __prometheus_profiles",
        "    echo default",
        "    if test -d $HOME/.prometheus/profiles",
        "        ls $HOME/.prometheus/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "complete -c prometheus -f -s p -l profile -d 'Profile name' -xa '(__prometheus_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__prometheus_profiles)' -d 'Profile name'"
                )
