...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    """
    from prometheus.gateway.status import get_running_pid, read_runtime_status
    ...
    print()
    print_error("Prometheus gateway is running with active connections: " + ", ".join(connected))
    print_info(
...
def _cmd_migrate(args):
    ...
    if not source_dir.is_dir():
        ...
        print_info(
            "You can specify a custom path: prometheus claw migrate --source /path/to/.openclaw"
        )
        return
    ...
    if not no_backup:
        try:
            from prometheus.cli.backup import _format_size, create_pre_migration_backup
    ...
    for source_dir in dirs_to_check:
        ...
        try:
            workspace_dirs = [
                ...
                and any(
                    (d / name).exists() for name in ("todo.json", "SOUL.md", "MEMORY.md", "USER.md")
                )
            ]
        except OSError:
            workspace_dirs = []
