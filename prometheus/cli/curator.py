"""CLI subcommand: `prometheus curator <subcommand>`."""
...
def _cmd_status(args) -> int:
    from prometheus.curator import get_interval_hours, is_enabled, load_state
    from prometheus.tools.skill_usage import agent_created_report
    ...
    _ih = get_interval_hours()
    _interval_label = f"{_ih // 24}d" if _ih % 24 == 0 and _ih >= 24 else f"{_ih}h"
    print(f"  interval:       every {_interval_label}")
    print(f"  stale after:    {state.get('stale_after_days', 14)}d unused")
    print(f"  archive after:  {state.get('archive_after_days', 30)}d unused")
...
def _cmd_run(args) -> int:
    from prometheus.curator import is_enabled, run_curator_review
    ...
    if not args.synchronous:
        print("llm pass running in background — check `prometheus curator status` later")
    return 0


def _cmd_pause(args) -> int:
    from prometheus.curator import set_paused
...
def _cmd_pin(args) -> int:
    from prometheus.tools.skill_usage import is_agent_created, set_pinned

    if not is_agent_created(args.skill):
        print(
...
def _cmd_restore(args) -> int:
    from prometheus.tools.skill_usage import restore_skill

    ok, msg = restore_skill(args.skill)
    print(f"curator: {msg}")
...
def cli_main(argv=None) -> int:
    """Standalone entry."""
    parser = argparse.ArgumentParser(prog="prometheus curator")
    register_cli(parser)
