...
def get_nous_subscription_features(
    ...
    # Tool Gateway via `prometheus model`, so direct credentials should NOT
    ...
    managed_web_available = (
        managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready("firecrawl")
    )
    managed_image_available = (
        managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready("fal-queue")
    )
    managed_tts_available = (
        managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready("openai-audio")
    )
    managed_browser_available = (
        managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready("browser-use")
    )
    managed_modal_available = (
        managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready("modal")
    )
...
def apply_nous_managed_defaults(
    ...
    *,
    ...
    if (
        ...
        and not (get_env_value("BROWSER_USE_API_KEY") or get_env_value("BROWSERBASE_API_KEY"))
    ):
...
def _get_gateway_direct_credentials() -> dict[str, bool]:
    ...
    return {
        ...
        "tts": bool(resolve_openai_audio_api_key() or get_env_value("ELEVENLABS_API_KEY")),
        "browser": bool(
...
def prompt_enable_tool_gateway(config: dict[str, object]) -> set[str]:
    ...
    try:
        from prometheus.cli.setup import prompt_choice
    except Exception:
        return set()
