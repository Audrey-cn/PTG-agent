...
from prometheus.cli.config import get_env_path, get_project_root, get_prometheus_home
from prometheus.constants_core import display_prometheus_home

PROJECT_ROOT = get_project_root()
PROMETHEUS_HOME = get_prometheus_home()
_DPH = display_prometheus_home()
...
from prometheus.constants_core import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.prometheus/.env contains provider auth/base URL settings."""
...
def check_ok(text: str, detail: str = ""):
    print(
        f"  {color('✓', Colors.GREEN)} {text}" + (f" {color(detail, Colors.DIM)}" if detail else "")
    )
...
def check_fail(text: str, detail: str = ""):
    print(
        f"  {color('✗', Colors.RED)} {text}" + (f" {color(detail, Colors.DIM)}" if detail else "")
    )
...
def run_doctor(args):
    ...
    os.environ.setdefault("PROMETHEUS_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Prometheus Doctor                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    env_path = PROMETHEUS_HOME / ".env"
    if env_path.exists():
        check_ok(f"{_DPH}/.env file exists")

        content = env_path.read_text()
        if _has_provider_env_config(content):
            check_ok("API key or custom endpoint configured")
        else:
            check_warn(f"No API key found in {_DPH}/.env")
            issues.append("Run 'prometheus setup' to configure API keys")
    else:
        fallback_env = PROJECT_ROOT / ".env"
        if fallback_env.exists():
            check_ok(".env file exists (in project directory)")
        else:
            check_fail(f"{_DPH}/.env file missing")
            if should_fix:
                env_path.parent.mkdir(parents=True, exist_ok=True)
                env_path.touch()
                check_ok(f"Created empty {_DPH}/.env")
                check_info("Run 'prometheus setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'prometheus setup' to create one")
                issues.append("Run 'prometheus setup' to create .env")

    config_path = PROMETHEUS_HOME / "config.yaml"
    if config_path.exists():
        check_ok(f"{_DPH}/config.yaml exists")
        ...
        try:
            ...
            try:
                ...
                from prometheus.cli.providers import (
            ...
            if isinstance(user_providers, dict):
                known_providers.update(
                    str(name).strip().lower() for name in user_providers if str(name).strip()
                )
            ...
            if (
                ...
                and (
                    ...
                    or (known_providers and not (provider_ids_to_accept & valid_provider_ids))
                )
            ):
                known_list = (
                    ", ".join(sorted(known_providers)) if known_providers else "(unavailable)"
                )
                check_fail(
                ...
                issues.append(
                    ...
                    f"Fix: run 'prometheus config set model.provider <valid_provider>'"
                )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom", "openrouter"):
                try:
                    from prometheus.cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.prometheus/.env or run 'prometheus setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'prometheus setup' or set the provider's API key in {_DPH}/.env, "
                                f"or switch providers with 'prometheus config set model.provider <name>'"
                            )
                except Exception:
                    pass
        ...
        if fallback_config.exists():
            ...
            if should_fix and example_config.exists():
                ...
                check_ok(f"Created {_DPH}/config.yaml from cli-config.yaml.example")
                fixed_count += 1
            elif should_fix:
                check_warn("config.yaml not found and no example to copy from")
                manual_issues.append(f"Create {_DPH}/config.yaml manually")
            else:
    ...
    if config_path.exists():
        try:
            from prometheus.cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'prometheus setup' to migrate config")
                else:
                    issues.append(
                        "Run 'prometheus doctor --fix' or 'prometheus setup' to migrate config"
                    )
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from prometheus.utils import atomic_yaml_write
                    ...
                    issues.append(
                        "Stale root-level provider/base_url in config.yaml — run 'prometheus doctor --fix'"
                    )
        except Exception:
            pass

        try:
            from prometheus.cli.config import validate_config_structure
    ...
    if prometheus_home.exists():
        check_ok(f"{_DPH} directory exists")
    else:
        if should_fix:
            prometheus_home.mkdir(parents=True, exist_ok=True)
            check_ok(f"Created {_DPH} directory")
            fixed_count += 1
        else:
            check_warn(f"{_DPH} not found", "(will be created on first use)")

    expected_subdirs = ["cron", "sessions", "logs", "skills", "memories"]
    for subdir_name in expected_subdirs:
        subdir_path = prometheus_home / subdir_name
        if subdir_path.exists():
            check_ok(f"{_DPH}/{subdir_name}/ exists")
        else:
            if should_fix:
                subdir_path.mkdir(parents=True, exist_ok=True)
                check_ok(f"Created {_DPH}/{subdir_name}/")
                fixed_count += 1
            else:
                check_warn(f"{_DPH}/{subdir_name}/ not found", "(will be created on first use)")
    ...
    if soul_path.exists():
        ...
        if lines:
            check_ok(f"{_DPH}/SOUL.md exists (persona configured)")
        else:
            check_info(f"{_DPH}/SOUL.md exists but is empty — edit it to customize personality")
    else:
        check_warn(
            f"{_DPH}/SOUL.md not found", "(create it to give Prometheus a custom personality)"
        )
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Prometheus Agent Persona\n\n"
                "<!-- Edit this file to customize how Prometheus communicates. -->\n\n"
                "You are Prometheus, a helpful AI assistant.\n",
                encoding="utf-8",
            )
            check_ok(f"Created {_DPH}/SOUL.md with basic template")
            fixed_count += 1

    memories_dir = prometheus_home / "memories"
    if memories_dir.exists():
        check_ok(f"{_DPH}/memories/ directory exists")
        ...
        check_warn(f"{_DPH}/memories/ not found", "(will be created on first use)")
        if should_fix:
            memories_dir.mkdir(parents=True, exist_ok=True)
            check_ok(f"Created {_DPH}/memories/")
            fixed_count += 1

    state_db_path = prometheus_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            conn.close()
            check_ok(f"{_DPH}/state.db exists ({count} sessions)")
        except Exception as e:
            check_warn(f"{_DPH}/state.db exists but has issues: {e}")
    else:
        check_info(f"{_DPH}/state.db not created yet (will be created on first session)")

    wal_path = prometheus_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'prometheus doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "prometheus"
        ...
        _cmd_link = _cmd_link_dir / "prometheus"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(prometheus not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')",
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/prometheus → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/prometheus points to wrong target",
                        f"(→ {_target}, expected → {_expected})",
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/prometheus → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(
                            f"Broken symlink at {_cmd_link_display}/prometheus — run 'prometheus doctor --fix'"
                        )
            elif _cmd_link.exists():
                check_ok(f"{_cmd_link_display}/prometheus exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/prometheus not found",
                    "(prometheus command may not work outside the venv)",
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/prometheus → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(
                        f"Missing {_cmd_link_display}/prometheus symlink — run 'prometheus doctor --fix'"
                    )
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'prometheus-agent[vercel]')")
            issues.append(
                "Install the Vercel optional dependency: pip install 'prometheus-agent[vercel]'"
            )
    ...
    if openrouter_key:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"\r  {color('✗', Colors.RED)} OpenRouter API {color('(out of credits — payment required)', Colors.DIM)}"
                )
                issues.append(
                    "OpenRouter account has insufficient credits. "
                    "Fix: run 'prometheus config set model.provider <provider>' to switch providers, "
                    "or fund your OpenRouter account at https://openrouter.ai/settings/credits"
                )
            elif response.status_code == 429:
                print(
                    f"\r  {color('✗', Colors.RED)} OpenRouter API {color('(rate limited)', Colors.DIM)}                "
                )
                issues.append(
    ...
    from prometheus.cli.auth import get_anthropic_key
    ...
    if anthropic_key:
        ...
        try:
            ...
            response = httpx.get("https://api.anthropic.com/v1/models", headers=headers, timeout=10)
            if (
                is_oauth
        ...
        if _key:
            ...
            try:
                ...
                if _base and _base.rstrip("/").endswith("/anthropic"):
                    from prometheus.agent.auxiliary_client import _to_openai_base_url

                    _base = _to_openai_base_url(_base)
                if base_url_host_matches(_base, "api.kimi.com") and _base.rstrip("/").endswith(
                    "/coding"
                ):
    ...
    try:
        ...
        if has_aws_credentials():
            ...
            try:
                ...
                print(
                    f"\r  {color('✓', Colors.GREEN)} {_label} {color(f'({_auth_var}, {_region}, {_model_count} models)', Colors.DIM)}           "
                )
            except ImportError:
                print(
                    f"\r  {color('⚠', Colors.YELLOW)} {_label} {color(f'(boto3 not installed — {sys.executable} -m pip install boto3)', Colors.DIM)}           "
                )
                issues.append(f"Install boto3 for Bedrock: {sys.executable} -m pip install boto3")
            except Exception as _e:
                _err_name = type(_e).__name__
                print(
                    f"\r  {color('⚠', Colors.YELLOW)} {_label} {color(f'({_err_name}: {_e})', Colors.DIM)}           "
                )
                issues.append(
                    f"AWS Bedrock: {_err_name} — check IAM permissions for bedrock:ListFoundationModels"
                )
    except ImportError:
        pass
    ...
    try:
        ...
        if api_disabled:
            issues.append(
                "Run 'prometheus setup' to configure missing API keys for full tool access"
            )
    except Exception as e:
    ...
    hub_dir = PROMETHEUS_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: prometheus skills list)")
    ...
    if github_token:
        ...
        check_warn(
            "No GITHUB_TOKEN", f"(60 req/hr rate limit — set in {_DPH}/.env for better rates)"
        )
    ...
    try:
        ...
        _mem_cfg_path = PROMETHEUS_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: prometheus memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: prometheus memory setup")
                issues.append("No Honcho API key — run 'prometheus memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail(
                    ...
                    "(set MEM0_API_KEY in .env or run prometheus memory setup)",
                )
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(
                    ...
                    "run: prometheus memory status",
                )
            else:
                check_warn(
                    f"{_active_memory_provider} plugin not found", "run: prometheus memory setup"
                )
        except Exception as _e:
    ...
    try:
        ...
        from prometheus.cli.profiles import _get_wrapper_dir, list_profiles, profile_exists
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "prometheus -p" in content:
                            _m = _re.search(r"prometheus -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print(
            color(
                f"  Found {len(remaining_issues)} issue(s) to address:", Colors.YELLOW, Colors.BOLD
            )
        )
        print()
        for i, issue in enumerate(remaining_issues, 1):
        ...
        if not should_fix:
            print(
                color(
                    "  Tip: run 'prometheus doctor --fix' to auto-fix what's possible.", Colors.DIM
                )
            )
    else:
        print(color("─" * 60, Colors.GREEN))
