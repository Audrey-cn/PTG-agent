...
def _cron_api(**kwargs):
    from prometheus.tools.cronjob_tools import cronjob as cronjob_tool
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(
            color(
                "Create one with 'prometheus cron create ...' or the /cron command in chat.",
                Colors.DIM,
            )
        )
        return
    ...
    from prometheus.cli.gateway import find_gateway_pids
    ...
    if not find_gateway_pids():
        ...
        print(
            color(
                "                    sudo prometheus gateway install --system  # Linux servers",
                Colors.DIM,
            )
        )
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print(
            "    sudo prometheus gateway install --system  # Linux servers: boot-time system service"
        )
