...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    """
    from prometheus.tools.skills_hub import unified_search
...
def _existing_categories() -> list[str]:
    """Return sorted subdirectory names under ``~/.prometheus/skills/`` that look
    ...
    """
    from prometheus.tools.skills_hub import SKILLS_DIR
...
def _prompt_for_category(c: Console, existing: list[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.prometheus/skills/<name>/)[/]"
        )
    try:
...
def do_search(
    query: str, source: str = "all", limit: int = 10, console: Console | None = None
) -> None:
    """Search registries and display results as a Rich table."""
    from prometheus.tools.skills_hub import GitHubAuth, create_source_router, unified_search
    ...
    c.print(table)
    c.print(
        "[dim]Use: prometheus skills inspect <identifier> to preview, "
        "prometheus skills install <identifier> to install[/]\n"
    )


def do_browse(
    page: int = 1, page_size: int = 20, source: str = "all", console: Console | None = None
) -> None:
    """Browse all available skills across registries, paginated.

    Official skills are always shown first, regardless of source filter.
    """
    from prometheus.tools.skills_hub import (
        GitHubAuth,
    ...
    c.print(
        "[dim]Tip: 'prometheus skills search <query>' searches deeper across all registries[/]\n"
    )
...
def do_install(
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]prometheus skills install {url} --name <your-name>[/]\n\n"
    ...
    try:
        ...
        from prometheus.tools.skills_hub import append_audit_log

        append_audit_log(
            "BLOCKED", bundle.name, bundle.source, bundle.trust_level, "invalid_path", str(exc)
        )
        return
        ...
        if bundle.source == "official":
            c.print(
                Panel(
                    ...
                    "It ships with prometheus-agent but is not activated by default.\n"
                    "Installing will copy it to your skills directory where the agent can use it.\n\n"
                    f"Files will be at: [cyan]{display_prometheus_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def do_inspect(identifier: str, console: Console | None = None) -> None:
    ...
    from prometheus.tools.skills_hub import GitHubAuth, create_source_router
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(
            Panel(
                ...
                subtitle="prometheus skills install <id> to install",
            )
        )
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    """Paginated hub browse for programmatic callers."""
    from prometheus.tools.skills_hub import GitHubAuth, create_source_router
    ...
    deduped = list(seen.values())
    deduped.sort(
        key=lambda r: (-_TRUST_RANK.get(r.trust_level, 0), r.source != "official", r.name.lower())
    )
    total = len(deduped)
    total_pages = max(1, (total + page_size - 1) // page_size)
    page = max(1, min(page, total_pages))
...
def do_list(
    source_filter: str = "all", enabled_only: bool = False, console: Console | None = None
) -> None:
    """List installed skills, distinguishing hub, builtin, and local skills."""
    from prometheus.agent.skill_utils import get_disabled_skill_names
    from prometheus.tools.skills_hub import HubLockFile, ensure_hub_dirs
    from prometheus.tools.skills_sync import _read_manifest
    from prometheus.tools.skills_tool import _find_all_skills
    ...
    for skill in sorted(all_skills, key=lambda s: (s.get("category") or "", s["name"])):
        ...
        table.add_row(
            name, category, source_display, f"[{trust_style}]{trust_label}[/]", status_cell
        )
...
def do_check(name: str | None = None, console: Console | None = None) -> None:
    ...
    from prometheus.tools.skills_hub import check_for_skill_updates
...
def do_update(name: str | None = None, console: Console | None = None) -> None:
    ...
    from prometheus.tools.skills_hub import HubLockFile, check_for_skill_updates
...
def do_audit(name: str | None = None, console: Console | None = None) -> None:
    ...
    from prometheus.tools.skills_guard import format_scan_report, scan_skill
    from prometheus.tools.skills_hub import SKILLS_DIR, HubLockFile
...
def do_uninstall(
    ...
    from prometheus.tools.skills_hub import uninstall_skill
...
def do_reset(
    ...
    from prometheus.tools.skills_sync import reset_bundled_skill
...
def do_tap(action: str, repo: str = "", console: Console | None = None) -> None:
    ...
    from prometheus.tools.skills_hub import TapsManager
    ...
    if action == "list":
        ...
        if not repo:
            c.print(
                "[bold red]Error:[/] Repo required. Usage: prometheus skills tap add owner/repo\n"
            )
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print(
                "[bold red]Error:[/] Repo required. Usage: prometheus skills tap remove owner/repo\n"
            )
            return
        if mgr.remove(repo):
...
def do_publish(
    skill_path: str, target: str = "github", repo: str = "", console: Console | None = None
) -> None:
    """Publish a local skill to a registry (GitHub PR or ClawHub submission)."""
    from prometheus.tools.skills_guard import format_scan_report, scan_skill
    from prometheus.tools.skills_hub import SKILLS_DIR, GitHubAuth
    ...
    if target == "github":
        if not repo:
            c.print(
                ...
                "Usage: prometheus skills publish <path> --to github --repo owner/repo\n"
            )
            return
        ...
        if not auth.is_authenticated():
            c.print(
                ...
                f"Set GITHUB_TOKEN in {display_prometheus_home()}/.env or run 'gh auth login'.\n"
            )
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Prometheus Skills Hub.\n\n"
                f"This skill was scanned by the Prometheus Skills Guard before submission.",
...
def do_snapshot_export(output_path: str, console: Console | None = None) -> None:
    ...
    from prometheus.tools.skills_hub import HubLockFile, TapsManager
...
def do_snapshot_import(
    input_path: str, force: bool = False, console: Console | None = None
) -> None:
...
def skills_command(args) -> None:
    """Router for `prometheus skills <subcommand>` — called from prometheus.cli.main."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: prometheus skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: prometheus skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print(
            "Usage: prometheus skills [browse|search|install|inspect|list|check|update|audit|uninstall|reset|publish|snapshot|tap]\n"
        )
        _console.print("Run 'prometheus skills <command> --help' for details.\n")
...
def handle_skills_slash(cmd: str, console: Console | None = None) -> None:
    ...
    if action == "browse":
        ...
        invalidate_cache = "--now" in args
        do_uninstall(
            args[0], console=c, skip_confirm=skip_confirm, invalidate_cache=invalidate_cache
        )
        ...
        invalidate_cache = "--now" in args
        do_reset(
            name, restore=restore, console=c, skip_confirm=True, invalidate_cache=invalidate_cache
        )
