...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from prometheus.cli.cli_output import prompt as _shared_prompt
...
def _probe_single_server(
    ...
    """
    from prometheus.tools.mcp_tool import (
        _connect_server,
        _ensure_mcp_loop,
        _run_on_mcp_loop,
        _stop_mcp_loop,
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info(
            "  prometheus mcp add github --command npx --args @modelcontextprotocol/server-github"
        )
    ...
    if url and auth_type == "oauth":
        ...
        oauth_ok = False
        try:
            from prometheus.tools.mcp_oauth_manager import get_manager
    ...
    try:
        choice = (
            input(color(f"  Enable all {len(tools)} tools? [Y/n/select]: ", Colors.YELLOW))
            .strip()
            .lower()
        )
    except (KeyboardInterrupt, EOFError):
    ...
    print()
    _success(
        f"Saved '{name}' to {display_prometheus_home()}/config.yaml ({tool_count}/{total} tools enabled)"
    )
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print(
            "Error: 'prometheus mcp configure' requires an interactive terminal.", file=_sys.stderr
        )
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
