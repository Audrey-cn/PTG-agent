"""CLI for the Prometheus Kanban board — ``prometheus kanban …`` subcommand.

Exposes the full 15-verb surface documented in the design spec
(``docs/prometheus-kanban-v1-spec.pdf``).  All DB work is delegated to
...
def _parse_workspace_flag(value: str) -> tuple[str, str | None]:
    ...
    if v.startswith("dir:"):
        ...
        if not path:
            raise argparse.ArgumentTypeError("--workspace dir: requires a path after the colon")
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this PROMETHEUS_HOME and its
    ...
    Used by ``prometheus kanban create`` (and callers) to warn when a task
    ...
    """
    try:
        from prometheus.gateway.status import get_running_pid  # type: ignore
    except Exception:
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`prometheus kanban daemon --force`).",
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Prometheus profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://prometheus-agent.nousresearch.com/docs/user-guide/features/kanban "
            "or docs/prometheus-kanban-v1-spec.pdf for the full design."
        ),
    )
    ...
    p_create.add_argument(
        "--workspace", default="scratch", help="scratch | worktree | dir:<path> (default: scratch)"
    )
    ...
    p_create.add_argument(
        "--created-by", default="user", help="Author name recorded on the task (default: user)"
    )
    p_create.add_argument(
        "--skill",
        action="append",
    ...
    p_list.add_argument(
        "--mine", action="store_true", help="Filter by $PROMETHEUS_PROFILE as assignee"
    )
    ...
    p_comment.add_argument(
        "--author", default=None, help="Author name (default: $PROMETHEUS_PROFILE or 'user')"
    )

    p_complete = sub.add_parser("complete", help="Mark one or more tasks done")
    p_complete.add_argument(
        "task_ids", nargs="+", help="One or more task ids (only --result applies to all of them)"
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `prometheus gateway start`.",
    )
    p_daemon.add_argument(
        "--interval", type=float, default=60.0, help="Seconds between dispatch ticks (default: 60)"
    )
    ...
    p_watch.add_argument(
        "--assignee", default=None, help="Only show events for tasks assigned to this profile"
    )
    p_watch.add_argument(
    ...
    p_log = sub.add_parser(
        "log",
        help="Print the worker log for a task (from $PROMETHEUS_HOME/kanban/logs/)",
    )
    ...
    p_hb.add_argument(
        "--note", default=None, help="Optional short note attached to the heartbeat event"
    )
    ...
    p_asg = sub.add_parser(
        ...
        "(union of ~/.prometheus/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``prometheus kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: prometheus kanban <action> [options]\n"
                "Run 'prometheus kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # PROMETHEUS_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("PROMETHEUS_PROFILE_NAME", "PROMETHEUS_PROFILE"):
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.prometheus/profiles/.")
        print("Create one with `prometheus -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  prometheus gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `prometheus -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        payload = {
            ...
            "children": children,
            "comments": [
                {"author": c.author, "body": c.body, "created_at": c.created_at} for c in comments
            ],
            "events": [
                {
                    "kind": e.kind,
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "prometheus kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    prometheus gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`prometheus kanban list --status ready` / "
                    f"`prometheus kanban list --status blocked` for recent "
