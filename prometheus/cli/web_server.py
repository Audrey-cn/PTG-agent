...
from prometheus.config import PrometheusConfig, get_prometheus_home
...
def _create_app():
    ...
    _config: PrometheusConfig | None = None
        ...
        for provider, data in providers.items():
            if isinstance(data, dict) and "models" in data:
                for model in data["models"]:
                    models.append({"provider": provider, "model": model, "name": model})
        return models
    ...
    async def list_sessions():
        return [{"id": sid, "data": data} for sid, data in _sessions.items()]
        ...
        if skills_dir.exists():
            for skill_path in skills_dir.iterdir():
                if skill_path.is_dir():
                    ...
                    if skill_md.exists():
                        skills.append(
                            {"name": skill_path.name, "path": str(skill_path), "has_skill_md": True}
                        )
        return skills
        ...
        try:
            while True:
                ...
                await websocket.send_json(
                    {"type": "ack", "message": "Received: " + message.get("text", str(message))}
                )
