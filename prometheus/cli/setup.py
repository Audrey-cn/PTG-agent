...
def _supports_same_provider_pool_setup(provider: str) -> bool:
    ...
    if provider == "openrouter":
        return True
    from prometheus.cli.auth import PROVIDER_REGISTRY
...
def _print_setup_summary(config: dict, prometheus_home):
    ...
    try:
        from prometheus.agent.auxiliary_client import get_available_vision_backends
    ...
    if _vision_backends:
        ...
        tool_status.append(
            ("Vision (image analysis)", False, "run 'prometheus setup' to configure")
        )
    ...
    if subscription_features.browser.managed_by_nous:
        ...
        missing_browser_hint = (
            "npm install -g agent-browser, set CAMOFOX_URL, or configure Browser Use or Browserbase"
        )
        if browser_provider == "Browserbase":
            missing_browser_hint = (
                "npm install -g agent-browser and set BROWSERBASE_API_KEY/BROWSERBASE_PROJECT_ID"
            )
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(
                ("Text-to-Speech (NeuTTS — not installed)", False, "run 'prometheus setup tts'")
            )
        ...
        if kittentts_ok:
            ...
            tool_status.append(
                ("Text-to-Speech (KittenTTS — not installed)", False, "run 'prometheus setup tts'")
            )
    else:
    ...
    try:
        from prometheus.cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning("Some tools are disabled. Run 'prometheus setup tools' to configure them,")
    ...
    print(f"   {color('Data:', Colors.YELLOW)}      {prometheus_home}/cron/, sessions/, logs/")
    print()
    ...
    print(f"   {color('prometheus config edit', Colors.GREEN)}    Open config in your editor")
...
def _prompt_vercel_sandbox_settings(config: dict):
    ...
    from prometheus.tools.terminal_tool import _SUPPORTED_VERCEL_RUNTIMES
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from prometheus.cli.main import select_provider_and_model
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from prometheus.cli.auth_commands import auth_add_command

            pool = load_pool(selected_provider)
            entries = pool.entries()
            entry_count = len(entries)
            manual_count = sum(
                1 for entry in entries if str(getattr(entry, "source", "")).startswith("manual")
            )
            ...
            print_info("them when a credential is exhausted or rate-limited. This preserves")
            print_info("your primary provider while reducing interruptions from quota issues.")
            print()
            if auto_count > 0:
                print_info(
    ...
    if _vision_needs_setup:
        ...
        if _vision_idx == 0:
            ...
            _base_url = (
                prompt("  Base URL (blank for OpenAI)").strip() or "https://api.openai.com/v1"
            )
            ...
            print_info(
                "Skipped — add later with 'prometheus setup' or configure AUXILIARY_VISION_* settings"
            )
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        current_image = cfg_get(
            config, "terminal", "docker_image", default="nikolaik/python-nodejs:python3.11-nodejs20"
        )
        ...
        from prometheus.tools.tool_backend_helpers import normalize_modal_mode
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning(
                    "Install failed — run manually: pip install 'prometheus-agent[vercel]'"
                )
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
    ...
    if selected_backend == "vercel_sandbox":
        save_env_value(
            "TERMINAL_VERCEL_RUNTIME", config["terminal"].get("vercel_runtime", "node24")
        )
    save_config(config)
    print()
...
def _setup_slack():
    ...
    print()
    print_info(
        "   Full guide: https://prometheus-agent.nousresearch.com/docs/user-guide/messaging/slack/"
    )
    print()
...
def _write_slack_manifest_and_instruct():
    ...
    """
    try:
        from prometheus.cli.slack_cli import _build_full_manifest
        ...
        print_info("   You can generate it manually later with: prometheus slack manifest --write")
...
def _setup_qqbot():
    ...
    from prometheus.cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info(
        "   Full guide: https://prometheus-agent.nousresearch.com/docs/user-guide/messaging/webhooks/"
    )
    print()
    ...
    print_info(
        "   https://prometheus-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes"
    )
    print()
    print_info("   Open config in your editor:  prometheus config edit")
    print_info("   Open config in your editor:  prometheus config edit")
...
def setup_gateway(config: dict):
    ...
    from prometheus.cli.gateway import _all_platforms, _configure_platform, _platform_status
    ...
    def _is_progress(status: str) -> bool:
        s = status.lower()
        return not (
            s == "not configured" or s.startswith("partially") or s.startswith("plugin disabled")
        )
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(f"     prometheus config set {plat.upper()}_HOME_CHANNEL <channel_id>")
        ...
        from prometheus.cli.gateway import (
        ...
        if service_running:
            ...
            from prometheus.constants_core import is_container
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    from prometheus.cli.tools_config import tools_command
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    try:
        from prometheus.cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> str | None:
    ...
    if section_key == "model":
        ...
        from prometheus.cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_prometheus", _OPENCLAW_SCRIPT)
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(
            color("  Would overwrite (conflicts with existing Prometheus config):", Colors.YELLOW)
        )
    ...
    if warnings_shown:
        ...
        print(
            color(
                "  Note: OpenClaw config values may have different semantics in Prometheus.",
                Colors.YELLOW,
            )
        )
        print(
            color(
                "  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Prometheus's yolo mode.",
                Colors.YELLOW,
            )
        )
        print(
...
def _offer_openclaw_migration(prometheus_home: Path) -> bool:
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info("Migration cancelled. You can run it later with: prometheus claw migrate")
    ...
    if conflicts:
        print_info(
            f"Skipped {conflicts} item(s) that already exist in Prometheus (use prometheus claw migrate --overwrite to force)."
        )
    if skipped:
...
def run_setup_wizard(args):
    ...
    """
    from prometheus.config import is_managed, managed_error
    ...
    print(color("│  Let's configure your Prometheus Agent installation.       │", Colors.MAGENTA))
...
def _run_quick_setup(config: dict, prometheus_home):
    ...
    has_anything_missing = (
        missing_required or missing_optional or missing_config or current_ver < latest_ver
    )
