...
def _get_default_prometheus_home() -> Path:
    ...
    """
    from prometheus.constants_core import get_default_prometheus_root
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    try:
        from prometheus.gateway.status import get_running_pid
...
def create_profile(
    ...
    if not soul_path.exists():
        try:
            from prometheus.cli.default_soul import DEFAULT_SOUL_MD
    ...
    try:
        result = subprocess.run(
            [
                sys.executable,
                "-c",
                "import json; from prometheus.tools.skills_sync import sync_skills; "
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if has_wrapper and remove_wrapper_script(name):
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from prometheus.cli.gateway import get_launchd_plist_path, get_service_name
...
def set_active_profile(name: str) -> None:
    ...
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. Create it with: prometheus profile create {name}"
        )
