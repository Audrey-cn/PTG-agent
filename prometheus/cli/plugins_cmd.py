...
def _resolve_git_url(identifier: str) -> str:
    ...
    raise ValueError(
        f"Invalid plugin identifier: '{identifier}'. Use a Git URL or owner/repo shorthand."
    )
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from prometheus.cli.config import get_env_value, save_env_value

    prometheus_home_display = get_prometheus_home_display()
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(
                f"\n[dim]  Skipped (you can set these later in {prometheus_home_display}/.env)[/dim]"
            )
            return
        ...
        if value:
            ...
            console.print(f"  [green]✓[/green] Saved to {prometheus_home_display}/.env")
        else:
            console.print(
                f"  [dim]  Skipped (set {name} in {prometheus_home_display}/.env later)[/dim]"
            )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                from prometheus.cli.config import recommended_update_command
    ...
    if user_dir.is_dir():
        ...
        for child in user_dir.iterdir():
            ...
            if manifest.get("name") == name:
                return True
    from prometheus.cli.plugins import get_bundled_plugins_dir

    repo_plugins = get_bundled_plugins_dir()
    if repo_plugins.is_dir():
        candidate = repo_plugins / name
        if candidate.is_dir() and (
            (candidate / "plugin.yaml").exists() or (candidate / "plugin.yml").exists()
        ):
            return True
    return False
...
def _discover_all_plugins() -> list:
    """Return a list of (name, version, description, source, dir_path) for every plugin."""
    try:
        import yaml
    except ImportError:
        yaml = None
...
def _discover_memory_providers() -> list[Tuple[str, str]]:
    ...
    try:
        from prometheus.plugins.memory import discover_memory_providers
...
def _discover_context_engines() -> list[Tuple[str, str]]:
    ...
    try:
        from prometheus.plugins.context_engine import discover_context_engines
...
def cmd_toggle() -> None:
    ...
    try:
        ...
        _run_composite_ui(
            curses, plugin_names, plugin_labels, plugin_selected, disabled_set, categories, console
        )
    except ImportError:
        _run_composite_fallback(
            plugin_names, plugin_labels, plugin_selected, disabled_set, categories, console
        )
...
def _run_composite_ui(
    ...
    from prometheus.cli.curses_ui import flush_stdin
