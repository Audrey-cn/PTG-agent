...
import prometheus.cli.auth as auth_mod
...
def _extract_retry_delay_seconds(message: str) -> float | None:
    ...
    sec_match = re.search(
        r"retry\s+(?:after\s+)?(\d+(?:\.\d+)?)\s*(?:sec|secs|seconds|s\b)", message, re.IGNORECASE
    )
    if sec_match:
...
def _iter_custom_providers(config: dict | None = None):
    ...
    if not isinstance(custom_providers, list):
        ...
        try:
            from prometheus.cli.config import get_compatible_custom_providers
...
def get_custom_provider_pool_key(base_url: str) -> str | None:
...
class CredentialPool:
    ...
    def _sync_anthropic_entry_from_credentials_file(
        ...
        if self.provider != "anthropic" or entry.source != "claude_code":
            return entry
        try:
            from prometheus.agent.anthropic_adapter import read_claude_code_credentials
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        try:
            ...
            if store_access and (
                store_access != entry_access or (store_refresh and store_refresh != entry_refresh)
            ):
                logger.debug(
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> PooledCredential | None:
        ...
        try:
            if self.provider == "anthropic":
                from prometheus.agent.anthropic_adapter import refresh_anthropic_oauth_pure
...
def _seed_from_singletons(provider: str, entries: list[PooledCredential]) -> tuple[bool, set[str]]:
    ...
    if provider == "anthropic":
        ...
        try:
            from prometheus.cli.copilot_auth import get_copilot_api_token, resolve_copilot_token
        ...
        try:
            from prometheus.cli.auth import resolve_qwen_runtime_credentials
...
def _seed_from_env(provider: str, entries: list[PooledCredential]) -> tuple[bool, set[str]]:
    ...
    try:
        from prometheus.cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
