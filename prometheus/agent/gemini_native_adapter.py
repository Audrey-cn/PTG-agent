...
from prometheus.agent.gemini_schema import sanitize_gemini_tool_parameters
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Prometheus)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Prometheus typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Prometheus retry/error classification."""
...
def _translate_tool_result_to_gemini(
    ...
    name = str(
        message.get("name") or tool_name_by_call_id.get(tool_call_id) or tool_call_id or "tool"
    )
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.prometheus/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `prometheus setup` "
    ...
    def _headers(self) -> dict[str, str]:
        headers = {
            ...
            "User-Agent": "prometheus-agent (gemini-native)",
        }
    ...
    ) -> Tuple[bool, _GeminiStreamChunk | None]:
        try:
