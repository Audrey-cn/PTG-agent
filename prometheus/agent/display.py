...
from prometheus.utils import safe_json_loads

_ANSI_RESET = "\033[0m"

diff_colors_cached: dict[str, str] | None = None


def _diff_ansi() -> dict[str, str]:
    """Return ANSI escapes for diff display, resolved from the active skin."""
    global diff_colors_cached
    if diff_colors_cached is not None:
        return diff_colors_cached
...
def _resolve_skill_manage_paths(args: dict) -> list[Path]:
    ...
    from prometheus.tools.skill_manager_tool import _find_skill, _resolve_skill_dir
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Prometheus' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("PROMETHEUS_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def get_cute_tool_message(
    ...
    if tool_name == "cronjob":
        ...
        if action == "create":
            ...
            label = (
                args.get("name") or (skills[0] if skills else None) or args.get("prompt", "task")
            )
