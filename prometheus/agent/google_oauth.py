...
#
# Resolution order:
#   1. PROMETHEUS_GEMINI_CLIENT_ID / PROMETHEUS_GEMINI_CLIENT_SECRET env vars (power users)
...
ENV_CLIENT_ID = "PROMETHEUS_GEMINI_CLIENT_ID"
ENV_CLIENT_SECRET = "PROMETHEUS_GEMINI_CLIENT_SECRET"
...
_CLIENT_SECRET_PATTERN = _re.compile(r"OAUTH_CLIENT_SECRET\s*=\s*['\"](GOCSPX-[A-Za-z0-9_-]+)['\"]")
...
_HEADLESS_ENV_VARS = ("SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "PROMETHEUS_HEADLESS")
...
def _credentials_path() -> Path:
    return get_prometheus_home() / "auth" / "google_oauth.json"
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "Prometheus looks for a locally installed gemini-cli to source the OAuth client. "
            "Either:\n"
            "  1. Install it: npm install -g @google/gemini-cli  (or brew install gemini-cli)\n"
            "  2. Set PROMETHEUS_GEMINI_CLIENT_ID and PROMETHEUS_GEMINI_CLIENT_SECRET in ~/.prometheus/.env\n"
            "\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `prometheus login --provider google-gemini-cli` first.",
...
class _OAuthCallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if state != type(self).expected_state:
            ...
            self._respond_html(
                400, _ERROR_PAGE.format(message="Callback received no authorization code.")
            )
...
<html><head><meta charset="utf-8"><title>Prometheus — signed in</title>
<style>
...
<html><head><meta charset="utf-8"><title>Prometheus — sign-in failed</title>
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; margin: 10vh auto; max-width: 32rem; text-align: center; color: #222; }}
h1 {{ color: #b42318; }} p {{ color: #555; }}
</style></head>
<body><h1>Sign-in failed</h1><p>{message}</p>
<p>Return to your terminal — Prometheus will walk you through a manual paste fallback.</p></body></html>
"""
...
def start_oauth_flow(
    *,
    ...
    auth_url = AUTH_ENDPOINT + "?" + urllib.parse.urlencode(params) + "#prometheus"
