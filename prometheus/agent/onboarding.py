...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Prometheus starts and finds ``~/.openclaw/``.

    Points users at ``prometheus claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``prometheus claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Prometheus, run "
        "`prometheus claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `prometheus claw cleanup` (renames it to ~/.openclaw.pre-migration — "
