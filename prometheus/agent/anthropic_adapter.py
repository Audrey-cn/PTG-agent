"""Anthropic Messages API adapter for Prometheus Agent."""
...
from prometheus.utils import base_url_host_matches, normalize_proxy_env_vars
...
# Prometheus effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See prometheus-agent#13848, #17057.
    """
...
def _prefer_refreshable_claude_code_token(
    ...
    Prometheus historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> str | None:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Prometheus)
    ...
    # 1. Prometheus-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Prometheus configs.
...
# ── Prometheus-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.prometheus/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_PROMETHEUS_OAUTH_FILE = get_prometheus_home() / ".anthropic_oauth.json"
...
def run_prometheus_oauth_login_pure() -> dict[str, Any] | None:
    """Run Prometheus-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Prometheus with your Claude Pro/Max subscription.")
    print()
...
def read_prometheus_oauth_credentials() -> dict[str, Any] | None:
    """Read Prometheus-managed OAuth credentials from ~/.prometheus/.anthropic_oauth.json."""
    if _PROMETHEUS_OAUTH_FILE.exists():
        try:
            data = json.loads(_PROMETHEUS_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError) as e:
            logger.debug("Failed to read Prometheus OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See prometheus-agent#13848.
            #
    ...
    # are preserved.  See prometheus-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    # silently hides reasoning text that Prometheus surfaces in its CLI. We
