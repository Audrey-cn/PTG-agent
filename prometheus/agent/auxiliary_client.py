...
from prometheus.utils import base_url_host_matches, base_url_hostname, normalize_proxy_env_vars
...
_OR_HEADERS = {
    "HTTP-Referer": "https://prometheus-agent.nousresearch.com",
    "X-OpenRouter-Title": "Prometheus Agent",
...
NOUS_EXTRA_BODY = {"tags": ["product=prometheus-agent"]}
...
def _codex_cloudflare_headers(access_token: str) -> dict[str, str]:
    ...
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Prometheus Agent)",
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from prometheus.agent.anthropic_adapter import build_anthropic_kwargs
        from prometheus.agent.transports import get_transport
        ...
        if temperature is not None:
            from prometheus.agent.anthropic_adapter import _forbids_sampling_params
...
class AnthropicAuxiliaryClient:
    ...
    def __init__(
        self, real_client: Any, model: str, api_key: str, base_url: str, is_oauth: bool = False
    ):
        self._real_client = real_client
...
def _maybe_wrap_anthropic(
    ...
    if _safe_isinstance(client_obj, CodexAuxiliaryClient):
        return client_obj
    try:
        from prometheus.agent.gemini_native_adapter import GeminiNativeClient
        ...
        pass
    try:
        from prometheus.agent.copilot_acp_client import CopilotACPClient
    ...
    try:
        from prometheus.agent.anthropic_adapter import build_anthropic_client
    except ImportError:
        logger.warning(
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> tuple[str, str] | None:
    ...
    try:
        from prometheus.cli.auth import resolve_nous_runtime_credentials
...
def _read_codex_access_token() -> str | None:
    """Read a valid, non-expired Codex OAuth access token from Prometheus auth store."""
    ...
    try:
        from prometheus.cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> tuple[OpenAI | None, str | None]:
    ...
    try:
        from prometheus.cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            try:
                from prometheus.cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if provider_id == "gemini":
                from prometheus.agent.gemini_native_adapter import (
                    GeminiNativeClient,
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from prometheus.cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> tuple[OpenAI | None, str | None]:
    try:
        from prometheus.agent.nous_rate_guard import nous_rate_limit_remaining
    ...
    model = _NOUS_MODEL
    try:
        from prometheus.cli.models import get_nous_recommended_aux_model
...
def _resolve_custom_runtime() -> tuple[str | None, str | None, str | None]:
    ...
    try:
        from prometheus.runtime_provider import resolve_runtime_provider
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `prometheus setup` or `prometheus model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_anthropic() -> tuple[Any | None, str | None]:
    ...
    from prometheus.agent.anthropic_adapter import _is_oauth_token

    is_oauth = _is_oauth_token(token)
    model = _API_KEY_PROVIDER_AUX_MODELS.get("anthropic", "claude-haiku-4-5-20251001")
    logger.debug(
        "Auxiliary client: Anthropic native (%s) at %s (oauth=%s)", model, base_url, is_oauth
    )
    try:
...
def _evict_cached_clients(provider: str) -> None:
    ...
    with _client_cache_lock:
        stale_keys = [
            key for key in _client_cache if _normalize_aux_provider(str(key[0])) == normalized
        ]
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "openai-codex":
            from prometheus.cli.auth import resolve_codex_runtime_credentials
    ...
    if not _stale_base_url_warned:
        ...
        if (
            ...
            logger.warning(
                ...
                "Run: prometheus model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.prometheus/.env",
                _env_base,
                _cfg_provider,
            )
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from prometheus.cli.copilot_auth import copilot_request_headers
...
def _normalize_resolved_model(model_name: str | None, provider: str) -> str | None:
    ...
    if not model_name:
        return model_name
    try:
        from prometheus.cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning(
                    ...
                    "but no Codex OAuth token found (run: prometheus model)"
                )
                return None, None
    ...
    try:
        from prometheus.runtime_provider import _get_named_custom_provider
        ...
        if custom_entry:
            ...
            logger.warning(
                "resolve_provider_client: named custom provider %r has no base_url", provider
            )
            return None, None
    except ImportError:
        pass
    ...
    try:
        ...
        logger.debug("prometheus.cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from prometheus.cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        final_model = _normalize_resolved_model(
            model or (main_runtime.get("model") if main_runtime else None) or _read_main_model(),
            provider,
        )
        ...
        logger.warning(
            "resolve_provider_client: external-process provider %s not directly supported", provider
        )
        return None, None

    elif pconfig.auth_type == "aws_sdk":
        try:
            from prometheus.agent.anthropic_adapter import build_anthropic_bedrock_client
            from prometheus.agent.bedrock_adapter import has_aws_credentials, resolve_bedrock_region
        except ImportError:
            logger.warning(
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    requested, resolved_model, resolved_base_url, resolved_api_key, resolved_api_mode = (
        ...
        if main_provider and main_provider not in ("auto", ""):
            ...
            if main_provider == "nous":
                ...
                rpc_client, rpc_model = resolve_provider_client(
                    main_provider, vision_model, api_mode=resolved_api_mode, is_vision=True
                )
    ...
    client, final_model = _get_cached_client(
        requested, resolved_model, async_mode, api_mode=resolved_api_mode, is_vision=True
    )
    if client is None:
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=prometheus-agent"])
    if merged_extra:
...
def call_llm(
    ...
    *,
        ...
        if client is None:
            ...
            if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `prometheus model`."
                )
    ...
    try:
        ...
        ) and _refresh_provider_credentials(resolved_provider):
            logger.info(
        ...
        if should_fallback and is_auto:
            ...
            if fb_client is not None:
                ...
                async_fb, async_fb_model = _to_async_client(
                    fb_client, fb_model, is_vision=(task == "vision")
                )
                return _validate_llm_response(
