...
from prometheus.prometheus_constants import OPENROUTER_BASE_URL
...
from prometheus_cli import __version__ as _PROMETHEUS_VERSION
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``prometheus_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        ...
        creds = resolve_nous_runtime_credentials(
            min_key_ttl_seconds=max(60, int(os.getenv("PROMETHEUS_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from prometheus_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from prometheus_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from prometheus_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if provider_id == "gemini":
                from prometheus.agent.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from prometheus_cli.models import copilot_default_headers
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    """
    try:
        from prometheus_cli.runtime_provider import resolve_runtime_provider
...
def _try_anthropic() -> Tuple[Optional[Any], Optional[str]]:
    try:
        from prometheus.agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
    except ImportError:
        return None, None
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "openai-codex":
            from prometheus_cli.auth import resolve_codex_runtime_credentials
        ...
        if normalized == "anthropic":
            from prometheus.agent.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `prometheus model` but the
    #    old OPENAI_BASE_URL lingers in ~/.prometheus/.env. ──
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from prometheus.agent.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from prometheus_cli.copilot_auth import copilot_request_headers
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from prometheus_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    try:
        from prometheus_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("prometheus_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from prometheus_cli.models import _should_use_copilot_responses_api
