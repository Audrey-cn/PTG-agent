...
from prometheus.agent.model_metadata import estimate_tokens_rough
...
_SENSITIVE_PROMETHEUS_DIRS = (Path("skills") / ".hub",)
...
async def _default_url_fetcher(url: str) -> str:
    from prometheus.tools.web_tools import web_extract_tool
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    prometheus_home = get_prometheus_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(prometheus_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(prometheus_home / rel for rel in _SENSITIVE_PROMETHEUS_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError(
            "path is a sensitive credential or internal Prometheus path and cannot be attached"
        )
