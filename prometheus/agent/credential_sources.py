"""Unified removal contract for every credential source Prometheus reads from.

Prometheus seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.prometheus/.env
    claude_code   — ~/.claude/.credentials.json
    prometheus_pkce   — ~/.prometheus/.anthropic_oauth.json
    ...
    manual        — user ran `prometheus auth add`
...
unify here is **removal**:
    ...
    ``prometheus auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), prometheus_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.prometheus/.env  → clear it
    ...
    """
    from prometheus.cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend(
            [
                f"Note: {env_var} is still set in your shell environment (not in ~/.prometheus/.env).",
                "  Unset it there (shell profile, systemd EnvironmentFile, "
                "launchd plist, etc.) or it will keep being visible to Prometheus.",
                f"  The pool entry is now suppressed — Prometheus will ignore "
                f"{env_var} until you run `prometheus auth add {provider}`.",
            ]
        )
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Prometheus stops reading it.
    """
    return RemovalResult(
        hints=[
            "Suppressed claude_code credential — it will not be re-seeded.",
            "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
            "Run `prometheus auth add anthropic` to re-enable if needed.",
        ]
    )


def _remove_prometheus_pkce(provider: str, removed) -> RemovalResult:
    """~/.prometheus/.anthropic_oauth.json is ours — delete it outright."""
    from prometheus.constants_core import get_prometheus_home

    result = RemovalResult()
    oauth_file = get_prometheus_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Prometheus Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `prometheus login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `prometheus auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Prometheus auth store is not enough — _seed_from_singletons() would
    ...
    via ``prometheus auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from prometheus.cli.auth import suppress_credential_source
    ...
    result.hints.extend(
        [
            ...
            "Run `prometheus auth add openai-codex` to re-enable if needed.",
        ]
    )
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(
        hints=[
            ...
            "Run `prometheus auth add qwen-oauth` to re-enable if needed.",
        ]
    )
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Prometheus stops picking the token up.
    """
    ...
    return RemovalResult(
        hints=[
            ...
            "Run `prometheus auth add copilot` to re-enable if needed.",
        ]
    )
...
def _register_all_sources() -> None:
    ...
    register(
        RemovalStep(
            provider="anthropic",
            source_id="prometheus_pkce",
            remove_fn=_remove_prometheus_pkce,
            description="~/.prometheus/.anthropic_oauth.json",
        )
    )
    register(
        RemovalStep(
