...
编史官(Chronicler)的底层引擎，负责：
1. 格式无关的种子读取 (FormatAgnosticReader)
2. 身份分类判定 (SemanticAuditEngine)
3. 谱系定位 (LineageAnchor)

设计原则：
- 宽松解析：支持 Markdown+YAML / JSON / 二进制TTGC / 纯文本
- 语义匹配：使用加权评分而非硬编码集合匹配
- 非破坏性：只读取不修改
"""
...
from prometheus.codec.layer1 import decode_seed, MAGIC
from prometheus.codec.layer2 import decode_seed_l2, GENESIS_BLOCK
...
class FormatAgnosticReader:
    ...
    def _extract_from_plain_text(self, text: str, file_path: str) -> SeedReading:
        fingerprints = {
            ...
            "has_founder_chronicle": bool(re.search(r'founder.chronicle|创始铭刻', text, re.IGNORECASE)),
...
class SemanticAuditEngine:
    ...
    def _check_lineage_structure(self, reading: SeedReading) -> bool:
        if reading.has_structured_data():
            ...
            return has_founder_covenant or has_lineage_laws or has_bloodline or has_evolution or has_current
    ...
    def _determine_identity(self, score: float, evidence: Dict) -> Tuple[SeedIdentity, float, str]:
        ...
        if score >= 90:
            ...
            return (
                ...
                score / 100,
                f"外来种子(有族谱) — 无创始标签，但存在谱系结构"
            )
        else:
            return (
                SeedIdentity.FOREIGN_RAW,
                score / 100,
                f"外来种子(无结构) — 无标签，无谱系，可能来自其他Agent"
            )
    ...
    def _locate_from_fingerprints(self, reading: SeedReading) -> LineageAnchor:
        has_lineage = reading.fingerprints.get("has_lineage", False) if reading.fingerprints else False
