...
process replacement so that ``prometheus sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``prometheus`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from prometheus.prometheus_cli._parser import (
...
def resolve_prometheus_bin() -> Optional[str]:
