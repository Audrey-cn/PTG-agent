...
Skins are defined as YAML files in ~/.prometheus/skins/ or as built-in presets.
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Prometheus Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " 🔥 Prometheus "       # Response box header label
...
.. code-block:: python
    ...
    from prometheus.prometheus_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Prometheus Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.prometheus/skins/

BUILT-IN SKINS
==============

- ``default`` — Classic Prometheus gold/kawaii (the current look)
...
Drop a YAML file in ``~/.prometheus/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces PROMETHEUS_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces PROMETHEUS_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Prometheus — gold and kawaii",
        "colors": {
...
def _skins_dir() -> Path:
    ...
    return get_prometheus_home() / "skins"
