"""
Prometheus Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.prometheus/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Prometheus PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing prometheus-agent or prometheus PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Prometheus Agent" comment and following line
                if '# Prometheus Agent' in line or '# prometheus-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('prometheus' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing prometheus
                if 'prometheus' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the prometheus wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "prometheus",
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains prometheus_cli reference)
                content = wrapper.read_text()
                if 'prometheus_cli' in content or 'prometheus-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``prometheus gateway run`` processes
    ...
    try:
        from prometheus.prometheus_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from prometheus.prometheus_cli.gateway import get_launchd_plist_path
...
def _is_default_prometheus_home(prometheus_home: Path) -> bool:
    """Return True when ``prometheus_home`` points at the default (non-profile) root."""
    try:
        from prometheus_constants import get_default_prometheus_root
        return prometheus_home.resolve() == get_default_prometheus_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its PROMETHEUS_HOME directory.

    We shell out to ``prometheus -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current PROMETHEUS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m prometheus_cli.main` so we don't depend on a `prometheus`
    #    wrapper that may be half-removed mid-uninstall.
    prometheus_invocation = [_sys.executable, "-m", "prometheus_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                prometheus_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's PROMETHEUS_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.prometheus/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.prometheus/ for future reinstall
    """
    project_root = get_project_root()
    prometheus_home = get_prometheus_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie PROMETHEUS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_prometheus_home(prometheus_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            🔥 Prometheus Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {prometheus_home / 'config.yaml'}")
    print(f"  Secrets: {prometheus_home / '.env'}")
    print(f"  Data:    {prometheus_home / 'cron/'}, {prometheus_home / 'sessions/'}, {prometheus_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their PROMETHEUS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Prometheus data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Prometheus code but keep your configuration and data.")
    ...
    log_info("Removing prometheus command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.prometheus/, just remove the prometheus-agent subdir
            if prometheus_home in project_root.parents or project_root.parent == prometheus_home:
    ...
    # 5. Optionally remove ~/.prometheus/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile PROMETHEUS_HOME dirs live under
        ...
        try:
            if prometheus_home.exists():
                ...
                log_success(f"Removed {prometheus_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {prometheus_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {prometheus_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {prometheus_home}/")
        print()
        print("To reinstall later with your existing settings:")
        print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/prometheus-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using Prometheus Agent! ⚕")
    print()
