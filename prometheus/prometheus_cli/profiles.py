"""
Profile management for multiple isolated Prometheus instances.

Each profile is a fully independent PROMETHEUS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.prometheus/profiles/<name>/`` by default.

The "default" profile is ``~/.prometheus`` itself — backward compatible,
zero migration needed.

Usage::

    prometheus profile create coder          # fresh profile + bundled skills
    prometheus profile create coder --clone  # also copy config, .env, SOUL.md, skills
    prometheus profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    prometheus -p coder chat                 # or via flag
    prometheus profile use coder             # set as sticky default
    prometheus profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See prometheus_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.prometheus`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.prometheus) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "prometheus-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "prometheus", "default", "test", "tmp", "root", "sudo",
})

# Prometheus subcommands that cannot be used as profile names/aliases
_PROMETHEUS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the prometheus root, NOT to the current PROMETHEUS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where PROMETHEUS_HOME points outside
    ``~/.prometheus``, profiles live under ``PROMETHEUS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_prometheus_home() / "profiles"


def _get_default_prometheus_home() -> Path:
    """Return the default (pre-profile) PROMETHEUS_HOME path.

    In standard deployments this is ``~/.prometheus``.
    In Docker/custom deployments where PROMETHEUS_HOME is outside ``~/.prometheus``
    (e.g. ``/opt/data``), returns PROMETHEUS_HOME directly.
    """
    from prometheus_constants import get_default_prometheus_root
    return get_default_prometheus_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_prometheus_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.prometheus
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its PROMETHEUS_HOME directory."""
    if name == "default":
        return _get_default_prometheus_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, prometheus subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _PROMETHEUS_SUBCOMMANDS:
        return f"'{name}' conflicts with a prometheus subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_prometheus_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.prometheus)."
        )
    ...
    if clone_from is not None or clone_all or clone_config:
        if clone_from is None:
            ...
            source_dir = get_prometheus_home()
        else:
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.prometheus/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    if not soul_path.exists():
        try:
            from prometheus.prometheus_cli.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches PROMETHEUS_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "PROMETHEUS_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.prometheus).\n"
            "To remove everything, use: prometheus uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set PROMETHEUS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("PROMETHEUS_HOME")
    try:
        os.environ["PROMETHEUS_HOME"] = str(profile_dir)
        from prometheus.prometheus_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["PROMETHEUS_HOME"] = old_home
        elif "PROMETHEUS_HOME" in os.environ:
            del os.environ["PROMETHEUS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.prometheus/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: prometheus profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from PROMETHEUS_HOME.

    Returns ``"default"`` if PROMETHEUS_HOME is not set or points to ``~/.prometheus``.
    Returns the profile name if PROMETHEUS_HOME points into ``~/.prometheus/profiles/<name>``.
    Returns ``"custom"`` if PROMETHEUS_HOME is set to an unrecognized path.
    """
    from prometheus_constants import get_prometheus_home
    prometheus_home = get_prometheus_home()
    resolved = prometheus_home.resolve()

    default_resolved = _get_default_prometheus_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.prometheus itself — its parent is ~/ and its
        # directory name is .prometheus, not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: prometheus profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.prometheus itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.prometheus). "
            "Specify a different name: prometheus profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="prometheus_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"prometheus.{old_name}"
    new_host = f"prometheus.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_prometheus_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for prometheus profile names."""
    return '''# Prometheus Agent profile completion
# Add to ~/.bashrc: eval "$(prometheus completion bash)"

_prometheus_profiles() {
    local profiles_dir="$HOME/.prometheus/profiles"
...
_prometheus_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_prometheus_profiles)" -- "$cur"))
        return
    fi
...
complete -F _prometheus_completion prometheus
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for prometheus profile names."""
    return '''#compdef prometheus
# Prometheus Agent profile completion
# Add to ~/.zshrc: eval "$(prometheus completion zsh)"

_prometheus() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.prometheus/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.prometheus/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a PROMETHEUS_HOME path string.

    Called early in the CLI entry point, before any prometheus modules
    are imported, to set the PROMETHEUS_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: prometheus profile create {profile_name}"
        )
