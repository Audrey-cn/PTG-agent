...
def _get_custom_provider_names() -> list:
    ...
    try:
        from prometheus.prometheus_cli.config import get_compatible_custom_providers, load_config
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_prometheus_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:prometheus_pkce",
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `prometheus auth
    ...
    if provider == "minimax-oauth":
        from prometheus.prometheus_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    raise SystemExit(f"`prometheus auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Prometheus reads from
    ...
    from prometheus.prometheus_cli.auth import suppress_credential_source
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `prometheus auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `prometheus auth` is called bare."""
