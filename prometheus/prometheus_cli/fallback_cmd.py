...
Subcommands:
  prometheus fallback [list]   Show the current fallback chain (default when no subcommand)
  prometheus fallback add      Pick provider + model via the same picker as `prometheus model`,
                           then append the selection to the chain
  prometheus fallback remove   Pick an entry to delete from the chain
  prometheus fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.prometheus/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from prometheus.prometheus_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `prometheus model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    print("  Docs: https://prometheus-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `prometheus model`, then append the selection to the chain."""
    from prometheus.prometheus_cli.main import _require_tty, select_provider_and_model
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``prometheus fallback [subcommand]``."""
