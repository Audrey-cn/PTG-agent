...
- ``prometheus_cli.providers``        -- canonical provider identity + overlays
- ``prometheus_cli.model_normalize``  -- per-provider name formatting
...
from prometheus.prometheus_cli.model_normalize import (
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from prometheus.prometheus_cli.model_switch import
...
def switch_model(
    ...
    from prometheus.prometheus_cli.runtime_provider import resolve_runtime_provider
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'prometheus model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'prometheus doctor' — config issues detected:"
        ...
        if not new_model:
            if pdef.base_url:
                from prometheus.prometheus_cli.runtime_provider import _auto_detect_local_model
    ...
    # Mirrors the same logic in prometheus_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from prometheus.prometheus_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from prometheus.prometheus_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # --- 1. Check Prometheus-mapped providers ---
    for prometheus_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(prometheus_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (PROMETHEUS_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from prometheus.prometheus_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and prometheus_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Prometheus release.
        model_ids = curated.get(prometheus_id, [])
        if prometheus_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(prometheus_id, model_ids)
        total = len(model_ids)
    ...
    from prometheus.prometheus_cli.providers import PROMETHEUS_OVERLAYS
    from prometheus.prometheus_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Prometheus provider ID.
    # PROMETHEUS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Prometheus IDs ("copilot").
    _mdev_to_prometheus = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in PROMETHEUS_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Prometheus slug — e.g. "github-copilot" → "copilot"
        prometheus_slug = _mdev_to_prometheus.get(pid, pid)
        if prometheus_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, prometheus_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or prometheus_slug in providers_store
                    or pid in pool_store or prometheus_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(prometheus_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", prometheus_slug, exc)
        ...
        # configured.
        if not has_creds and prometheus_slug == "anthropic":
            try:
                ...
                if (prometheus_creds and prometheus_creds.get("accessToken")) or \
        ...
        if prometheus_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(prometheus_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(prometheus_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(prometheus_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Prometheus slug, fall back to overlay key
            model_ids = curated.get(prometheus_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if prometheus_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(prometheus_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": prometheus_slug,
            "name": get_label(prometheus_slug),
            "is_current": prometheus_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "prometheus",
        ...
        seen_slugs.add(prometheus_slug.lower())
        _record_builtin_endpoint(prometheus_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or PROMETHEUS_OVERLAYS (keeps /model in sync
    # with `prometheus model`).
    try:
        from prometheus.prometheus_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Prometheus's canonical write key (matches
            ...
            # Prometheus writes ``models:`` as a dict keyed by model id
            # (see prometheus_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Prometheus's own writer uses; a hyphen variant is accepted
            ...
            # active model. Prometheus's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, prometheus_cli/config.py) already consume that dict.
