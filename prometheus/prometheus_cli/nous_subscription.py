...
from prometheus.prometheus_cli.config import get_env_value, load_config
...
def prompt_enable_tool_gateway(config: Dict[str, object]) -> set[str]:
    ...
    try:
        from prometheus.prometheus_cli.setup import prompt_choice
    except Exception:
        return set()
    ...
    if changed:
        from prometheus.prometheus_cli.config import save_config
        save_config(config)
