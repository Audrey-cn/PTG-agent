"""
Prometheus Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m prometheus_cli.main web          # Start on http://127.0.0.1:9119
    python -m prometheus_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["PROMETHEUS_WEB_DIST"]) if "PROMETHEUS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Prometheus Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Prometheus-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``prometheus dashboard --tui``
# or PROMETHEUS_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.prometheus/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_prometheus_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "prometheus-update": "prometheus-update.log",
}
...
def _spawn_prometheus_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``prometheus <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``prometheus_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "prometheus_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "PROMETHEUS_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``prometheus gateway restart`` in the background."""
    try:
        proc = _spawn_prometheus_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/prometheus/update")
async def update_prometheus():
    """Kick off ``prometheus update`` in the background."""
    try:
        proc = _spawn_prometheus_action(["update"], "prometheus-update")
    except Exception as exc:
        _log.exception("Failed to spawn prometheus update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Prometheus supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in prometheus_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.prometheus/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``prometheus auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Prometheus resolves Anthropic creds in this order at runtime:
    1. ``~/.prometheus/.anthropic_oauth.json`` — Prometheus-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            ...
            _PROMETHEUS_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_prometheus_oauth_credentials = None  # type: ignore
        _PROMETHEUS_OAUTH_FILE = None  # type: ignore

    prometheus_creds = None
    if read_prometheus_oauth_credentials:
        try:
            prometheus_creds = read_prometheus_oauth_credentials()
        except Exception:
            prometheus_creds = None
    if prometheus_creds and prometheus_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "prometheus_pkce",
            "source_label": f"Prometheus PKCE ({_PROMETHEUS_OAUTH_FILE})",
            "token_preview": _truncate_token(prometheus_creds.get("accessToken")),
            "expires_at": prometheus_creds.get("expiresAt"),
            "has_refresh_token": bool(prometheus_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Prometheus even
    when they also have a separate Prometheus-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "prometheus auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "prometheus auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "prometheus auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "prometheus auth add qwen-oauth",
    ...
    },
    {
        ...
        "name": "MiniMax (OAuth)",
        "flow": "pkce",
        "cli_command": "prometheus auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from prometheus.prometheus_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("prometheus_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Prometheus-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _PROMETHEUS_OAUTH_FILE
            if _PROMETHEUS_OAUTH_FILE.exists():
                _PROMETHEUS_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from prometheus.prometheus_cli.auth import clear_provider_auth
...
#          → persists to ~/.prometheus/.anthropic_oauth.json AND credential pool
...
# Guarded so prometheus web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Prometheus file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``prometheus auth add anthropic``.
    """
    from agent.anthropic_adapter import _PROMETHEUS_OAUTH_FILE
    payload = {
    ...
    _PROMETHEUS_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _PROMETHEUS_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "prometheus-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from prometheus.prometheus_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("PROMETHEUS_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from prometheus.prometheus_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from prometheus.prometheus_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = (
            os.getenv("PROMETHEUS_CODEX_BASE_URL", "").strip().rstrip("/")
...
async def get_logs(
    ...
    from prometheus.prometheus_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_prometheus_home() / "logs" / log_name
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_prometheus_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from prometheus.prometheus_cli import profiles as profiles_mod
    try:
...
def _profile_setup_command(name: str) -> str:
    ...
    return "prometheus setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from prometheus.prometheus_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from prometheus.prometheus_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from prometheus.prometheus_cli.tools_config import (
...
#
# The endpoint spawns the same ``prometheus --tui`` binary the CLI uses, behind
...
from prometheus.prometheus_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``prometheus --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``PROMETHEUS_TUI_RESUME`` env var —
    matching what ``prometheus_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``PROMETHEUS_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from prometheus.prometheus_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if resume:
        env["PROMETHEUS_TUI_RESUME"] = resume

    if sidecar_url:
        env["PROMETHEUS_TUI_SIDECAR_URL"] = sidecar_url
...
# PROMETHEUS_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__PROMETHEUS_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__PROMETHEUS_DASHBOARD_EMBEDDED_CHAT__={chat_js};</script>"
        )
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "Prometheus Teal",  "description": "Classic dark teal — the canonical Prometheus look"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.prometheus/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.prometheus/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_prometheus_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.prometheus/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as prometheus_cli.plugins):
    1. User plugins:    ~/.prometheus/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.prometheus/plugins/  (only if PROMETHEUS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from prometheus.prometheus_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_prometheus_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("PROMETHEUS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".prometheus" / "plugins", "project"))
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"prometheus_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Prometheus Web UI → http://{host}:{port}")
