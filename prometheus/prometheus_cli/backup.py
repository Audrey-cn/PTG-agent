...
`prometheus backup` creates a zip archive of the entire ~/.prometheus/ directory
(excluding the prometheus-agent repo and transient files).

`prometheus import` restores from a backup zip, overlaying onto the current
PROMETHEUS_HOME root.
"""
...
from prometheus_constants import get_default_prometheus_root, get_prometheus_home, display_prometheus_home
...
_EXCLUDED_DIRS = {
    "prometheus-agent",     # the codebase repo — re-clone instead
...
def run_backup(args) -> None:
    ...
    for dirpath, dirnames, filenames in os.walk(prometheus_root, followlinks=False):
        ...
        for fname in filenames:
            ...
            # Skip the output zip itself if it happens to be inside prometheus root
            try:
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a prometheus home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.prometheus/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a prometheus dir name
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # prometheus_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: prometheus profile list  (after installing prometheus)")
...
# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and prometheus backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to PROMETHEUS_HOME).
...
# — `prometheus update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(prometheus_home: Optional[Path] = None) -> Path:
...
def create_quick_snapshot(
    ...
    prometheus_home: Optional[Path] = None,
) -> Optional[str]:
...
def _write_full_zip_backup(out_path: Path, prometheus_root: Path) -> Optional[Path]:
...
def _pre_update_backup_dir(prometheus_home: Optional[Path] = None) -> Path:
    home = prometheus_home or get_prometheus_home()
...
def create_pre_update_backup(
    ...
    but writes to ``<PROMETHEUS_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``prometheus update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `prometheus claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of PROMETHEUS_HOME under ``backups/`` before a
    ``prometheus claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``prometheus import <archive>``.  Writes to
    ``<PROMETHEUS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `prometheus import` and the
