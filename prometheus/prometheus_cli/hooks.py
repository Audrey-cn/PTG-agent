"""prometheus hooks — inspect and manage shell-script hooks.

Usage::

    prometheus hooks list
    prometheus hooks test <event> [--for-tool X] [--payload-file F]
    prometheus hooks revoke <command>
    prometheus hooks doctor

Consent records live under ``~/.prometheus/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.prometheus/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``prometheus hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: prometheus hooks {list|test|revoke|doctor}")
        print("Run 'prometheus hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.prometheus/config.yaml.")
        print("See `prometheus hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `prometheus hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `prometheus hooks test` and `prometheus hooks doctor`
...
def _cmd_test(args) -> None:
    from prometheus.prometheus_cli.config import load_config
    from prometheus.prometheus_cli.plugins import VALID_HOOKS
    from agent import shell_hooks
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Prometheus wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `prometheus hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `prometheus hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `prometheus hooks doctor`.")
