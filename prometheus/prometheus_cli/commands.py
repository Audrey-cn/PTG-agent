...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Prometheus config/state", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Prometheus is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.prometheus/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Prometheus Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`prometheus_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/prometheus`` subcommand mapping, and
    ...
    """
    try:
        from prometheus.prometheus_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``prometheus skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/prometheus <verb>`` subcommand.
    ...
    avoidance. ``/prometheus`` is always reserved as the first entry so the
    legacy ``/prometheus <subcommand>`` form keeps working for anything that
    ...
    # Reserve /prometheus as the catch-all top-level command.
...
def slack_app_manifest(request_url: str = "https://prometheus-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    ...
    Maps both canonical names and aliases so /prometheus bg do stuff works
    the same as /prometheus background do stuff.

    Plugin-registered slash commands are included so ``/prometheus <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from prometheus.prometheus_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _model_completions(self, sub_text: str, sub_lower: str):
        ...
        try:
            from prometheus.prometheus_cli.model_switch import (
