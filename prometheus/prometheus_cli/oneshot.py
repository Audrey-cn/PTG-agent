...
configured for "cli" in `prometheus tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (PROMETHEUS_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `prometheus chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    - PROMETHEUS_INFERENCE_MODEL
    - PROMETHEUS_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    if unresolved:
        try:
            ...
            from prometheus.prometheus_cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        sys.stderr.write(f"prometheus -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to PROMETHEUS_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            PROMETHEUS_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
...
def _run_agent(
    ...
    # Imports are local so they don't run when prometheus is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from prometheus.prometheus_cli.config import load_config
    from prometheus.prometheus_cli.models import detect_provider_for_model
    from prometheus.prometheus_cli.runtime_provider import resolve_runtime_provider
    from prometheus.prometheus_cli.tools_config import _get_platform_tools
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from prometheus.prometheus_cli import model_switch as _ms
    ...
    agent = AIAgent(
        ...
        #                PROMETHEUS_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via PROMETHEUS_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via PROMETHEUS_YOLO_MODE=1
