...
under ``~/.prometheus/logs/``.

Usage examples::

    prometheus logs                    # last 50 lines of agent.log
    prometheus logs -f                 # follow agent.log in real time
    prometheus logs errors             # last 50 lines of errors.log
    prometheus logs gateway -n 100    # last 100 lines of gateway.log
    prometheus logs --level WARNING    # only WARNING+ lines
    prometheus logs --session abc123   # filter by session ID substring
    prometheus logs --component tools  # only tool-related lines
    prometheus logs --since 1h         # lines from the last hour
    prometheus logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Prometheus runs — try 'prometheus chat' first)")
        sys.exit(1)
    ...
    if follow:
        ...
        print(f"--- {display_prometheus_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
