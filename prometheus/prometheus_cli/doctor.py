...
Diagnoses issues with Prometheus Agent setup.
"""
...
from prometheus.prometheus_cli.config import get_project_root, get_prometheus_home, get_env_path
from prometheus_constants import display_prometheus_home

PROJECT_ROOT = get_project_root()
PROMETHEUS_HOME = get_prometheus_home()
_DHH = display_prometheus_home()  # user-facing display path (e.g. ~/.prometheus or ~/.prometheus/profiles/coder)

# Load environment variables from ~/.prometheus/.env so API key checks work
...
from prometheus.prometheus_cli.models import _PROMETHEUS_USER_AGENT
from prometheus.prometheus_cli.vercel_auth import describe_vercel_auth
from prometheus_constants import OPENROUTER_MODELS_URL
...
from prometheus_constants import is_termux as _is_termux
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `prometheus`.
    ...
    # Check ~/.prometheus/.env (primary location for user config)
    ...
    # Check ~/.prometheus/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from prometheus.prometheus_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from prometheus.prometheus_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom", "openrouter"):
                try:
                    from prometheus.prometheus_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            ...
                            issues.append(
                                ...
                                f"Run 'prometheus setup' or set the provider's API key in {_DHH}/.env, "
    ...
    if config_path.exists():
        try:
            from prometheus.prometheus_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    ...
                    issues.append("Run 'prometheus doctor --fix' or 'prometheus setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'prometheus doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses Prometheus' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `prometheus auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Prometheus a custom personality)")
        if should_fix:
    ...
    # =========================================================================
    # Check: Command installation (prometheus bin symlink)
    # =========================================================================
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    if should_fix:
                        ...
                        issues.append(f"Broken symlink at {_cmd_link_display}/prometheus — run 'prometheus doctor --fix'")
                ...
                if should_fix:
                    ...
                    issues.append(f"Missing {_cmd_link_display}/prometheus symlink — run 'prometheus doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            issues.append("Install the Vercel optional dependency: pip install 'prometheus-agent[vercel]'")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'prometheus setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run prometheus memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: prometheus memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: prometheus memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from prometheus.prometheus_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'prometheus doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
