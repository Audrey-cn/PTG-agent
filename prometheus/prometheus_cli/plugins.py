...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with prometheus-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.prometheus/plugins/<name>/``
3. **Project plugins** – ``./.prometheus/plugins/<name>/`` (opt-in via
   ``PROMETHEUS_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``prometheus_agent.plugins``
   entry-point group.
...
class PluginManifest:
    ...
    #              in ~/.prometheus/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``prometheus plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``prometheus <subcommand>``
        ...
        try:
            from prometheus.prometheus_cli.commands import resolve_command
    ...
    def register_skill(
        ...
        ``~/.prometheus/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to prometheus_cli/. Two layouts are
        ...
        # 2. User plugins (~/.prometheus/plugins/)
        user_dir = get_prometheus_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.prometheus/plugins/)
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with prometheus and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Prometheus ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `prometheus plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        ...
        ``prometheus_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="prometheus-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``prometheus tools`` TUI so plugin-provided toolsets appear
