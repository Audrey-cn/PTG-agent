...
Powers both:
  - `prometheus skills <subcommand>` (CLI argparse entry point)
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print(table)
    c.print("[dim]Use: prometheus skills inspect <identifier> to preview, "
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    c.print("[dim]Tip: 'prometheus skills search <query>' searches deeper across all registries[/]\n")
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="prometheus skills install <id> to install"))
...
def do_list(source_filter: str = "all",
    ...
    config — ``prometheus -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``PROMETHEUS_HOME`` at process
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: prometheus skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: prometheus skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
...
def skills_command(args) -> None:
    """Router for `prometheus skills <subcommand>` — called from prometheus.prometheus_cli/main.py."""
    ...
    if action == "browse":
        ...
        _console.print("Usage: prometheus skills [browse|search|install|inspect|list|check|update|audit|uninstall|reset|publish|snapshot|tap]\n")
