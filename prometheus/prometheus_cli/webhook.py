"""prometheus webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    prometheus webhook subscribe <name> [options]
    prometheus webhook list
    prometheus webhook remove <name>
    prometheus webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.prometheus/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: prometheus gateway run
"""
...
def webhook_command(args):
    """Entry point for 'prometheus webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: prometheus webhook {subscribe|list|remove|test}")
        print("Run 'prometheus webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (prometheus gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: prometheus webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from prometheus webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (prometheus gateway run)")
