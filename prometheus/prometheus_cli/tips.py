...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or prometheus -c.",
    ...
    "Ctrl+Z suspends Prometheus to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "prometheus -c resumes your most recent CLI session. prometheus -c \"project name\" resumes by title.",
    "prometheus -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "prometheus -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "prometheus chat -t web,terminal enables only specific toolsets for a focused session.",
    "prometheus chat -s github-pr-workflow preloads a skill at launch.",
    "prometheus chat -q \"query\" runs a single non-interactive query and exits.",
    "prometheus chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "prometheus chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "prometheus --yolo bypasses all dangerous command approval prompts for the entire session.",
    "prometheus chat --source telegram tags the session for filtering in prometheus sessions list.",
    "prometheus -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "prometheus doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "prometheus dump outputs a compact setup summary — great for bug reports.",
    "prometheus config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "prometheus config edit opens config.yaml in your default editor.",
    "prometheus config check scans for missing or stale configuration options.",
    "prometheus sessions browse opens an interactive session picker with search.",
    "prometheus sessions stats shows session counts by platform and database size.",
    "prometheus sessions prune --older-than 30 cleans up old sessions.",
    "prometheus skills search react --source skills-sh searches the skills.sh public directory.",
    "prometheus skills check scans installed hub skills for upstream updates.",
    "prometheus skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "prometheus skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "prometheus mcp add github --command npx adds MCP servers from the command line.",
    "prometheus mcp serve runs Prometheus itself as an MCP server for other agents.",
    "prometheus auth add lets you add multiple API keys for credential pool rotation.",
    "prometheus completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "prometheus logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "prometheus backup creates a zip backup of your entire Prometheus home directory.",
    "prometheus profile create coder creates an isolated profile that becomes its own command.",
    "prometheus profile create work --clone copies your current config and keys to a new profile.",
    "prometheus update syncs new bundled skills to ALL profiles automatically.",
    "prometheus gateway install sets up Prometheus as a system service (systemd/launchd).",
    "prometheus memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "prometheus webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: prometheus tools disables unused tools, prometheus skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "prometheus models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as prometheus -w).",
    ...
    "PROMETHEUS_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Prometheus tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'prometheus profile create coder' creates the 'coder' command.",
    "prometheus profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Prometheus prints a resume command with session ID and stats.",
    "prometheus sessions export backup.jsonl exports all sessions for backup or analysis.",
    "prometheus -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "prometheus skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .prometheus/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: prometheus cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Prometheus runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "prometheus gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Prometheus via a local macOS server.",
    ...
    "Context files (.prometheus.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.prometheus/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Prometheus loads project context from .prometheus.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — prometheus tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.prometheus/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "prometheus chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "prometheus plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "prometheus acp runs Prometheus as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "PROMETHEUS_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "prometheus login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "prometheus status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.prometheus/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Prometheus your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.prometheus/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Prometheus auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.prometheus/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at PROMETHEUS_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "PROMETHEUS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in PROMETHEUS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.prometheus/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'prometheus profile create ls' would be rejected.",
    "prometheus profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "prometheus claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "PROMETHEUS_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via prometheus tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.prometheus/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.prometheus/skills/.hub/quarantine/ holds skills pending security review.",
]
