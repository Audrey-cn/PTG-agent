...
`prometheus tools` and `prometheus setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.prometheus/config.yaml under
...
from prometheus.prometheus_cli.nous_subscription import (
...
from prometheus.prometheus_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _PROMETHEUS_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl", "spotify", "discord", "discord_admin"}

# Platform-scoped toolsets: only appear in the `prometheus tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``prometheus tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from prometheus.prometheus_cli.plugins import discover_plugins, get_plugin_toolsets
...
from prometheus.prometheus_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "PROMETHEUS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "PROMETHEUS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "PROMETHEUS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "PROMETHEUS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "PROMETHEUS_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_prometheus_home()}/prometheus-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in prometheus-agent directory)")
            return
        ...
        # Run the full `prometheus auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.prometheus/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from prometheus.prometheus_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            ...
            # toolset enable; they can retry with `prometheus auth spotify`.
        ...
        try:
            from prometheus.prometheus_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _get_platform_tools(
    ...
    *,
    ...
    # "prometheus-cli" (which include all _PROMETHEUS_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "prometheus-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # otherwise saving via `prometheus tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `prometheus tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `prometheus tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by prometheus tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (prometheus-cli, prometheus-telegram, etc.)
    ...
    # Opening `prometheus tools` is the user's opt-in to reconfigure tools, so treat
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://prometheus-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from prometheus_constants import display_prometheus_home
    print(color(f"  Tool configuration saved to {display_prometheus_home()}/config.yaml", Colors.DIM))
