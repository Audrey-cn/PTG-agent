"""
Status command for prometheus CLI.

Shows the status of all Prometheus Agent components.
"""
...
from prometheus.prometheus_cli.auth import AuthError, resolve_provider
from prometheus.prometheus_cli.colors import Colors, color
from prometheus.prometheus_cli.config import get_env_path, get_env_value, get_prometheus_home, load_config
from prometheus.prometheus_cli.models import provider_label
from prometheus.prometheus_cli.nous_subscription import get_nous_subscription_features
from prometheus.prometheus_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``prometheus config``'s
...
def show_status(args):
    """Show status of all Prometheus Agent components."""
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: prometheus auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: prometheus model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: prometheus auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: prometheus model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from prometheus.prometheus_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'prometheus-agent[vercel]')"
    ...
    try:
        from prometheus.prometheus_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   prometheus gateway")
    ...
    jobs_file = get_prometheus_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_prometheus_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'prometheus doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'prometheus setup' to configure", Colors.DIM))
    print()
