#!/usr/bin/env python3
"""
Prometheus CLI - Main entry point.

Usage:
    prometheus                     # Interactive chat (default)
    prometheus chat                # Interactive chat
    prometheus gateway             # Run gateway in foreground
    prometheus gateway start       # Start gateway as service
    prometheus gateway stop        # Stop gateway service
    prometheus gateway status      # Show gateway status
    prometheus gateway install     # Install gateway service
    prometheus gateway uninstall   # Uninstall gateway service
    prometheus setup               # Interactive setup wizard
    prometheus logout              # Clear stored authentication
    prometheus status              # Show status of all components
    prometheus cron                # Manage cron jobs
    prometheus cron list           # List cron jobs
    prometheus cron status         # Check if cron scheduler is running
    prometheus doctor              # Check configuration and dependencies
    prometheus honcho setup                    # Configure Honcho AI memory integration
    prometheus honcho status                   # Show Honcho config and connection status
    prometheus honcho sessions                 # List directory → session name mappings
    prometheus honcho map <name>               # Map current directory to a session name
    prometheus honcho peer                     # Show peer names and dialectic settings
    prometheus honcho peer --user NAME         # Set user peer name
    prometheus honcho peer --ai NAME           # Set AI peer name
    prometheus honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    prometheus honcho mode                     # Show current memory mode
    prometheus honcho mode [hybrid|honcho|local]  # Set memory mode
    prometheus honcho tokens                   # Show token budget settings
    prometheus honcho tokens --context N       # Set session.context() token cap
    prometheus honcho tokens --dialectic N     # Set dialectic result char cap
    prometheus honcho identity                 # Show AI peer identity representation
    prometheus honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    prometheus honcho migrate                  # Step-by-step migration guide: OpenClaw native → Prometheus + Honcho
    prometheus version             Show version
    prometheus update              Update to latest version
    prometheus uninstall           Uninstall Prometheus Agent
    prometheus acp                 Run as an ACP server for editor integration
    prometheus sessions browse     Interactive session picker with search

    prometheus claw migrate --dry-run  # Preview migration without changes
"""
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any prometheus module import.
#
# Many modules cache PROMETHEUS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("PROMETHEUS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.prometheus/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set PROMETHEUS_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 1.5 If PROMETHEUS_HOME is already set and no explicit flag was given, trust it.
    # This lets child processes (relaunch, subprocess) inherit the parent's
    # profile choice without having to pass --profile again.
    if profile_name is None and os.environ.get("PROMETHEUS_HOME"):
        return

    # 2. If no flag, check active_profile in the prometheus root
    if profile_name is None:
        try:
            from prometheus_constants import get_default_prometheus_root

            active_path = get_default_prometheus_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set PROMETHEUS_HOME
    if profile_name is not None:
        try:
            from prometheus.prometheus_cli.profiles import resolve_profile_env

            prometheus_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent prometheus from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["PROMETHEUS_HOME"] = prometheus_home
...
from prometheus.prometheus_cli.env_loader import load_prometheus_dotenv

load_prometheus_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → PROMETHEUS_REDACT_SECRETS env
# var BEFORE prometheus_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "PROMETHEUS_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early
        _cfg_path = get_prometheus_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["PROMETHEUS_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `prometheus` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from prometheus_logging import setup_logging as _setup_logging
...
try:
    from prometheus.prometheus_cli.config import load_config as _load_config_early
    from prometheus_constants import apply_ipv4_preference as _apply_ipv4
...
from prometheus.prometheus_cli import __version__, __release_date__
from prometheus_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from prometheus.prometheus_cli.config import get_env_path, get_prometheus_home, load_config
    from prometheus.prometheus_cli.auth import get_auth_status

    # Determine whether Prometheus itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from prometheus.prometheus_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_prometheus_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from prometheus.prometheus_cli.auth import PROVIDER_REGISTRY
    ...
    # Only count these if Prometheus has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Prometheus to use their tokens.
    if _has_prometheus_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from prometheus_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, prometheus_bin
        cli_args: the original CLI arguments (everything after 'prometheus')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    prometheus_bin = container_info["prometheus_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo prometheus {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo prometheus {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    print(f"  prometheus --tui --resume {target}")
    if title:
        print(f'  prometheus --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @prometheus/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@prometheus" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: PROMETHEUS_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("PROMETHEUS_TUI_DIR")
    if env:
        p = Path(env)
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _prometheus_ink_bundle_stale(tui_dir):
        return True
...
def _prometheus_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "prometheus-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``PROMETHEUS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("PROMETHEUS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    prometheus_home = os.environ.get("PROMETHEUS_HOME") or str(Path.home() / ".prometheus")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "PROMETHEUS_HOME": prometheus_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(prometheus_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (PROMETHEUS_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("PROMETHEUS_NODE")
    ...
    # pre-built dist + node_modules (nix / full PROMETHEUS_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("PROMETHEUS_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("PROMETHEUS_QUIET"):
    ...
    if tui_dev:
        if _prometheus_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/prometheus-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
            if result.returncode != 0:
                ...
                print("@prometheus/ink build failed.")
                if preview:
                    print(preview)
                sys.exit(1)
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from prometheus.prometheus_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="prometheus-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["PROMETHEUS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["PROMETHEUS_PYTHON_SRC_ROOT"] = os.environ.get(
        "PROMETHEUS_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("PROMETHEUS_PYTHON", sys.executable)
    env.setdefault("PROMETHEUS_CWD", os.getcwd())
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")
    if model:
        env["PROMETHEUS_MODEL"] = model
        env["PROMETHEUS_INFERENCE_MODEL"] = model
    if provider:
        env["PROMETHEUS_TUI_PROVIDER"] = provider
        env["PROMETHEUS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["PROMETHEUS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    ...
    if resume_session_id:
        env["PROMETHEUS_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("PROMETHEUS_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'prometheus sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Prometheus isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  prometheus setup")
        print()

        from prometheus.prometheus_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'prometheus setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from prometheus.prometheus_cli.banner import prefetch_update_check
    ...
    # user's ~/.prometheus/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["PROMETHEUS_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["PROMETHEUS_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["PROMETHEUS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from prometheus.prometheus_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from prometheus.prometheus_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Prometheus?")
        print()
    ...
    session_dir = get_prometheus_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: prometheus gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  prometheus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Prometheus Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  prometheus gateway")
        ...
        print()
        print("  Or install as a service: prometheus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'prometheus whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from prometheus.prometheus_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``prometheus model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from prometheus.prometheus_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from prometheus.prometheus_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from prometheus.prometheus_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("PROMETHEUS_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'prometheus model' for "
                "available providers, or run 'prometheus doctor' to diagnose config "
                "issues."
            )
    ...
    from prometheus.prometheus_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from prometheus.prometheus_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.prometheus/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.prometheus/.env if the active provider is not 'custom'.
    ...
    """
    from prometheus.prometheus_cli.config import get_env_value, save_env_value, load_config
...
#
# Prometheus uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `prometheus model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `prometheus model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from prometheus.prometheus_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from prometheus.prometheus_cli.config import load_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``prometheus model`` flow, then route aux tasks to them here.
    """
    from prometheus.prometheus_cli.config import load_config
    from prometheus.prometheus_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from prometheus.prometheus_cli.auth import _prompt_model_selection
    from prometheus.prometheus_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from prometheus.prometheus_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from prometheus.prometheus_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from prometheus.prometheus_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from prometheus.prometheus_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from prometheus.prometheus_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from prometheus.prometheus_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from prometheus.prometheus_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `prometheus auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from prometheus.prometheus_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from prometheus.prometheus_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from prometheus.prometheus_cli.models import _PROVIDER_MODELS
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.prometheus/config.yaml.
    """
    from prometheus.prometheus_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from prometheus.prometheus_cli.auth import _save_model_choice, deactivate_provider
    from prometheus.prometheus_cli.config import get_env_value, load_config, save_config
    ...
    from prometheus.prometheus_cli.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"Prometheus will still save it."
        )
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `prometheus model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from prometheus.prometheus_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from prometheus.prometheus_cli.config import get_env_value, save_env_value, load_config, save_config
    from prometheus.prometheus_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Prometheus will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from prometheus.prometheus_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from prometheus.prometheus_cli.config import save_env_value, load_config, save_config
    from prometheus.prometheus_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from prometheus.prometheus_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from prometheus.prometheus_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Prometheus turns to `copilot --acp`.")
    print("  Prometheus currently starts its own ACP subprocess for each request.")
    print("  Prometheus uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set PROMETHEUS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so prometheus knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Prometheus typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Prometheus, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from prometheus.prometheus_cli.auth import AuthError
        from prometheus.prometheus_cli.models import fetch_lmstudio_models
        ...
        from prometheus.prometheus_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from prometheus_constants import display_prometheus_home as _dhh_fn

            print(
                f"    Prometheus will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        ...
        print("    4. Re-run:               prometheus model")
        print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from prometheus.prometheus_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'prometheus model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Prometheus CLI with a provider."""
    from prometheus.prometheus_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from prometheus.prometheus_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from prometheus.prometheus_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from prometheus.prometheus_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from prometheus.prometheus_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from prometheus.prometheus_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``prometheus slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: prometheus slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `prometheus slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from prometheus.prometheus_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from prometheus.prometheus_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from prometheus.prometheus_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from prometheus.prometheus_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from prometheus.prometheus_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from prometheus.prometheus_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from prometheus.prometheus_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Prometheus home directory to a zip file."""
    if getattr(args, "quick", False):
        from prometheus.prometheus_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from prometheus.prometheus_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from prometheus.prometheus_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Prometheus Agent v{__version__} ({__release_date__})")
    ...
    try:
        from prometheus.prometheus_cli.banner import check_for_updates
        from prometheus.prometheus_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Prometheus Agent."""
    _require_tty("uninstall")
    from prometheus.prometheus_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``prometheus update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``prometheus_cli/web_dist/`` (per vite.config.ts
    outDir: "../prometheus_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "prometheus_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``prometheus update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``prometheus web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (prometheus web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``prometheus dashboard`` processes other than ourselves.

    ``prometheus dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``prometheus update`` replaces files on disk, the running
    ...
    patterns = [
        ...
        "prometheus_cli.main dashboard",
        "prometheus_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "prometheus.*dashboard"`) keeps us consistent
            # with `prometheus_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``prometheus dashboard`` processes.

    Called at the end of ``prometheus update`` (default ``reason``) and also
    from ``prometheus dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    prometheus dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Prometheus Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/prometheus-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="prometheus-update-")
        zip_path = os.path.join(tmp_dir, f"prometheus-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to prometheus-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"prometheus-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "prometheus-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Prometheus behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes prometheus completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Prometheus couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Prometheus couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Prometheus behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `prometheus update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/prometheus-agent.git",
    "git@github.com:NousResearch/prometheus-agent.git",
    "https://github.com/NousResearch/prometheus-agent",
    "git@github.com:NousResearch/prometheus-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/prometheus-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_prometheus_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_prometheus_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
    ...
    if not has_upstream:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/prometheus-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/prometheus-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``prometheus update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from prometheus_constants import get_default_prometheus_root

    default_home = get_default_prometheus_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "prometheus-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``prometheus update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.prometheus/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``prometheus update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``prometheus update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.prometheus/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``prometheus update --gateway``) the update is already
    ...
    try:
        ...
        # prometheus_cli.config.get_prometheus_home to simulate setup failure.
        from prometheus.prometheus_cli.config import get_prometheus_home as _get_prometheus_home

        logs_dir = _get_prometheus_home() / "logs"
...
def _cmd_update_check():
    """Implement ``prometheus update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``prometheus update`` without requiring a reinstall.  The
    ...
    ``prometheus`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/prometheus.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v prometheus'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/prometheus, code at /usr/local/lib/prometheus-agent).
    fhs_link = Path("/usr/local/bin/prometheus")
    ...
    path_comment = (
        "# Prometheus Agent — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of PROMETHEUS_HOME before running the update.
    ...
    PROMETHEUS_HOME directories.  The ``--backup`` flag on ``prometheus update``
    ...
    try:
        from prometheus.prometheus_cli.backup import create_pre_update_backup
    except Exception as exc:
    ...
    # Render path using display_prometheus_home so the user sees ~/.prometheus/...
    try:
        from prometheus_constants import get_prometheus_home, display_prometheus_home
        home = get_prometheus_home()
        try:
            display_path = f"{display_prometheus_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  prometheus import {out_path}")
...
def cmd_update(args):
    """Update Prometheus Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from prometheus.prometheus_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Prometheus Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Prometheus Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/prometheus-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # though `git pull` can't touch $PROMETHEUS_HOME, this is cheap
        # belt-and-suspenders insurance and gives the user something to
        # restore from via `/snapshot list` / `/snapshot restore <id>`.
        try:
            from prometheus.prometheus_cli.backup import create_quick_snapshot
        ...
        # the old bytecode (e.g. get_prometheus_home added to prometheus_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload prometheus_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_prometheus_home() added since the last release.
        try:
            import importlib
            import prometheus_constants as _hc
        ...
        try:
            from prometheus.prometheus_cli.profiles import (
                list_profiles,
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'prometheus config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: prometheus config migrate")
            else:
                print()
                print("Skipped. Run 'prometheus config migrate' later to configure.")
        else:
        ...
        # When running as ``prometheus update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_prometheus_home() / ".update_exit_code"
            try:
        ...
        try:
            from prometheus.prometheus_cli.gateway import (
                is_macos,
            ...
            # Discover all prometheus-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. prometheus-gateway.service or prometheus-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: prometheus gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: prometheus -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Prometheus gateway unit files are still installed.
        # When both prometheus.service (from a pre-rename install) and the
        # current prometheus-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `prometheus update` surfaces the issue until the user migrates.
        try:
            from prometheus.prometheus_cli.gateway import (
                has_legacy_prometheus_units,
                _find_legacy_prometheus_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_prometheus_units():
                print()
                print("⚠ Legacy Prometheus gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_prometheus_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (prometheus.service) fight the current")
                print("  prometheus-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    prometheus gateway migrate-legacy")
                print()
        ...
        print("  prometheus model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``prometheus -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from prometheus_constants import display_prometheus_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `prometheus profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_prometheus_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → prometheus -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.prometheus)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  prometheus profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     prometheus -p {name} chat")
                else:
        ...
        from prometheus.prometheus_cli.profiles import profile_exists
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec prometheus -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from prometheus.prometheus_cli.profiles import rename_profile
        ...
        from prometheus.prometheus_cli.profiles import export_profile
        ...
        from prometheus.prometheus_cli.profiles import import_profile
...
def _report_dashboard_status() -> int:
    """Print ``prometheus dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``prometheus dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No prometheus dashboard processes running.")
        return 0

    print(f"{len(pids)} prometheus dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No prometheus dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `prometheus update`.
    ...
    if "PROMETHEUS_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from prometheus.prometheus_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("PROMETHEUS_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from prometheus.prometheus_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Prometheus log files."""
    from prometheus.prometheus_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for prometheus CLI."""
    from prometheus.prometheus_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from prometheus.prometheus_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://prometheus-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `prometheus model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy prometheus.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Prometheus gateway unit files "
            "(e.g. prometheus.service) left over from older installs. Profile "
            "units (prometheus-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Prometheus Agent with an interactive wizard. "
        "Run a specific section: prometheus setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        help="(Default on existing installs.) Re-run the full wizard, "
             ...
             "compatibility — a bare 'prometheus setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Prometheus.",
    )
    ...
    slack_manifest.add_argument(
        ...
        help="Write manifest to a file instead of stdout. With no PATH "
             "writes to $PROMETHEUS_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Prometheus")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Prometheus CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: prometheus-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate Prometheus with Spotify via PKCE")
    auth_spotify.add_argument("spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")
    auth_spotify.add_argument("--client-id", help="Spotify app client_id (or set PROMETHEUS_SPOTIFY_CLIENT_ID)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Prometheus Agent components",
    )
    ...
    # =========================================================================
    from prometheus.prometheus_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.prometheus/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.prometheus/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Prometheus Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Prometheus setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Prometheus Agent. Use 'prometheus debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    prometheus debug share              Upload debug report and print URL
    prometheus debug share --lines 500  Include more log lines
    prometheus debug share --expire 30  Keep paste for 30 days
    prometheus debug share --local      Print report locally (no upload)
    prometheus debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'prometheus debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Prometheus home directory to a zip file",
        description="Create a zip archive of your entire Prometheus configuration, "
        "skills, sessions, and data (excludes the prometheus-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/prometheus-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Prometheus backup from a zip file",
        description="Extract a previously created Prometheus backup into your "
        "Prometheus home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Prometheus Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from prometheus.prometheus_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.prometheus/skills/.bundled_manifest) "
            "so future 'prometheus update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from prometheus.prometheus_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from prometheus.prometheus_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/prometheus-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `prometheus plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from prometheus.prometheus_cli.plugins_cmd import plugins_command
    ...
    try:
        from prometheus.prometheus_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_prometheus_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_prometheus_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_prometheus_home()}/memories/\n")
        else:
            from prometheus.prometheus_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'prometheus tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # prometheus tools list [--platform cli]
    ...
    # prometheus tools disable <name...> [--platform cli]
    ...
    # prometheus tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from prometheus.prometheus_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from prometheus.prometheus_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Prometheus as an MCP server",
        description=(
            "Manage MCP server connections and run Prometheus as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'prometheus mcp add' to connect to a new server, or\n"
            "'prometheus mcp serve' to expose Prometheus conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Prometheus as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from prometheus.prometheus_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch prometheus --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from prometheus.prometheus_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Prometheus",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Prometheus",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.prometheus/ (by default a "
        "single restore-point archive is written to ~/.prometheus/backups/ "
        "before apply; restorable with 'prometheus import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from prometheus.prometheus_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Prometheus Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Prometheus Agent",
        description="Remove Prometheus Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Prometheus Agent as an ACP (Agent Client Protocol) server",
        description="Start Prometheus Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Prometheus Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Prometheus instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Prometheus Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `prometheus --tui` via PTY/WebSocket). "
            "Alternatively set PROMETHEUS_DASHBOARD_TUI=1."
        ),
    )
    ...
    # for `prometheus dashboard` cmdlines and SIGTERM them directly — the same
    # path `prometheus update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running prometheus dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running prometheus dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Prometheus log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    prometheus logs                    Show last 50 lines of agent.log
    prometheus logs -f                 Follow agent.log in real time
    prometheus logs errors             Show last 50 lines of errors.log
    prometheus logs gateway -n 100     Show last 100 lines of gateway.log
    prometheus logs --level WARNING    Only show WARNING and above
    prometheus logs --session abc123   Filter by session ID
    prometheus logs --component tools  Only show tool-related lines
    prometheus logs --since 1h         Lines from the last hour
    prometheus logs --since 30m -f     Follow, starting from 30 min ago
    prometheus logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``prometheus -c Pokemon Agent Dev`` → ``prometheus -c 'Pokemon Agent Dev'``
    ...
    from prometheus.prometheus_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'prometheus -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (prometheus hooks list, cron
    ...
    if (
        ...
        try:
            from prometheus.prometheus_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
    ...
    if getattr(args, "oneshot", None):
        from prometheus.prometheus_cli.oneshot import run_oneshot
