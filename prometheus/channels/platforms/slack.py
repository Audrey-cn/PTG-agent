...
class SlackAdapter(PlatformAdapter):
    ...
    def start(self) -> bool:
        ...
        if not self.bot_token or not self.app_token:
            logger.error("Slack 需要配置 bot_token 和 app_token")
            return False
        try:
            ...
            self._thread.start()
            self._started = True
            logger.info("Slack 适配器已启动")
            return True
        except Exception as e:
            logger.error("Slack 启动失败: %s", e)
            return False
