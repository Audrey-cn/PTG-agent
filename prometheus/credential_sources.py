        ...
        for env_var, (provider, base_url) in self._ENV_MAP.items():
            ...
            if val:
                entries.append(CredentialEntry(
                    key=val, provider=provider, source=f"env:{env_var}", base_url=base_url,
                ))
        return entries
...
class ConfigCredentialSource(CredentialSource):
    ...
    def discover(self) -> list[CredentialEntry]:
        ...
        if key:
            ...
            entries.append(CredentialEntry(
                key=key, provider=provider, source="config:api.key", base_url=base_url,
            ))
                ...
                with open(dotenv_path, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        if any(p in upper_k for p in key_patterns):
                            entries.append(CredentialEntry(
                                key=v, provider="", source=f"dotenv:{dotenv_path.name}:{k}", base_url="",
                            ))
            except Exception as e:
