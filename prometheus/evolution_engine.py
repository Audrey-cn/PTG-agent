...
class EvolutionEngine:
    ...
    def _save_state(self):
        log_path = get_evolution_log_path()
        with _file_lock(log_path), open(log_path, "w", encoding="utf-8") as f:
    ...
    def _contains_sensitive_info(self, text: str) -> bool:
        ...
        return any(keyword.lower() in text_lower for keyword in self.CONFIG["sensitive_keywords"])
    ...
    def _is_duplicate(self, content_hash: str, target_file: str) -> bool:
        for p in self._state.get("proposals", []):
            if (
                p.get("content_hash") == content_hash
                and p.get("target_file") == target_file
                and p.get("status") in ["pending", "accumulating", "ready_for_review"]
            ):
                return True
        return False
    ...
    def _generate_id(self) -> str:
        return datetime.now().strftime("%Y%m%d_%H%M%S_") + self._hash_content(
