"""
Prometheus MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Prometheus-specific extra)

Usage:
    prometheus mcp serve
    prometheus mcp serve --verbose
...
logger = logging.getLogger("prometheus.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using PROMETHEUS_HOME."""
    try:
        from prometheus.constants_core import get_prometheus_home

        return get_prometheus_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("PROMETHEUS_HOME", Path.home() / ".prometheus")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = get_prometheus_home() / "channel_directory.json"
    except ImportError:
        directory_file = (
            Path(os.environ.get("PROMETHEUS_HOME", Path.home() / ".prometheus"))
...
class EventBridge:
    ...
    This is the Prometheus equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def poll_events(
        ...
        return {
            "events": [
                {"cursor": e.cursor, "type": e.type, "session_key": e.session_key, **e.data}
                for e in events
            ],
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = get_prometheus_home() / "state.db"
        except ImportError:
            db_file = (
                Path(os.environ.get("PROMETHEUS_HOME", Path.home() / ".prometheus")) / "state.db"
            )
...
def create_mcp_server(event_bridge: EventBridge | None = None) -> FastMCP:
    """Create and return the Prometheus MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "prometheus",
        instructions=(
            "Prometheus Agent messaging bridge. Use these tools to interact with "
    ...
    def messages_send(
        ...
        try:
            from prometheus.tools.send_message_tool import send_message_tool

            result_str = send_message_tool({"action": "send", "target": target, "message": message})
            return result_str
        except ImportError:
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Prometheus MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
