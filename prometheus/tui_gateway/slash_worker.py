"""Persistent slash-command worker — one PrometheusCLI per TUI session.
...
def _run(cli: PrometheusCLI, command: str) -> str:
...
def main():
    ...
    os.environ["PROMETHEUS_SESSION_KEY"] = args.session_key
    os.environ["PROMETHEUS_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = PrometheusCLI(
            model=args.model or None, compact=True, resume=args.session_key, verbose=False
        )
