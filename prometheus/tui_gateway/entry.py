...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `PROMETHEUS_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("PROMETHEUS_TUI_SIDECAR_URL")
...
# tested; override via ``PROMETHEUS_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("PROMETHEUS_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``PROMETHEUS_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
def main():
    ...
    if _has_mcp_servers:
        try:
            from prometheus.tools.mcp_tool import discover_mcp_tools
