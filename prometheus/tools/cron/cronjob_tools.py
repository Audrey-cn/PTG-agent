"""Cron job management tools for Prometheus Agent."""
...
def _origin_from_env() -> dict[str, str] | None:
    ...
    origin_platform = get_session_env("PROMETHEUS_SESSION_PLATFORM")
    origin_chat_id = get_session_env("PROMETHEUS_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("PROMETHEUS_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("PROMETHEUS_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _resolve_model_override(model_obj: dict[str, Any] | None) -> tuple:
    ...
    job doesn't drift when the user later changes their default via prometheus model.
...
def _normalize_optional_job_value(
    value: Any | None, *, strip_trailing_slash: bool = False
) -> str | None:
    if value is None:
        return None
...
def _validate_cron_script_path(script: str | None) -> str | None:
    ...
    Scripts must be relative paths that resolve within PROMETHEUS_HOME/scripts/.
    ...
    # Only relative paths within ~/.prometheus/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.prometheus/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.prometheus/scripts/ and use just the filename."
        )
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a Python script that runs before each cron job execution. Its stdout is injected into the prompt as context. Use for data collection and change detection. Relative paths resolve under {display_prometheus_home()}/scripts/. On update, pass empty string to clear.",
            },
            "context_from": {
                "type": "array",
                "items": {"type": "string"},
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("PROMETHEUS_INTERACTIVE")
        or os.getenv("PROMETHEUS_GATEWAY_SESSION")
        or os.getenv("PROMETHEUS_EXEC_ASK")
    )
