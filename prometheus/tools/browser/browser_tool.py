...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    prometheus_node_bin = str(prometheus_home / "node" / "bin")
    return [prometheus_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_dialog_policy_config() -> tuple[str, float]:
    from prometheus.tools.browser_supervisor import (
        _VALID_POLICIES,
...
from prometheus.constants_core import is_termux as _is_termux_environment
...
def _reap_orphaned_browser_sessions():
    ...
    pattern = os.path.join(tmpdir, "agent-browser-p_*")
    socket_dirs = glob.glob(pattern)
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-cdp_*"))
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-prometheus_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        try:
            ...
            logger.info(
                "Reaped orphaned browser daemon PID %d (session %s)", daemon_pid, session_name
            )
            reaped += 1
...
def _truncate_snapshot(snapshot_text: str, max_chars: int = 8000) -> str:
    ...
    chars = 0
    for line in lines:
        if chars + len(line) + 1 > max_chars - 80:
            break
        result.append(line)
        chars += len(line) + 1
...
def browser_navigate(url: str, task_id: str | None = None) -> str:
    ...
    from prometheus.agent.redact import _PREFIX_RE
    ...
    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_navigate
    ...
    result = _run_browser_command(
        nav_session_key, "open", [url], timeout=max(_get_command_timeout(), 60)
    )
...
def browser_snapshot(
    ...
    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: str | None = None) -> str:
    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: str | None = None) -> str:
    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_type
...
def browser_scroll(direction: str, task_id: str | None = None) -> str:
    ...
    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_scroll
...
def browser_back(task_id: str | None = None) -> str:
    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: str | None = None) -> str:
    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_press
...
def browser_console(
    clear: bool = False, expression: str | None = None, task_id: str | None = None
) -> str:
    if expression is not None:
        return _browser_eval(expression, task_id)

    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_console
...
def _camofox_eval(expression: str, task_id: str | None = None) -> str:
    from prometheus.tools.browser_camofox import _ensure_tab, _post
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = prometheus_home / "browser_recordings"
...
def browser_get_images(task_id: str | None = None) -> str:
    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: str | None = None) -> str:
    if _is_camofox_mode():
        from prometheus.tools.browser_camofox import camofox_vision
    ...
    screenshots_dir = get_prometheus_dir("cache/screenshots", "browser_screenshots")
    ...
    try:
        ...
        try:
            ...
            from prometheus.tools.vision_tools import (
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if _is_camofox_mode():
        try:
            from prometheus.tools.browser_camofox import camofox_close, camofox_soft_cleanup
...
if __name__ == "__main__":
    ...
    if check_browser_requirements():
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    print("     Docker: pull the latest image")
        except FileNotFoundError:
    ...
    print("\nUsage:")
    print("  from prometheus.tools.browser_tool import browser_navigate, browser_snapshot")
