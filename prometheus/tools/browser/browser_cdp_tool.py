...
# ``websockets`` is a transitive dependency of prometheus-agent (via fal_client
...
def _browser_cdp_via_supervisor(
    ...
    if loop is None or not loop.is_running():
        return tool_error(
            "CDP supervisor loop is not running. Try reconnecting with /browser connect."
        )
...
def _browser_cdp_check() -> bool:
    ...
    """
    try:
        from prometheus.tools.browser_tool import (  # type: ignore[import-not-found]
            _get_cdp_override,
