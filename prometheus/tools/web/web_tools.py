...
from prometheus.agent.auxiliary_client import (
    async_call_llm,
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``prometheus tools``).
...
def _get_direct_firecrawl_config() -> (
    Tuple[dict[str, str], Tuple[str, str | None, str | None]] | None
):
...
def _is_tool_gateway_ready() -> bool:
    ...
    return (
        resolve_managed_tool_gateway("firecrawl", token_reader=_read_nous_access_token) is not None
    )
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "run `prometheus tools` and select Nous Subscription as the web provider."
        )
...
def _resolve_web_extract_auxiliary(
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        from prometheus.agent.auxiliary_client import get_auxiliary_extra_body

        extra_body = get_auxiliary_extra_body() or {"tags": ["product=prometheus-agent"]}
...
Create a markdown summary that captures all key information in a well-organized, scannable format. Include important quotes and code snippets in their original formatting. Focus on actionable information, specific details, and unique insights."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(
                "LLM returned empty content (attempt %d/%d), retrying", attempt + 1, max_retries
            )
...
async def _process_large_content_chunked(
    content: str, context_str: str, model: str | None, chunk_size: int, max_output_size: int
) -> str | None:
    """
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "prometheus-agent"
    return _exa_client
...
async def web_crawl_tool(
    ...
    try:
        ...
        for item in data_list:
            ...
            if page_blocked:
                logger.info(
                    "Blocked crawled page %s by rule %s", page_blocked["host"], page_blocked["rule"]
                )
                pages.append(
                    {
                        "url": page_url,
...
def check_web_api_key() -> bool:
    ...
    return any(
        _is_backend_available(backend) for backend in ("exa", "parallel", "firecrawl", "tavily")
    )
