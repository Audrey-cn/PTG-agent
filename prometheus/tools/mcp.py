...
class MCPManager:
    ...
    def __init__(self):
        ...
        self.config_path = get_paths().mcp
    ...
    def list_servers(self) -> list[dict[str, str]]:
        ...
        return [{"name": s.get("name", "?"), "command": s.get("command", "")} for s in servers]
...
def add_server(name: str, command: str, args: list[str] | None = None) -> dict[str, Any]:
    """
    添加 MCP 服务器
    ...
    try:
        ...
        # 添加新服务器
        servers.append({"name": name, "command": command, "args": args or []})
