...
def is_safe_path(path: str) -> bool:
    ...
    # 检查敏感路径
    return all(not abs_path.startswith(blocked) for blocked in SENSITIVE_PATHS + BLOCKED_PATHS)
...
def patch_file(path: str, old_string: str, new_string: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "path": path, "changes": 1}
...
def search_files(
    pattern: str, path: str = ".", glob: str | None = None, limit: int = 50
) -> dict[str, Any]:
    """
    搜索文件内容
                    ...
                    with open(file_path, encoding="utf-8", errors="replace") as f:
                        for i, line in enumerate(f, 1):
                            if regex.search(line):
                                ...
                                matches.append(
                                    {"file": rel_path, "line": i, "content": line.rstrip()}
                                )
...
def list_directory(path: str = ".") -> dict[str, Any]:
    ...
    try:
        ...
        return {"path": path, "files": sorted(files), "directories": sorted(directories)}
