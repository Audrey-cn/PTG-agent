...
def _get_active_adapter():
    ...
    try:
        from prometheus.gateway.platforms.yuanbao import get_active_adapter
...
async def search_sticker(query: str = "", limit: int = 10) -> dict:
    ...
    """
    from prometheus.gateway.platforms.yuanbao_sticker import search_stickers
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（PROMETHEUS_SESSION_CHAT_ID）。
    ...
    target = (chat_id or "").strip() or get_session_env("PROMETHEUS_SESSION_CHAT_ID", "")
    if not target:
        return {
            "success": False,
...
def _check_yuanbao():
    ...
    try:
        ...
        if get_session_env("PROMETHEUS_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
...
async def _handle_yb_send_dm(args, **kw):
    ...
    if not group_code:
        try:
            ...
            chat_id = get_session_env("PROMETHEUS_SESSION_CHAT_ID", "")
