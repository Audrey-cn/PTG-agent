...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``prometheus_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from prometheus.cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _get_default_output_dir() -> str:
    ...
    return str(get_prometheus_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.prometheus/config.yaml
# ===========================================================================
def _load_tts_config() -> dict[str, Any]:
    """
    Load TTS configuration from ~/.prometheus/config.yaml.
...
# custom voice-cloning scripts, etc.) into Prometheus without any Python code
...
#           output_format: wav
#
# Prometheus writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__PROMETHEUS_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append(
            (
                token,
...
def _generate_command_tts(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            raise RuntimeError(
                f"TTS provider '{provider_name}' exited with code {exc.returncode}: {detail}"
            ) from exc
...
def _generate_xai_tts(text: str, output_path: str, tts_config: dict[str, Any]) -> str:
    ...
    base_url = (
        str(xai_config.get("base_url") or get_env_value("XAI_BASE_URL") or DEFAULT_XAI_BASE_URL)
        .strip()
        .rstrip("/")
    )

    # Match the documented minimal POST /v1/tts shape by default. Only send
    # output_format when Prometheus actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Prometheus caches Piper voice models.

    Resolves to ``~/.prometheus/cache/piper-voices/`` under the active
    PROMETHEUS_HOME so voice downloads follow profile boundaries.
    """
    from prometheus.constants_core import get_prometheus_dir

    root = Path(get_prometheus_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.prometheus/config.yaml (tts: section).
    ...
    platform = get_session_env("PROMETHEUS_SESSION_PLATFORM", "").lower()
    ...
    try:
                ...
                return json.dumps(
                    {
                        ...
                        "Run: pip install 'prometheus-agent[mistral]'",
                    },
                    ensure_ascii=False,
                )
                ...
                return json.dumps(
                    {
                        ...
                        "Run prometheus setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all].",
                    },
                    ensure_ascii=False,
                )
                ...
                return json.dumps(
                    {
                        ...
                        "Run 'prometheus setup tts' and choose KittenTTS, or install manually: "
                ...
                return json.dumps(
                    {
                        ...
                        "Run 'prometheus tools' and select Piper under TTS, or install manually: "
        ...
        logger.info(
            "TTS audio saved: %s (%s bytes, provider: %s)", file_str, f"{file_size:,}", provider
        )
    ...
    try:
        ...
        def _play_via_tempfile(audio_iter, stop_evt):
            ...
            try:
                ...
                from prometheus.tools.voice_mode import play_audio_file
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_prometheus_home()}/audio_cache/<timestamp>.mp3",
            },
        },
        "required": ["text"],
    },
