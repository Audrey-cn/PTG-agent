...
LOCAL_STT_COMMAND_ENV = "PROMETHEUS_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "PROMETHEUS_LOCAL_STT_LANGUAGE"
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set PROMETHEUS_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local(file_path: str, model_name: str) -> dict[str, Any]:
    ...
    try:
        if _local_model is None or _local_model_name != model_name:
            logger.info(
                "Loading faster-whisper model '%s' (first load downloads the model)...", model_name
            )
...
def _transcribe_local_command(file_path: str, model_name: str) -> dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="prometheus-local-stt-") as output_dir:
...
def _transcribe_xai(file_path: str, model_name: str) -> dict[str, Any]:
    ...
    base_url = (
        str(xai_config.get("base_url") or get_env_value("XAI_STT_BASE_URL") or XAI_STT_BASE_URL)
        .strip()
        .rstrip("/")
    )
    language = str(
        xai_config.get("language")
        or os.getenv("PROMETHEUS_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
