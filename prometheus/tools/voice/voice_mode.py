...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "prometheus_voice")
            ...
            if self._on_silence_stop is not None:
                ...
                if rms > self._silence_threshold:
                    ...
                    if self._speech_start == 0.0:
                        ...
                        logger.debug(
                            "Speech confirmed (%.2fs above threshold)", now - self._speech_start
                        )
                    ...
                    if self._dip_start == 0.0:
                        ...
                        logger.debug(
                            "Speech attempt reset (dip lasted %.2fs)", now - self._dip_start
                        )
                ...
                if self._has_spoken and rms <= self._silence_threshold:
                    ...
                    if self._silence_start == 0.0:
                        ...
                        logger.info(
                            "Silence detected (%.1fs), auto-stopping", self._silence_duration
                        )
                        should_fire = True
...
def check_voice_requirements() -> dict[str, Any]:
    ...
    from prometheus.tools.transcription_tools import _get_provider, _load_stt_config, is_stt_enabled
