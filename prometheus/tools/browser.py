    ...
    def get_browser(cls, headless: bool = True):
        ...
        if cls._browser is None or cls._browser.is_closed():
            if not PLAYWRIGHT_AVAILABLE:
                raise ImportError("Playwright 未安装，请运行: pip install playwright && playwright install")
...
def navigate(url: str, headless: bool = True) -> Dict[str, Any]:
    ...
    try:
        if not PLAYWRIGHT_AVAILABLE:
            return {"error": "Playwright 未安装，请运行: pip install playwright && playwright install"}
...
def take_screenshot(path: Optional[str] = None) -> Dict[str, Any]:
    """
    截图
