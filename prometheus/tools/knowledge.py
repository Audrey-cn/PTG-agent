...
"""
╔══════════════════════════════════════════════════════════════╗
║   📚 普罗米修斯 · 知识管理 · Knowledge Manager              ║
║                                                              ║
║   Prometheus 的结构化知识检索层。                             ║
║                                                              ║
║   设计哲学：                                                  ║
║     • 默认连接 Hermes Wiki（复用已有知识）                    ║
║     • Wiki 不可用时自动创建本地知识库                         ║
║     • 可配置指向任意目录作为 Wiki 源                          ║
║     • 为未来独立 Agent 框架预留 fallback 机制                ║
║                                                              ║
║   两层架构：                                                  ║
║     第一层：种子结构索引（Prometheus 自建）                    ║
║       → 种子↔基因↔概念的快速关系查询                         ║
║     第二层：知识检索（接入 Hermes Wiki 或本地 fallback）      ║
║       → 通用知识的全文检索和关联发现                          ║
║                                                              ║
║   [独立Agent模式] 当 Prometheus 脱离 Hermes 独立运行时，      ║
║   Wiki 连接失败会自动降级到本地知识库，无需人工干预。          ║
╚══════════════════════════════════════════════════════════════╝
"""
...
class WikiConnector:
    ...
    def _parse_page(self, filepath: str) -> Optional[WikiPage]:
    ...
    def search(self, query: str, limit: int = 10,
               page_type: str = None, 
               min_maturity: str = None) -> List[dict]:
        """全文搜索 Wiki 内容。
    ...
    def _extract_snippet(self, content: str, query_tokens: Set[str], 
...
class SeedIndex:
    ...
    def _parse_seed(self, filepath: str) -> Optional[SeedInfo]:
        """解析种子文件，提取结构信息。"""
...
class KnowledgeManager:
    ...
    def __init__(self, wiki_paths: List[str] = None,
        ...
        # Wiki 连接
        self._wiki: Optional[WikiConnector] = None
        self._local: Optional[LocalKnowledge] = None
    ...
    def search(self, query: str, limit: int = 10,
        ...
        results["total"] = (len(results["seeds"]) + 
                           len(results["wiki"]) + 
                           len(results["local"]))
...
_km: Optional[KnowledgeManager] = None
