...
1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``prometheus kanban complete …``
   inside the container, where ``prometheus`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.prometheus/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``prometheus kanban …``), the dashboard
(``prometheus dashboard``), and the slash command (``/kanban …``) — all
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "note": {
                "type": "string",
                "description": (
                    "Optional short note describing current progress. Shown in the event log."
                ),
            },
        },
        "required": [],
