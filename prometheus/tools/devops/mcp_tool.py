...
# per-profile log file (~/.prometheus/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> Tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            prometheus_home = os.path.expanduser(
                os.getenv("PROMETHEUS_HOME", os.path.join(os.path.expanduser("~"), ".prometheus"))
            )
            candidates = [
                os.path.join(prometheus_home, "node", "bin", resolved_command),
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        from prometheus.tools.osv_check import check_package_for_malware
        ...
        # ~/.prometheus/logs/mcp-stderr.log.
...
def _get_auth_error_types() -> tuple:
    ...
    try:
        ...
        pass
    try:
        from prometheus.tools.mcp_oauth import OAuthNonInteractiveError
    ...
    return json.dumps(
        {
            "error": (
                ...
                f"Run `prometheus mcp login {server_name}` (or delete the tokens "
                f"file under ~/.prometheus/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> dict[str, dict]:
    """Read ``mcp_servers`` from the Prometheus config file.
    ...
    try:
        ...
        try:
            from prometheus.cli.env_loader import load_prometheus_dotenv

            load_prometheus_dotenv()
        except Exception:
            pass
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        if not server or not server.session:
            _bump_server_error(server_name)
            return json.dumps(
                {"error": f"MCP server '{server_name}' is not connected"}, ensure_ascii=False
            )
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Prometheus's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Prometheus registry schema format.
...
def probe_mcp_server_tools() -> dict[str, list[tuple]]:
    ...
    Designed for ``prometheus tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Prometheus registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple prometheus
