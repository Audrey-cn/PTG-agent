...
SEND_MESSAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "message": {
                "type": "string",
                "description": "The message text to send. To send an image or file, include MEDIA:<local_path> (e.g. 'MEDIA:/tmp/prometheus/cache/img_xxx.jpg') in the message — the platform will deliver it as a native media attachment.",
            },
        },
        "required": [],
    },
...
def _handle_list():
    ...
    try:
        from prometheus.gateway.channel_directory import format_directory_for_display
...
def _handle_send(args):
    ...
    if target_ref and not is_explicit:
        try:
            from prometheus.gateway.channel_directory import resolve_channel_name
    ...
    try:
        from prometheus.gateway.config import Platform, load_gateway_config
    ...
    if not pconfig or not pconfig.enabled:
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(
                    f"Platform '{platform_name}' is not configured. Set up credentials in ~/.prometheus/config.yaml or environment variables."
                )
        else:
            return tool_error(
                f"Platform '{platform_name}' is not configured. Set up credentials in ~/.prometheus/config.yaml or environment variables."
    ...
    from prometheus.gateway.platforms.base import BasePlatformAdapter
    ...
    if not chat_id:
        ...
        if not home and platform_name == "weixin":
            ...
            if wx_home:
                from prometheus.gateway.config import HomeChannel
            ...
            return json.dumps(
                {
                    ...
                    f"or set a home channel via: prometheus config set {platform_name.upper()}_HOME_CHANNEL <channel_id>"
                }
            )
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                from prometheus.gateway.mirror import mirror_to_session
                from prometheus.gateway.session_context import get_session_env

                source_label = get_session_env("PROMETHEUS_SESSION_PLATFORM", "cli")
                user_id = get_session_env("PROMETHEUS_SESSION_USER_ID", "") or None
...
def _get_cron_auto_delivery_target():
    ...
    platform = get_session_env("PROMETHEUS_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("PROMETHEUS_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("PROMETHEUS_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_via_adapter(platform, pconfig, chat_id, chunk):
    ...
    try:
        from prometheus.gateway.run import _gateway_runner_ref
    ...
    return {
        "error": f"No live adapter for platform '{platform.value}'. Is the gateway running with this platform connected?"
    }
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None):
    ...
    from prometheus.gateway.platforms.base import BasePlatformAdapter, utf16_len
    from prometheus.gateway.platforms.discord import DiscordAdapter
    from prometheus.gateway.platforms.slack import SlackAdapter
    ...
    if platform == Platform.SLACK and message:
        try:
            ...
            logger.debug(
                "Failed to apply Slack mrkdwn formatting in _send_to_platform", exc_info=True
            )
        ...
        if formatted.strip():
            try:
                ...
                if (
                    ...
                    if not _has_html:
                        try:
                            from prometheus.gateway.platforms.telegram import _strip_mdv2
...
async def _send_discord(token, chat_id, message, thread_id=None, media_files=None):
    ...
    try:
        ...
        if thread_id:
            ...
            _channel_type = None
            try:
                from prometheus.gateway.channel_directory import lookup_channel_type
                ...
                if cached is not None:
                    ...
                    try:
                        ...
                        async with (
                            ...
                            info_sess.get(info_url, headers=json_headers, **_req_kw) as info_resp,
                        ):
            ...
            if is_forum:
                ...
                async with aiohttp.ClientSession(
                    ...
                    if valid_media:
                        ...
                        form.add_field(
                            "payload_json", payload_json, content_type="application/json"
                        )
                        ...
                        try:
                            ...
                            return _error(
                                _sanitize_error_text(f"Discord forum thread upload failed: {e}")
                            )
                    else:
                        async with session.post(
                            thread_url,
...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    from prometheus.gateway.platforms.signal_rate_limit import (
    ...
    try:
        ...
        if failed_batches and len(failed_batches) == len(att_batches):
            return _error(
                f"Signal: every batch ({len(att_batches)}) hit rate limit; no attachments delivered"
            )
...
async def _send_matrix(token, extra, chat_id, message):
    ...
    try:
        ...
        txn_id = f"prometheus_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        from prometheus.gateway.platforms.matrix import MatrixAdapter
    except ImportError:
        return {
...
async def _send_wecom(extra, chat_id, message):
    ...
    try:
        from prometheus.gateway.platforms.wecom import WeComAdapter, check_wecom_requirements
...
async def _send_weixin(pconfig, chat_id, message, media_files=None):
    ...
    try:
        from prometheus.gateway.platforms.weixin import (
...
async def _send_bluebubbles(extra, chat_id, message):
    ...
    try:
        from prometheus.gateway.platforms.bluebubbles import (
            BlueBubblesAdapter,
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        from prometheus.gateway.platforms.feishu import FEISHU_AVAILABLE, FeishuAdapter

        if not FEISHU_AVAILABLE:
            return {
                "error": "Feishu dependencies not installed. Run: pip install 'prometheus-agent[feishu]'"
            }
        from prometheus.gateway.platforms.feishu import FEISHU_DOMAIN, LARK_DOMAIN
    except ImportError:
        return {
            "error": "Feishu dependencies not installed. Run: pip install 'prometheus-agent[feishu]'"
        }
...
def _check_send_message():
    ...
    platform = get_session_env("PROMETHEUS_SESSION_PLATFORM", "")
...
async def _send_yuanbao(chat_id, message, media_files=None):
    ...
    try:
        from prometheus.gateway.platforms.yuanbao import get_active_adapter, send_yuanbao_direct
    except ImportError:
    ...
    if adapter is None:
        return _error(
            "Yuanbao adapter is not running. Start the gateway with yuanbao platform enabled first."
        )
