...
class GitHubAuth:
    ...
    def _resolve_token(self) -> str | None:
        ...
        if token:
            ...
            self._app_token_expiry = time.time() + 3500
            return token
...
class GitHubSource(SkillSource):
    ...
    def search(self, query: str, limit: int = 10) -> list[SkillMeta]:
        ...
        seen = {}
        for r in results:
            if r.name not in seen or _trust_rank.get(r.trust_level, 0) > _trust_rank.get(
    ...
    def inspect(self, identifier: str) -> SkillMeta | None:
        ...
        if isinstance(metadata, dict):
            prometheus_meta = metadata.get("prometheus", {})
            if isinstance(prometheus_meta, dict):
...
class WellKnownSkillSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> SkillBundle | None:
        ...
        try:
            ...
            logger.warning(
                "Well-known skill identifier contained unsafe skill name: %s", identifier
            )
            return None
    ...
    def _parse_identifier(self, identifier: str) -> dict | None:
        raw = (
            identifier[len("well-known:") :] if identifier.startswith("well-known:") else identifier
        )
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> SkillMeta | None:
        ...
        if isinstance(metadata, dict):
            prometheus_meta = metadata.get("prometheus", {})
            if isinstance(prometheus_meta, dict):
                raw_tags = prometheus_meta.get("tags", [])
...
class SkillsShSource(SkillSource):
    ...
    def _discover_identifier(self, identifier: str, detail: dict | None = None) -> str | None:
        ...
        try:
            ...
            resp = httpx.get(
                root_url, headers=self.github.auth.get_headers(), timeout=15, follow_redirects=True
            )
            if resp.status_code == 200:
                entries = resp.json()
    ...
    def _finalize_inspect_meta(
        self, meta: SkillMeta, canonical: str, detail: dict | None
    ) -> SkillMeta:
        ...
        if isinstance(detail, dict):
            ...
            if body_summary:
                ...
                meta.description = (
                    f"{meta.description} · {weekly_installs} weekly installs on skills.sh"
                )
        return meta
...
class ClawHubSource(SkillSource):
    ...
    All skills are treated as community trust.
    """
    ...
    def _search_catalog(self, query: str, limit: int = 10) -> list[SkillMeta]:
        cache_key = (
            f"clawhub_search_catalog_v1_{hashlib.md5(f'{query}|{limit}'.encode()).hexdigest()}"
        )
        cached = _read_index_cache(cache_key)
        if cached is not None:
    ...
    def _extract_files(self, version_data: dict[str, Any]) -> dict[str, str]:
        ...
        for file_meta in file_list:
            ...
            raw_url = (
                file_meta.get("rawUrl") or file_meta.get("downloadUrl") or file_meta.get("url")
            )
...
class LobeHubSource(SkillSource):
    ...
    def search(self, query: str, limit: int = 10) -> list[SkillMeta]:
        ...
        for agent in agents:
            ...
            searchable = (
                f"{title} {desc} {' '.join(tags) if isinstance(tags, list) else ''}".lower()
            )
            if query_lower in searchable:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official but not activated by default.
    """
    ...
    def _scan_all(self) -> list[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                prometheus_meta = meta_block.get("prometheus", {})
                if isinstance(prometheus_meta, dict):
                    tags = prometheus_meta.get("tags", [])
...
def uninstall_skill(skill_name: str) -> tuple[bool, str]:
    ...
    append_audit_log(
        "UNINSTALL", skill_name, entry["source"], entry["trust_level"], "n/a", "user_request"
    )
...
PROMETHEUS_INDEX_URL = "https://prometheus-agent.nousresearch.com/docs/api/skills-index.json"
PROMETHEUS_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "prometheus-index.json"
PROMETHEUS_INDEX_TTL = 6 * 3600


def _load_prometheus_index() -> dict | None:
    """Fetch the centralized skills index, with local cache."""
    if PROMETHEUS_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - PROMETHEUS_INDEX_CACHE_FILE.stat().st_mtime
            if age < PROMETHEUS_INDEX_TTL:
                return json.loads(PROMETHEUS_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    try:
        resp = httpx.get(PROMETHEUS_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Prometheus index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Prometheus index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    try:
        PROMETHEUS_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        PROMETHEUS_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class PrometheusIndexSource(SkillSource):
    """Skill source backed by the centralized Prometheus Skills Index."""
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_prometheus_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> SkillBundle | None:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "prometheus-index")
    ...
    def inspect(self, identifier: str) -> SkillMeta | None:
        """Return metadata from the index."""
        index = self._ensure_loaded()
        entry = self._find_entry(identifier, index)
        if entry:
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "prometheus-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: GitHubAuth | None = None) -> list[SkillSource]:
    ...
    sources: list[SkillSource] = [
        OptionalSkillSource(),
        PrometheusIndexSource(auth=auth),
...
def parallel_search_sources(
    ...
    if source_filter == "all":
        for src in sources:
            if src.source_id() == "prometheus-index" and getattr(src, "is_available", False):
