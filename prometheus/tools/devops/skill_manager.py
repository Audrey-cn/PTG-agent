...
def scan_for_dangerous_patterns(content: str) -> list:
    ...
    for pattern in DANGEROUS_PATTERNS:
        ...
        for match in matches:
            issues.append({"pattern": pattern, "match": match.group(0), "position": match.start()})
    return issues
...
def create_skill(args):
    ...
    if dangerous:
        return tool_error({"message": "检测到潜在危险模式", "issues": dangerous})
    ...
    if not validation["valid"]:
        ...
        return tool_error({"message": "创建的技能文件无效", "issues": validation["issues"]})

    return tool_result({"name": name, "path": str(skill_file), "status": "created"})
...
def edit_skill(args):
    ...
    if not validation["valid"]:
        ...
        return tool_error({"message": "更新的技能文件无效，已回滚", "issues": validation["issues"]})

    return tool_result(
        {"name": name, "path": skill.path, "status": "updated", "backup": backup_path}
    )
...
def delete_skill(args):
    ...
    return tool_result({"name": name, "status": "deleted", "backup": backup_dir})
...
def suggest_skill_from_session(args):
    ...
    if len(tool_calls) < 3:
        return tool_result({"suggested": False, "reason": "工具调用链太短，不需要创建技能"})

    suggestion = {
        "suggested": True,
        "name": args.get(
            "suggested_name", f"auto_skill_{hashlib.md5(str(tool_calls).encode()).hexdigest()[:8]}"
        ),
