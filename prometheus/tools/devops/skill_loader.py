...
class SkillParser:
    ...
    @staticmethod
    def parse_file(filepath: str) -> Skill | None:
        """解析单个 SKILL.md 文件"""
    ...
    @staticmethod
    def parse_content(content: str, filepath: str = "<inline>") -> Skill | None:
        """解析 SKILL.md 内容"""
        # 验证开头
...
class SkillLoader:
    ...
    def search(self, query: str) -> list[Skill]:
        ...
        for skill in self._skills.values():
            if (
                query_lower in skill.meta.name.lower()
                or query_lower in skill.meta.description.lower()
                or any(query_lower in tag.lower() for tag in skill.meta.tags)
            ):
...
def find_skill(name: str, skill_paths: list[str] = None) -> Skill | None:
    """便捷函数：查找单个 Skill"""
    loader = SkillLoader(skill_paths=skill_paths)
    loader.scan()
...
def main():
    ...
    parser.add_argument(
        "command",
        choices=["scan", "list", "get", "search", "validate", "stats", "catalog"],
        help="操作命令",
    )
