...
def managed_nous_tools_enabled() -> bool:
    ...
    try:
        ...
        from prometheus.cli.models import check_nous_free_tier

        return not check_nous_free_tier()
    except Exception:
        return False


def normalize_browser_cloud_provider(value: Optional[object]) -> str:
    """Return a normalized browser provider key."""
    provider = str(value or _DEFAULT_BROWSER_PROVIDER).strip().lower()
    return provider or _DEFAULT_BROWSER_PROVIDER


def coerce_modal_mode(value: Optional[object]) -> str:
...
def normalize_modal_mode(value: Optional[object]) -> str:
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    managed_mode_blocked = requested_mode == "managed" and not managed_nous_tools_enabled()
...
def resolve_openai_audio_api_key() -> str:
    ...
    return (os.getenv("VOICE_TOOLS_OPENAI_KEY", "") or os.getenv("OPENAI_API_KEY", "")).strip()
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.prometheus/.env`` (via
    ``prometheus.config.get_env_value`` when available) so tool-side
    ...
    if value is None:
        try:
            from prometheus.config import get_env_value
