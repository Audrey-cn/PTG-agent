...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/prometheus_bg_{session.id}.log"
        pid_path = f"{temp_dir}/prometheus_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/prometheus_bg_{session.id}.exit"
    ...
    def _env_poller_loop(
        ...
        while not session.exited:
            ...
            try:
                ...
                if new_output:
                    delta = (
                        new_output[prev_output_len:] if len(new_output) > prev_output_len else ""
                    )
    ...
    def poll(self, session_id: str) -> dict:
        ...
        with session._lock:
            output_preview = (
                strip_ansi(session.output_buffer[-1000:]) if session.output_buffer else ""
            )
    ...
    def has_active_for_session(self, session_key: str) -> bool:
        ...
        with self._lock:
            return any(
                s.session_key == session_key and not s.exited for s in self._running.values()
            )
    ...
    def _prune_if_needed(self):
        now = time.time()
        expired = [
            sid for sid, s in self._finished.items() if (now - s.started_at) > FINISHED_TTL_SECONDS
        ]
        for sid in expired:
...
def _handle_process(args, **kw):
    ...
    if action == "list":
        return json.dumps(
            {"processes": process_registry.list_sessions(task_id=task_id)}, ensure_ascii=False
        )
