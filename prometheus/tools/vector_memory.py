...
class VectorStore:
    ...
    def _init_db(self):
        with self._conn() as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_vectors_importance ON vectors(importance DESC)"
            )
            conn.execute(
...
class SemanticMemory:
    ...
    def superpose_memories(self, contents: list[str], weights: list[float] = None) -> dict:
