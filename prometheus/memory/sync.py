...
"""
╔══════════════════════════════════════════════════════════════╗
║   🔄 普罗米修斯 · 同步管理器 · Sync Manager                 ║
║                                                              ║
║   MD 文件 ↔ SQLite 双向同步                                  ║
║     - sync_from_md(): MD → SQLite                           ║
║     - sync_to_md(): SQLite → MD                             ║
║     - reconcile(): 解决冲突                                  ║
╚══════════════════════════════════════════════════════════════╝
"""
...
class SyncManager:
    ...
    def sync_from_md(self, force: bool = False) -> Dict[str, int]:
        ...
        md_files = glob.glob(
            os.path.join(self.storage.data_dir, "**/*.md"), 
            recursive=True
        )
    ...
    def verify_integrity(self) -> Dict[str, any]:
        ...
        if diff["md_only"] or diff["sqlite_only"] or diff["conflicts"]:
            issues.append(f"Sync differences detected: {len(diff['md_only'])} MD-only, {len(diff['sqlite_only'])} SQLite-only, {len(diff['conflicts'])} conflicts")
