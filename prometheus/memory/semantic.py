...
class SemanticStore:
    ...
    def recall(self, query: str, limit: int = 5, layer: str = None,
               min_similarity: float = 0.1) -> List[VectorRecord]:
        """语义检索"""
        ...
        with self._conn() as conn:
            if layer:
                rows = conn.execute(
                    "SELECT * FROM vectors WHERE layer = ?", (layer,)
                ).fetchall()
            else:
    ...
    def forget(self, record_id: str) -> bool:
        ...
        with self._conn() as conn:
            cursor = conn.execute(
                "DELETE FROM vectors WHERE record_id = ?", (record_id,)
            )
