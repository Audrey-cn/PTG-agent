...
║   创始人: Audrey · 001X                                     ║
║   诞生:   2026-04-29                                        ║
║   使命:   Teach-To-Grow 种子的基因编辑与管理               ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝

用法:
    prometheus view <种子路径>       查看种子完整DNA
    prometheus genes <种子路径>      列出所有基因位点
    prometheus edit <种子路径>       交互式基因编辑
    prometheus decode <种子路径>     解码为史诗叙事
    prometheus lexicon <种子路径>    管理标签词典
    prometheus vault                 种子仓库管理
    prometheus snapshot save <名称>  保存编辑快照
    prometheus snapshot restore      恢复最近快照
    prometheus snapshot list         列出所有快照
    prometheus audit <种子路径>      完整安全审计
    prometheus lineage <种子路径>    生成谱系史诗
    prometheus create <名称>         创建新种子
    prometheus insert <种子> <基因> [锚点] [pos]  插入基因到种子
    prometheus remove <种子> <基因> [--force]     移除基因
    prometheus library              查看基因库
    prometheus health <种子路径>     基因健康度审计
    prometheus fusion <种子A> <种子B>  基因融合分析
    prometheus extract <路径>        外来技能基因拆解
    prometheus memory <子命令>       向量记忆系统（remember/recall/search/status）
    prometheus knowledge <子命令>    知识管理（search/stats/add）
    prometheus dict <子命令>         字典管理（scan/extend）
    prometheus config <子命令>       配置管理（show/set）
    prometheus status                系统总览状态
    prometheus skills                列出可用 Skill 工作流
    prometheus onboard               引导式初始化向导
    prometheus model <子命令>        模型管理（list/info/select/route）
    prometheus channel <子命令>      频道管理（list/start/stop/status）
    prometheus agent <子命令>        Agent管理（create/list/start/stop/status/remove）
"""
...
IMMUTABLE_GENES = {
    ...
    "G008-auditor": ["integrity_check", "origin_verification", "mutation_review", "capability_inventory", "four_layer_framework", "risk_level_system"],
}

# 创始印记不可编辑
FOUNDER_TAGS = ["audrey_001x", "transcend_binary", "human_genesis",
                "divine_parallel", "form_sovereignty", "eternal_mark", "carbon_covenant",
                "promethean_gift", "engineer_craft", "open_source"]
...
def inject_founder_chronicle(content: str, epoch: str) -> str:
    ...
    if 'founder_chronicle:' not in content:
        ...
        tags_json = ', '.join(f'"{t}"' for t in FOUNDER_TAGS)
        # 分行排版，每行最多 3 个标签保持美观
        tags_lines = []
        current_line = []
    ...
    # 3. 在文件末尾添加签名铭刻
    if '*此种子由 Audrey · 001X 的创始印记保护*' not in content and '*此种子由普罗米修斯框架铭刻*' not in content:
        signature = f"""
---
...
def cmd_decode(seed_path: str):
    ...
    genesis = data.get('life_crest', {}).get('genesis', {})
...
""")
    ...
    print(f"║     {'✅' if founder_intact else '❌'} 永恒印记完整({len(FOUNDER_TAGS)}/{len(FOUNDER_TAGS)})")

    # 永恒印记数量
    print(f"║     {'✅' if len(seal_tags) >= len(FOUNDER_TAGS) else '❌'} 永恒印记数量: {len(seal_tags)}")

    # 铭刻来源判定
    has_founder = bool(founder_covenant and eternal_seals)
    has_ttg_markers = bool(genea.get('lineage_laws') or genea.get('bloodline_registry') or genea.get('current_genealogy'))
    
    if has_founder:
        print(f"║     ✅ 铭刻来源: 普罗米修斯框架产物")
        print(f"║        创始契约存在，谱系传承有效")
    elif has_ttg_markers:
        print(f"║     ❌ 铭刻来源: ⚠️ 疑似基因篡改")
        print(f"║        存在族谱结构但创始契约缺失")
    else:
        print(f"║     ❌ 铭刻来源: ⚠️ 外来种子")
        print(f"║        无族谱结构，无创始契约")
...
def load_seed(seed_path: str) -> Optional[dict]:
...
def save_seed(seed_path: str, data: dict):
    ...
    if not block_updates:
        ...
        print("⚠️ 无法定位对应YAML块，补丁已保存至: {}".format(backup_path))
        return
...
class PrometheusAPI:
    ...
    def health(self, seed_path: str) -> dict:
        ...
        data = load_seed(seed_path)
        if not data: return {"error": f"无法读取: {seed_path}"}
    ...
    def fusion(self, seed_a: str, seed_b: str) -> dict:
        ...
        return analyzer.analyze_fusion(genes_a, genes_b,
                                       os.path.basename(seed_a),
                                       os.path.basename(seed_b))
    ...
    def library(self) -> dict:
        ...
        lib = GeneLibrary()
        return {"standard": lib.list_standard(), "narrative": lib.list_narrative(), "optional": lib.list_optional()}
    ...
    def snapshot_list(self) -> list:
        ...
        for f in sorted(os.listdir(SNAPSHOT_DIR), reverse=True):
            if f.endswith('.json'):
                with open(os.path.join(SNAPSHOT_DIR, f)) as fp:
                    try: snapshots.append(json.load(fp))
                    except: pass
        return snapshots[:20]

    def gene_insert(self, seed_path: str, gene_id: str, anchor: str = None,
        ...
        if not gene_def:
            return {"success": False, "message": "基因 {} 不在基因库中".format(gene_id)}

        if not os.path.exists(seed_path):
            return {"success": False, "message": "种子文件不存在: {}".format(seed_path)}
        ...
        if 'locus: "' + gene_id + '"' in content:
            return {"success": False, "message": "基因 {} 已存在于种子中".format(gene_id)}
        ...
        if anchor and position in ('before', 'after'):
            ...
            if anchor_line is None:
                return {"success": False, "message": "锚点基因 {} 不存在".format(anchor)}
        ...
        log_action("gene_insert", "{} into {}".format(gene_id, seed_path))
        return {"success": True, "message": "基因 {} ({}) 已插入".format(gene_id, name),
    ...
    def gene_remove(self, seed_path: str, gene_id: str, force: bool = False) -> dict:
        ...
        if 'locus: "' + gene_id + '"' not in content:
            return {"success": False, "message": "基因 {} 不在种子中".format(gene_id)}
            ...
            for line in lines:
                ...
                if in_target and 'carbon_bonded: true' in line:
                    return {"success": False,
                            "message": "基因 {} 是碳基依赖级基因，不可移除".format(gene_id)}
                if in_target and line.strip().startswith('- locus:') and 'locus: "' + gene_id + '"' not in line:
                    break
        ...
        if target_start is None:
            return {"success": False, "message": "基因 {} 未找到".format(gene_id)}
        ...
        log_action("gene_remove", "{} from {}".format(gene_id, seed_path))
        return {"success": True, "message": "基因 {} 已移除".format(gene_id),
                "gene_id": gene_id}

    def forge(self, parent_seed: str, genes: list = None,
              combinations: str = "power_set", ordering: bool = False,
              max_variants: int = 50, output_dir: str = None,
        ...
        """
        from genes.forge import forge as _forge, MutationSpace
        ms = MutationSpace(genes=genes or [], combinations=combinations,
                           ordering=ordering, max_variants=max_variants)
    ...
    def bank_fuse(self, gene_a: str, gene_b: str,
                  new_gene_id: str = None, new_name: str = None) -> dict:
        """融合两个基因为新基因。"""
        from genes.bank import GeneBank
    ...
    def sieve(self, lab_dir: str, parent_seed: str = None,
              top_k: int = 5) -> dict:
        """筛选锻造批次，返回评分排名。"""
    ...
    def epigenetics_boost(self, seed_path: str, gene_id: str,
                          enhancer: float = None, silencer: float = None,
                          reason: str = "") -> dict:
        """调节基因表达强度
    ...
    def allele_switch(self, seed_path: str, gene_id: str, allele_id: str,
                      reason: str = "") -> dict:
        """切换等位基因版本
    ...
    def allele_register(self, gene_id: str, gene_name: str, allele_data: dict) -> dict:
        """注册新的等位基因版本"""
        from genes.alleles import AlleleManager, Allele
        mgr = AlleleManager()
    ...
    def pathway_trigger(self, seed_path: str, gene_id: str, event: str,
...
def _verify_founder_chronicle(data: dict, seed_path: str = "") -> dict:
    ...
    checks = []
    checks.append({"name": f"永恒印记完整({len(FOUNDER_TAGS)}/{len(FOUNDER_TAGS)})", "passed": founder_intact})
    checks.append({"name": f"永恒印记数量: {len(seal_tags)}", "passed": len(seal_tags) >= len(FOUNDER_TAGS)})
    
    if has_founder:
        checks.append({"name": "铭刻来源: 普罗米修斯框架产物", "passed": True, "detail": "创始契约存在，谱系传承有效"})
    elif has_ttg_markers:
        checks.append({"name": "铭刻来源: ⚠️ 疑似基因篡改", "passed": False, "detail": "存在族谱结构但创始契约缺失——可能被移除"})
    else:
        checks.append({"name": "铭刻来源: ⚠️ 外来种子", "passed": False, "detail": "无族谱结构，无创始契约——非普罗米修斯框架产物"})
...
class SeedGardener:
    ...
    def _read_seed_brief(self, path):
        ...
        try:
            ...
            if m: info["life_id"] = m.group(1)
            
            # sacred_name
            m = re.search(r'sacred_name:\s*"([^"]*)"', content)
            if m: info["sacred_name"] = m.group(1)
            
            # generation
            m = re.search(r'generation:\s*(\d+)', content)
            if m: info["generation"] = int(m.group(1))
            
            # variant
            m = re.search(r'veariant:\s*"([^"]*)"', content)
            if m: info["variant"] = m.group(1)
            
            # parent
            m = re.search(r'parent:\s*"([^"]*)"', content)
            if m: info["parent_id"] = m.group(1)
...
class DormancyGuard:
    ...
    def check_timeout(self):
        ...
        # 检查超时
        timeout_days = self.TIMEOUTS.get(f"{state}_to_{'growing' if state == 'sprouting' else 'blooming' if state == 'growing' else 'dormant'}")
...
def main():
    ...
    if action == 'view' and len(sys.argv) > 2:
        ...
        if len(sys.argv) > 2 and sys.argv[2] == 'save':
            ...
            if len(sys.argv) > 3:
                # 如果第三个参数是路径，则它是种子路径
                if sys.argv[3].startswith('/') or sys.argv[3].startswith('~') or '.ttg' in sys.argv[3]:
            ...
            if not seed_arg:
                # 默认使用始祖种子
                seed_arg = os.path.expanduser("~/.hermes/skills/teach-to-grow/teach-to-grow-core.ttg")
        ...
        if sub_cmd == 'stamp':
            ...
            for i, arg in enumerate(sys.argv[4:], start=4):
                if arg == '--author' and i+1 < len(sys.argv):
                    ...
                    pass
            narrative = sys.argv[narrative_idx] if len(sys.argv) > narrative_idx else "编史官审阅并记录"
        ...
        if data_a and data_b:
            ...
            genes_b = data_b.get('dna_encoding', {}).get('gene_loci', []) if isinstance(data_b.get('dna_encoding'), dict) else []
            analyzer = GeneFusionAnalyzer()
            result = analyzer.analyze_fusion(genes_a, genes_b, 
        ...
        print("🔥 基因锻炉启动")
        print("  亲代: {}".format(parent))
        print("  基因池: {}".format(genes))
        print("  模式: {}".format(mode))
        
        result = api.forge(parent, genes=genes, combinations=mode,
                          ordering=ordering, max_variants=max_v,
                          output_dir=output_dir, batch_name=batch_name)
        
        if "error" in result:
            print("❌ {}".format(result["error"]))
        else:
            print("✅ 锻造完成! {} 个变异体 → ~/.hermes/gene-lab/{}/".format(
        ...
        if "error" in result:
            ...
            if result.get("top"):
                ...
                for t in result["top"]:
                    ...
                    print("       健康:{:.0f} 完整:{:.0f} 新颖:{:.0f} 简洁:{:.0f}".format(
        ...
        if "error" in result:
            ...
            print("  🌍 入土 {:.0f}  🌱 发芽 {:.0f}  🌿 生长 {:.0f}".format(
        ...
        if sub == 'list':
            ...
            if gene:
                ...
                print("❌ 基因 {} 不存在".format(sys.argv[3]))
    ...
    # ── 系统状态 ──
        ...
        from knowledge import KnowledgeManager
        km = KnowledgeManager()
        print(f"  📚 Wiki: {'✅' if km.stats()['wiki_connected'] else '❌ 未连接'} ({km.stats()['wiki_pages']} 页)")
        ...
        if sub == 'silence' and len(sys.argv) >= 5:
            ...
            while i < len(sys.argv):
                if sys.argv[i] == '--enhancer' and i + 1 < len(sys.argv):
                    enhancer = float(sys.argv[i + 1]); i += 2
                elif sys.argv[i] == '--silencer' and i + 1 < len(sys.argv):
                    silencer = float(sys.argv[i + 1]); i += 2
                else:
                    i += 1
            ...
            if not history:
                ...
                for h in history[-10:]:
                    print(f"  {h.get('timestamp', '?')[:19]} · {h.get('gene', '?')} · {h.get('action', '?')}")
        else:
            print("未知子命令")
...
""")
        ...
        if sub == 'scan' and len(sys.argv) >= 4:
            ...
            from genes.repair import print_damage_report, DamageReport
            ...
            if not history:
                ...
                for h in history[-10:]:
                    ...
                    print(f"    损伤: {h.get('damages_found', 0)} · 修复: {h.get('repairs_made', 0)}")
        else:
            print("未知子命令")
    ...
    # ── 模型管理系统 ──
        ...
        from prometheus.models import (
            get_provider_registry, get_model_router, PROVIDER_SPECS
        )
...
""")
        ...
        if sub == 'list':
            ...
            for fb in fallback_info:
                mark = "★" if fb['provider_id'] == registry.active_id else ("✅" if fb['available'] else "❌")
    ...
    # ── 频道管理系统 ──
        ...
        from prometheus.channels import get_channel_registry, ChannelType, ChannelConfig
...
""")
    ...
    # ── Agent 管理系统 ──
        ...
        from prometheus.agents import get_agent_manager, create_default_agent, AgentConfig
...
""")
        ...
        if sub == 'create' and len(sys.argv) > 4:
            ...
            for a in agents:
                ...
                started = a.started_at[:16] if a.started_at else "-"
                print(f"  {icon} [{a.config.agent_id}] {a.config.name} ({a.state})")
                print(f"     提供者: {a.config.provider_id} · 工具: {len(a.config.tools)}个 · 频道: {len(a.config.channels)}个")
                if a.error_message:
