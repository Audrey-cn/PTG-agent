...
class StreamingConfig:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "StreamingConfig":
        ...
        return cls(
            ...
            fresh_final_after_seconds=float(data.get("fresh_final_after_seconds", 60.0)),
        )
...
_PLATFORM_CONNECTED_CHECKERS: dict[Platform, Callable[[PlatformConfig], bool]] = {
    ...
    Platform.WECOM_CALLBACK: lambda cfg: bool(cfg.extra.get("corp_id") or cfg.extra.get("apps")),
    Platform.BLUEBUBBLES: lambda cfg: bool(
        cfg.extra.get("server_url") and cfg.extra.get("password")
    ),
    Platform.QQBOT: lambda cfg: bool(cfg.extra.get("app_id") and cfg.extra.get("client_secret")),
    Platform.YUANBAO: lambda cfg: bool(cfg.extra.get("app_id") and cfg.extra.get("app_secret")),
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_prometheus_home() / "sessions")
    ...
    def get_reset_policy(
        self, platform: Platform | None = None, session_type: str | None = None
    ) -> SessionResetPolicy:
        """
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.prometheus/config.yaml (primary user-facing config)
    3. ~/.prometheus/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
