    ...
    def parse(cls, target: str, origin: SessionSource | None = None) -> "DeliveryTarget":
        ...
        if ":" in target:
            ...
            try:
                ...
                return cls(
                    platform=platform, chat_id=chat_id, thread_id=thread_id, is_explicit=True
                )
            except ValueError:
...
class DeliveryRouter:
    ...
    def __init__(self, config: GatewayConfig, adapters: dict[Platform, Any] = None):
        ...
        self.output_dir = get_prometheus_home() / "cron" / "output"
    ...
    def _save_full_output(self, content: str, job_id: str) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_dir = get_prometheus_home() / "cron" / "output"
    ...
    async def _deliver_to_platform(
        ...
        if len(content) > MAX_PLATFORM_OUTPUT:
            ...
            logger.info(
                "Cron output truncated (%d chars) — full output: %s", len(content), saved_path
            )
            content = (
