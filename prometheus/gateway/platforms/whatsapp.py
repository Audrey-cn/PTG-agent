...
class WhatsAppAdapter(BasePlatformAdapter):
    ...
    # Default bridge location relative to the prometheus-agent install
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(
            config.extra.get(
                "session_path", get_prometheus_dir("platforms/whatsapp/session", "whatsapp/session")
            )
        )
        self._reply_prefix: str | None = config.extra.get("reply_prefix")
        self._dm_policy = (
            str(config.extra.get("dm_policy") or os.getenv("WHATSAPP_DM_POLICY", "open"))
            .strip()
            .lower()
        )
        ...
        self._group_policy = (
            str(config.extra.get("group_policy") or os.getenv("WHATSAPP_GROUP_POLICY", "open"))
            .strip()
            .lower()
        )
    ...
    def _compile_mention_patterns(self):
        ...
        for pattern in patterns:
            ...
            try:
                ...
                logger.warning(
                    "[%s] Invalid WhatsApp mention pattern %r: %s", self.name, pattern, exc
                )
        if compiled:
        ...
        try:
            ...
            if data.get("status") != "connected":
                ...
                for _attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: prometheus whatsapp")
