...
class MattermostAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._reply_mode: str = (
            config.extra.get("reply_mode", "") or os.getenv("MATTERMOST_REPLY_MODE", "off")
        ).lower()
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    ...
                    continue
                logger.warning(
                    "Mattermost: failed to download %s after %d attempts: %s", url, attempt + 1, exc
                )
                return await self.send(chat_id, f"{caption or ''}\n{url}".strip(), reply_to)
                ...
                for image_url, alt_text in chunk:
                    ...
                    if image_url.startswith("file://"):
                        ...
                        try:
                            ...
                            logger.warning(
                                "Mattermost: download failed for %s: %s", image_url[:80], dl_err
                            )
                            continue
                        fname = (
        ...
        while not self._closing:
            try:
                ...
                if isinstance(exc, aiohttp.WSServerHandshakeError) and exc.status in (401, 403):
                    logger.error(
                        "Mattermost WS auth failed (HTTP %d) — stopping reconnect", exc.status
                    )
                    return
    ...
    async def _handle_ws_event(self, event: dict[str, Any]) -> None:
        ...
        if channel_type_raw != "D":
            require_mention = os.getenv("MATTERMOST_REQUIRE_MENTION", "true").lower() not in (
                "false",
                "0",
                "no",
            )
            ...
            try:
                ...
                async with self._session.get(
                    ...
                    if resp.status < 400:
                        ...
                        logger.warning(
                            "Mattermost: failed to download file %s: HTTP %s", fid, resp.status
                        )
            except Exception as exc:
