...
from prometheus.utils import normalize_proxy_url
...
def _no_proxy_entry_matches(entry: str, host: str, port: Optional[int] = None) -> bool:
...
def is_host_excluded_by_no_proxy(hostname: str, no_proxy_value: Optional[str] = None) -> bool:
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.prometheus/.env manually."
)
...
# Default location: {PROMETHEUS_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_prometheus_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_prometheus_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_prometheus_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_prometheus_dir("cache/documents", "document_cache")
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?prometheus\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+prometheus[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._fatal_error_handler: (
            Callable[[BasePlatformAdapter], Awaitable[None] | None] | None
        ) = None
    ...
    def _mark_connected(self) -> None:
        ...
        self._fatal_error_retryable = True
        try:
            from prometheus.gateway.status import write_runtime_status
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        from prometheus.gateway.status import acquire_scoped_lock
    ...
    def set_busy_session_handler(
        self, handler: Callable[[MessageEvent, str], Awaitable[bool]] | None
    ) -> None:
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from prometheus.cli.commands import should_bypass_active_session
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        callback_generation = getattr(interrupt_event, "_prometheus_run_generation", None)
        ...
        try:
            ...
            if response:
                ...
                if (
                    ...
                    try:
                        from prometheus.tools.tts_tool import (
                ...
                if images:
                    ...
                    try:
                        ...
                        logger.warning(
                            "[%s] Error batching images: %s", self.name, batch_err, exc_info=True
                        )
