...
from prometheus.gateway.platforms.helpers import ThreadParticipationTracker
...
# Uses get_prometheus_dir() so each profile gets its own Matrix store.
import contextlib

from prometheus.constants_core import get_prometheus_dir as _get_prometheus_dir

_STORE_DIR = _get_prometheus_dir("platforms/matrix/store", "matrix/store")
...
def check_matrix_requirements() -> bool:
    ...
    try:
        ...
        logger.warning("Matrix: mautrix not installed. Run: pip install 'mautrix[encryption]'")
        return False
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._password: str = config.extra.get("password", "") or os.getenv("MATRIX_PASSWORD", "")
        ...
        self._require_mention: bool = os.getenv("MATRIX_REQUIRE_MENTION", "true").lower() not in (
            "false",
            "0",
            "no",
        )
        ...
        self._dm_auto_thread: bool = os.getenv("MATRIX_DM_AUTO_THREAD", "false").lower() in (
            "true",
            "1",
            "yes",
        )
        ...
        self._reactions_enabled: bool = os.getenv("MATRIX_REACTIONS", "true").lower() not in (
            "false",
            "0",
            "no",
        )
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("PROMETHEUS_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("PROMETHEUS_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                _acct_id = self._user_id or "prometheus"
    ...
    async def send_image(
        ...
        try:
            ...
            try:
                ...
                async with (
                    _aiohttp.ClientSession(**_sess_kw) as http,
                    http.get(
                        image_url,
            ...
            try:
                ...
                if file_bytes is not None:
                    if is_encrypted_media:
                        ...
                        key_value = (
                            file_content.get("key") if isinstance(file_content, dict) else None
                        )
                        if isinstance(key_value, dict):
                            key_value = key_value.get("k")

                        iv_value = (
                            file_content.get("iv") if isinstance(file_content, dict) else None
                        )
                    ...
                    if file_bytes is not None:
                        ...
                        if msg_type == MessageType.PHOTO:
                            ...
                            ext = (
                                Path(
                                    body or ("voice.ogg" if is_voice_message else "audio.ogg")
                                ).suffix
                                or ".ogg"
                            )
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Prometheus Agent" become "Agent".
        """
        if not body:
            return ""
