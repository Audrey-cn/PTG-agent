...
from prometheus.gateway.platforms.helpers import MessageDeduplicator, ThreadParticipationTracker
...
class VoiceReceiver:
    ...
    def _on_packet(self, data: bytes):
        ...
        try:
            ...
            if self._packet_debug_count <= 10:
                logger.warning(
                    "NaCl decrypt failed: %s (hdr=%d, enc=%d)", e, header_size, len(encrypted)
                )
            return
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("PROMETHEUS_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("PROMETHEUS_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        try:
            ...
            if roles_env:
                self._allowed_role_ids = {
                    int(rid.strip()) for rid in roles_env.split(",") if rid.strip().isdigit()
                }
            ...
            from prometheus.gateway.platforms.base import proxy_kwargs_for_bot, resolve_proxy_url
    ...
    async def _run_post_connect_initialization(self) -> None:
        ...
        try:
            ...
            if sync_policy == "bulk":
                ...
                logger.info(
                    "[%s] Synced %d slash command(s) via bulk tree sync", self.name, len(synced)
                )
                return
    ...
    def _canonicalize_app_command_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
        """Reduce command payloads to the semantic fields Prometheus manages."""
    ...
    async def _safe_sync_slash_commands(self) -> dict[str, int]:
        ...
        existing_by_key = {
            (
                int(
                    getattr(getattr(command, "type", None), "value", getattr(command, "type", 1))
                    or 1
                ),
    ...
    async def _send_to_forum(self, forum_channel: Any, content: str) -> SendResult:
        ...
        partial-send issues.
        """
        from prometheus.tools.send_message_tool import _derive_forum_thread_name
        ...
        try:
            ...
            logger.error(
                "[%s] Failed to create forum thread in %s: %s", self.name, forum_channel.id, e
            )
                ...
                for image_url, alt_text in chunk:
                    ...
                    if image_url.startswith("file://"):
                        ...
                        if not os.path.exists(local_path):
                            ...
                            continue
                        files.append(
                            _discord_mod.File(local_path, filename=os.path.basename(local_path))
                        )
                    else:
                        ...
                        try:
                            ...
                            async with aiohttp_session.get(
                                ...
                                **_req_kw,
                                ...
                                files.append(
                                    _discord_mod.File(
                                        _io.BytesIO(data), filename=f"image_{len(files)}.{ext}"
                                    )
                                )
                        except Exception as dl_err:
                            logger.warning(
                                "[%s] Download failed for %s: %s", self.name, image_url[:80], dl_err
                            )
                            continue
    ...
    async def _process_voice_input(self, guild_id: int, user_id: int, pcm_data: bytes):
        ...
        from prometheus.tools.voice_mode import is_whisper_hallucination
        ...
        try:
            ...
            from prometheus.tools.transcription_tools import transcribe_audio
    ...
    async def send_image(
        ...
        try:
            ...
            async with (
                ...
                session.get(image_url, timeout=aiohttp.ClientTimeout(total=30), **_req_kw) as resp,
            ):
                if resp.status != 200:
    ...
    async def send_animation(
        ...
        if not is_safe_url(animation_url):
            logger.warning(
                "[%s] Blocked unsafe animation URL during Discord send_animation", self.name
            )
    ...
    async def get_chat_info(self, chat_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "[%s] Failed to get chat info for %s: %s", self.name, chat_id, e, exc_info=True
            )
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Prometheus session")
        ...
        @tree.command(name="status", description="Show Prometheus session status")
        ...
        @tree.command(name="stop", description="Stop the running Prometheus agent")
        ...
        @tree.command(
            name="reload-skills",
            description="Re-scan ~/.prometheus/skills/ for new or removed skills",
        )
        ...
        @tree.command(name="update", description="Update Prometheus Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Prometheus gateway")
        ...
        @tree.command(
            name="thread", description="Create a new thread and start a Prometheus session in it"
        )
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Prometheus in the thread",
        ...
        # prometheus_cli/commands.py automatically appear as Discord slash
            ...
            if has_args:
                ...
                def _make_args_handler(__name: str, __hint: str):
                    ...
                    async def _handler(interaction: discord.Interaction, args: str = ""):
                        await self._run_simple_slash(interaction, f"/{__name} {args}".strip())
        ...
        try:
            from prometheus.cli.commands import (
                COMMAND_REGISTRY,
        ...
        try:
            from prometheus.cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        """
        try:
            from prometheus.cli.commands import discord_skill_commands_by_category
            ...
            skill_lookup: dict[str, tuple[str, str]] = {n: (d, k) for n, d, k in entries}
            ...
            async def _skill_handler(
                ...
                if not entry:
                    await interaction.response.send_message(
                        f"Unknown skill: `{name}`. Start typing for autocomplete suggestions.",
                        ephemeral=True,
                    )
                    return
                _desc, cmd_key = entry
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Prometheus skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Prometheus session in the thread
    ...
    def _resolve_channel_skills(
        ...
        """
        from prometheus.gateway.platforms.base import resolve_channel_skills

        return resolve_channel_skills(self.config.extra, channel_id, parent_id)

    def _resolve_channel_prompt(self, channel_id: str, parent_id: str | None = None) -> str | None:
        """Resolve a Discord per-channel prompt, preferring the exact channel over its parent."""
        from prometheus.gateway.platforms.base import resolve_channel_prompt
    ...
    async def _create_thread(
        ...
        *,
        ...
        display_name = (
            getattr(getattr(interaction, "user", None), "display_name", None) or "unknown user"
        )
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Prometheus: **{name}**"
    ...
    async def _auto_create_thread(self, message: DiscordMessage) -> Any | None:
        ...
        thread_name = content[:80] if content else "Prometheus"
        if len(content) > 80:
        ...
        try:
            ...
            display_name = (
                getattr(getattr(message, "author", None), "display_name", None) or "unknown user"
            )
            reason = f"Auto-threaded from mention by {display_name}"
            try:
                seed_msg = await message.channel.send(
                    f"\U0001f9f5 Thread created by Prometheus: **{thread_name}**"
                )
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``prometheus update --gateway``
    ...
    async def send_model_picker(
        ...
        try:
            ...
            try:
                from prometheus.cli.providers import get_label
    ...
    async def _cache_discord_document(self, att, ext: str) -> bytes:
        ...
        from prometheus.gateway.platforms.base import proxy_kwargs_for_aiohttp, resolve_proxy_url
        ...
        if not isinstance(message.channel, discord.DMChannel):
            ...
            if allowed_channels_raw:
                ...
                if "*" not in allowed_channels and not (channel_ids & allowed_channels):
                    logger.debug(
                        "[%s] Ignoring message in non-allowed channel: %s", self.name, channel_ids
                    )
                    return
            ...
            is_free_channel = (
                "*" in free_channels or bool(channel_ids & free_channels) or is_voice_linked_channel
            )
    ...
    def _text_batch_key(self, event: MessageEvent) -> str:
        ...
        from prometheus.gateway.session import build_session_key
...
if DISCORD_AVAILABLE:
    ...
    class ExecApprovalView(discord.ui.View):
        ...
        async def _resolve(
            ...
            try:
                from prometheus.tools.approval import resolve_gateway_approval
    ...
    class SlashConfirmView(discord.ui.View):
        ...
        async def _resolve(
            ...
            try:
                ...
                logger.info(
                    "Discord button resolved slash-confirm for session %s (choice=%s, user=%s)",
                    self.session_key,
                    choice,
                    interaction.user.display_name,
                )
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``prometheus update`` prompts.
