...
def parse_drive_comment_event(data: Any) -> dict[str, Any] | None:
    ...
    *data* may be a ``CustomizedEvent`` (WebSocket) whose ``.event`` is a dict,
    ...
    evt: dict = (
        event if isinstance(event, dict) else (vars(event) if hasattr(event, "__dict__") else {})
    )
...
async def query_document_meta(
    ...
    logger.debug(
        "[Feishu-Comment] query_document_meta: file_token=%s file_type=%s", file_token, file_type
    )
...
async def batch_query_comment(
    ...
    """
    logger.debug(
        "[Feishu-Comment] batch_query_comment: file_token=%s comment_id=%s", file_token, comment_id
    )
    ...
    if items and isinstance(items, list):
        ...
        return item
    logger.warning(
        "[Feishu-Comment] batch_query_comment: empty items, raw data keys=%s", list(data.keys())
    )
    return {}
...
async def list_whole_comments(
    ...
    logger.info(
        "[Feishu-Comment] list_whole_comments: total %d whole comments fetched", len(all_comments)
    )
    return all_comments
...
async def list_comment_replies(
    ...
    *,
    ...
    """
    logger.debug(
        "[Feishu-Comment] list_comment_replies: file_token=%s comment_id=%s", file_token, comment_id
    )
...
def _resolve_model_and_runtime() -> tuple[str, dict]:
    ...
    from prometheus.gateway.run import _load_gateway_config, _resolve_gateway_model

    user_config = _load_gateway_config()
    model = _resolve_gateway_model(user_config)

    from prometheus.gateway.run import _resolve_runtime_agent_kwargs
    ...
    if not model and runtime_kwargs.get("provider"):
        try:
            from prometheus.cli.models import get_default_model_for_provider
...
def _run_comment_agent(prompt: str, client: Any, session_key: str = "") -> str:
    ...
    from prometheus.tools.feishu_doc_tool import set_client as set_doc_client
    from prometheus.tools.feishu_drive_tool import set_client as set_drive_client
...
async def handle_drive_comment_event(
    ...
    *,
    ...
    if not to_open_id or (self_open_id and to_open_id != self_open_id):
        logger.debug(
            "[Feishu-Comment] Skipping event not addressed to self: to=%s", to_open_id or "(empty)"
        )
        return
    ...
    # Access control
    from prometheus.gateway.platforms.feishu_comment_rules import (
        has_wiki_keys,
        is_user_allowed,
        load_config,
        resolve_rule,
    ...
    meta_task = asyncio.ensure_future(query_document_meta(client, file_token, file_type))
    ...
    logger.info(
        "[Feishu-Comment] [Step 4/5] Prompt built (%d chars), running agent...", len(prompt)
    )
