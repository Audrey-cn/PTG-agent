...
from prometheus.gateway.platforms.qqbot.constants import (
    API_BASE,
...
from prometheus.gateway.platforms.qqbot.utils import (
    build_user_agent,
)
from prometheus.gateway.platforms.qqbot.utils import (
...
class QQAdapter(BasePlatformAdapter):
        ...
        while self._running:
            try:
                ...
                if await self._reconnect(backoff_idx):
                    ...
                    if backoff_idx >= MAX_RECONNECT_ATTEMPTS:
                        logger.error(
                            "[%s] Max reconnect attempts reached (QQCloseError)", self._log_tag
                        )
                        return
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "intents": (1 << 25)
                | (1 << 30)
                | (1 << 12),  # C2C_GROUP_AT_MESSAGES + PUBLIC_GUILD_MESSAGES + DIRECT_MESSAGE
                "shard": [0, 1],
                "properties": {
                    "$os": "macOS",
                    "$browser": "prometheus-agent",
                    "$device": "prometheus-agent",
                },
            },
        }
        try:
            if self._ws and not self._ws.closed:
                ...
                logger.warning("[%s] Cannot send Identify: WebSocket not connected", self._log_tag)
        except Exception as exc:
    ...
    async def _send_resume(self) -> None:
        ...
        try:
            if self._ws and not self._ws.closed:
                ...
                logger.warning("[%s] Cannot send Resume: WebSocket not connected", self._log_tag)
        except Exception as exc:
    ...
    async def _download_and_cache(self, url: str, content_type: str) -> str | None:
    ...
    def _resolve_stt_config(self) -> dict[str, str] | None:
        ...
        # 2. QQ-specific env vars (set by `prometheus setup gateway` / `prometheus gateway`)
    ...
    async def _wait_for_reconnection(self) -> bool:
        ...
        while waited < self._RECONNECT_WAIT_SECONDS:
            ...
            if self.is_connected:
                ...
                return True
        logger.warning(
            "[%s] Still not connected after %.0fs", self._log_tag, self._RECONNECT_WAIT_SECONDS
        )
        return False
    ...
    async def _send_chunk(
        ...
        retryable = not any(k in error_msg.lower() for k in ("invalid", "forbidden", "not found"))
        return SendResult(success=False, error=error_msg, retryable=retryable)

    async def _send_c2c_text(
        self, openid: str, content: str, reply_to: str | None = None
    ) -> SendResult:
    ...
    async def _send_group_text(
        self, group_openid: str, content: str, reply_to: str | None = None
    ) -> SendResult:
    ...
    async def _load_media(self, source: str, file_name: str | None = None) -> tuple[str, str, str]:
        ...
        if not local_path.exists() or not local_path.is_file():
            ...
            if source.startswith("<") or len(source) < 3:
                raise ValueError(f"Invalid media source (looks like a placeholder): {source!r}")
