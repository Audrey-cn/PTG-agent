...
from prometheus.gateway.platforms.helpers import MessageDeduplicator
...
class SlackAdapter(BasePlatformAdapter):
    ...
    def _describe_slack_api_error(
        ...
        file_label = str(
            (file_obj or {}).get("name") or (file_obj or {}).get("id") or "this attachment"
        )
    ...
    def _describe_slack_download_failure(
        ...
        if httpx is not None and isinstance(exc, httpx.HTTPStatusError):
            ...
            if status == 404:
                return (
                    f"Slack attachment {file_label} returned HTTP 404 and is no longer reachable."
                )
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_prometheus_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /prometheus prefix). A single regex
            ...
            # manifest (see `prometheus slack manifest`). In Socket Mode, Slack
            ...
            from prometheus.cli.commands import slack_native_slashes
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/prometheus$")

            @self._app.command(_slash_pattern)
            async def handle_prometheus_command(ack, command):
                await ack()
                await self._handle_slash_command(command)

            # Register Block Kit action handlers for approval buttons
            for _action_id in (
                "prometheus_approve_once",
                "prometheus_approve_session",
                "prometheus_approve_always",
                "prometheus_deny",
            ):
            ...
            for _action_id in (
                "prometheus_confirm_once",
                "prometheus_confirm_always",
                "prometheus_confirm_cancel",
            ):
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Prometheus session — matching the per-thread behavior channels already
    ...
    def _extract_assistant_thread_metadata(self, event: dict) -> dict[str, str]:
        ...
        team_id = event.get("team") or event.get("team_id") or assistant_thread.get("team_id") or ""
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        team_id = event.get("team") or event.get("team_id") or assistant_meta.get("team_id", "")
        ...
        if not is_dm and bot_uid:
            if channel_id in self._slack_free_response_channels():
                ...
                in_mentioned_thread = (
                    event_thread_ts is not None and event_thread_ts in self._mentioned_threads
                )
        ...
        from prometheus.gateway.platforms.base import resolve_channel_prompt, resolve_channel_skills
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "prometheus_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "prometheus_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "prometheus_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "prometheus_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "prometheus_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "prometheus_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "prometheus_confirm_once": "once",
            "prometheus_confirm_always": "always",
            "prometheus_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "prometheus_approve_once": "once",
            "prometheus_approve_session": "session",
            "prometheus_approve_always": "always",
            "prometheus_deny": "deny",
        }
    ...
    async def _fetch_thread_context(
        ...
        try:
            ...
            for msg in messages:
                ...
                if is_bot and not is_parent and self_bot_uid and msg_user == self_bot_uid:
                    continue
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/prometheus <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/prometheus
        ...
        if slash_name in ("prometheus", ""):
            # Legacy /prometheus <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from prometheus.cli.commands import slack_subcommand_map
    ...
    def _has_active_session_for_thread(
        ...
        try:
            from prometheus.gateway.session import SessionSource, build_session_key
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            for attempt in range(3):
                try:
                    ...
                    if audio:
                        from prometheus.gateway.platforms.base import cache_audio_from_bytes

                        return cache_audio_from_bytes(response.content, ext)
                    else:
                        from prometheus.gateway.platforms.base import cache_image_from_bytes
