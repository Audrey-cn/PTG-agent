...
def _strip_markdown_to_plain_text(text: str) -> str:
    ...
    horizontal rules, \\r\\n normalisation).
    """
    from prometheus.gateway.platforms.helpers import strip_markdown
    ...
    *,
    ...
    for row in resolved.get("content", []) or []:
        ...
        row_text = _normalize_feishu_text(
            "".join(
                _render_post_element(item, image_keys, media_refs, mentions_map) for item in row
            )
        )
        if row_text:
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_prometheus_home() / "feishu_seen_message_ids.json"
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``prometheus_action`` in their value dict so that
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return {
                    ...
                    "type": btn_type,
                    "value": {"prometheus_action": action_name, "approval_id": approval_id},
                }
    ...
    def _on_drive_comment_event(self, data: Any) -> None:
        ...
        """
        from prometheus.gateway.platforms.feishu_comment import handle_drive_comment_event
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        prometheus_action = (
            action_value.get("prometheus_action") if isinstance(action_value, dict) else None
        )

        if prometheus_action:
    ...
    def _handle_approval_card_action(
        ...
        if approval_id is None:
            ...
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("prometheus_action"), "deny")
    ...
    async def _resolve_sender_name_from_api(self, sender_id: str | None) -> str | None:
    ...
    def _is_self_sent_bot_message(self, event: Any) -> bool:
        ...
        if self._bot_open_id and sender_open_id == self._bot_open_id:
            return True
        return bool(self._bot_user_id and sender_user_id == self._bot_user_id)
...
# Called by `prometheus gateway setup` via _setup_feishu() in prometheus_cli/gateway.py.
# =============================================================================
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    if "?" in qr_url:
        qr_url += "&from=prometheus&tp=prometheus"
    else:
        qr_url += "?from=prometheus&tp=prometheus"
    return {
