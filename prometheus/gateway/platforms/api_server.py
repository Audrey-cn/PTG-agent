    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_prometheus_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
...
def _derive_chat_session_id(
    ...
    the same Prometheus session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through prometheus-agent's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``prometheus sessions list``
        shows API-server conversations alongside CLI and gateway ones.
        """
        if self._session_db is None:
            try:
                from prometheus.state import SessionDB
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the prometheus-api-server default.
        """
        from run_agent import AIAgent

        from prometheus.cli.tools_config import _get_platform_tools
        from prometheus.gateway.run import (
        ...
        max_iterations = int(os.getenv("PROMETHEUS_MAX_ITERATIONS", "90"))
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "prometheus-agent"})
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        """
        from prometheus.gateway.status import read_runtime_status

        runtime = read_runtime_status() or {}
        return web.json_response(
            {
                "status": "ok",
                "platform": "prometheus-agent",
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return prometheus-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Prometheus version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response(
            {
                "object": "prometheus.api_server.capabilities",
                "platform": "prometheus-agent",
                ...
                "features": {
                    ...
                    "session_continuity_header": "X-Prometheus-Session-Id",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # Allow caller to continue an existing session by passing X-Prometheus-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Prometheus-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Prometheus-Session-Id rejected: "
            ...
            # conversation map to the same Prometheus session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``prometheus.tool.progress`` with ``status: running``.
                ...
                from prometheus.agent.display import build_tool_preview, get_tool_emoji
        ...
        return web.json_response(response_data, headers={"X-Prometheus-Session-Id": session_id})
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Prometheus-Session-Id"] = session_id
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: prometheus.tool.progress`` SSE event so
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    await response.write(
                        f"event: prometheus.tool.progress\ndata: {event_data}\n\n".encode()
                    )
                else:
                    content_chunk = {
                        "id": completion_id,
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from prometheus.cli.auth import has_usable_secret
