...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Prometheus Agent")
        ...
        msg_id = f"<prometheus-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
    ...
    def _send_email_with_attachments(
        ...
        logger.info(
            "[Email] Sent multi-attachment email to %s (%d files)", to_addr, len(file_paths)
        )
        return msg_id
