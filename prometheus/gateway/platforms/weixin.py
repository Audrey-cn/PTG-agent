...
def _account_dir(prometheus_home: str) -> Path:
    path = Path(prometheus_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(prometheus_home: str, account_id: str) -> Path:
    return _account_dir(prometheus_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(prometheus_home, account_id)
    atomic_json_write(path, payload)
    with contextlib.suppress(OSError):
        path.chmod(0o600)


def load_weixin_account(prometheus_home: str, account_id: str) -> dict[str, Any] | None:
...
class ContextTokenStore:
    ...
    def __init__(self, prometheus_home: str):
        self._root = _account_dir(prometheus_home)
...
def _sync_buf_path(prometheus_home: str, account_id: str) -> Path:
    return _account_dir(prometheus_home) / f"{account_id}.sync.json"


def _load_sync_buf(prometheus_home: str, account_id: str) -> str:
    path = _sync_buf_path(prometheus_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(prometheus_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(prometheus_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    """Native Prometheus adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._prometheus_home = prometheus_home
        self._token_store = ContextTokenStore(prometheus_home)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(prometheus_home, self._account_id)
            if persisted:
    ...
    async def connect(self) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Prometheus regardless of this "
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._prometheus_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._prometheus_home, self._account_id, sync_buf)
        ...
        try:
            ...
            for media_path, is_voice in media_files:
                try:
                    ...
                    logger.warning(
                        "[%s] media delivery failed for %s: %s", self.name, media_path, exc
                    )
            ...
            for file_path in local_files:
                try:
                    ...
                    logger.warning(
                        "[%s] local file delivery failed for %s: %s", self.name, file_path, exc
                    )

            # Deliver text content.
            chunks = [
                c for c in self._split_text(self.format_message(final_content)) if c and c.strip()
            ]
            for idx, chunk in enumerate(chunks):
                client_id = f"prometheus-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = f"prometheus-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
...
async def send_weixin_direct(
    *,
    ...
    token_store = ContextTokenStore(str(get_prometheus_home()))
