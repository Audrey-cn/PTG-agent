...
from prometheus.gateway.platforms.yuanbao_media import (
    build_file_msg_body,
        ...
        for line in lines:
            ...
            if stripped.startswith("|") and stripped.endswith("|"):
                ...
                if re.match(r"^\|[\s\-:]+(\|[\s\-:]+)+\|$", stripped):
                    ...
                    normalized = "|".join(cell.strip() if cell.strip() else cell for cell in cells)
...
class RecallGuardMiddleware(InboundMiddleware):
    ...
    def _interrupt_for_recall(
        cls, adapter, session_key: str, recalled_id: str, group_code: str, from_account: str
    ) -> None:
    ...
    def _schedule_content_redact(
        cls, adapter, session_key: str, recalled_text: str, group_code: str, from_account: str
    ) -> None:
                ...
                for entry in transcript:
                    if entry.get("role") == "user" and entry.get("content") == recalled_text:
                        ...
                        try:
                            ...
                            logger.info(
                                "[%s] Recall redact: session %s", adapter.name, session_key[:30]
                            )
                        except Exception as exc:
                    ...
                    for line in f:
                        ...
                        if line:
                            try:
                                ...
                                logger.debug("[%s] Skipping invalid JSON line", adapter.name)
        except Exception as exc:
...
class SkipSelfMiddleware(InboundMiddleware):
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        if self._is_self_reference(ctx.from_account, ctx.adapter._bot_id):
            logger.debug(
                "[%s] Ignoring self-sent message from %s", ctx.adapter.name, ctx.from_account
            )
            return  # Stop pipeline
        await next_fn()
            ...
            if elem_type == "TIMTextElem":
                ...
                if raw_data:
                    try:
                        ...
                        logger.debug("Face data parse error")
...
class GroupAtGuardMiddleware(InboundMiddleware):
    ...
    def _build_group_channel_prompt(msg_body: list, bot_id: str | None) -> str:
        ...
        bot_mention = (
            GroupAtGuardMiddleware._extract_bot_mention_text(msg_body, bot_id) or "unknown"
        )
        return (
...
class QuoteContextMiddleware(InboundMiddleware):
    ...
    @staticmethod
    def _extract_quote_context(cloud_custom_data: str) -> tuple[str | None, str | None]:
...
class MediaResolveMiddleware(InboundMiddleware):
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        if PlaceholderFilterMiddleware.is_skippable_placeholder(ctx.raw_text, len(ctx.media_urls)):
            logger.debug(
                "[%s] Skip placeholder after media download: %r", adapter.name, ctx.raw_text
            )
            return  # Stop pipeline
        await next_fn()
        ...
        async def _dispatch_inbound_event() -> None:
            ...
            try:
                (
                    extra_img_urls,
                    extra_img_mimes,
                ) = await MediaResolveMiddleware._collect_observed_media(
                    adapter,
                    ctx.source,
                )
            except Exception as exc:
            ...
            if extra_img_urls:
                ...
                for u, m in zip(extra_img_urls, extra_img_mimes, strict=False):
                    if u in current:
                        continue
                    media_urls.append(u)
            ...
            for u, m in zip(media_urls, media_types, strict=False):
                if not u.startswith("/"):
                    continue
...
class ConnectionManager:
    ...
    async def open(self) -> bool:
        ...
        if self._ws is not None:
            try:
                ...
                logger.debug("[%s] Operation error (ignored)", adapter.name)
            ...
            while True:
                ...
                if cmd_type == CMD_TYPE["Response"] and cmd == "auth-bind":
                    ...
                    if connect_id:
                        ...
                        logger.info(
                            "[%s] BIND_ACK received: connectId=%s", adapter.name, connect_id
                        )
                        return True
                    else:
    ...
    def _extract_sender_key(self, raw_data: bytes) -> str:
        ...
        try:
            ...
            logger.debug("[%s] _extract_group_code error", adapter.name)
        ...
        try:
            ...
            logger.debug("[%s] _extract_sender_from_push error", adapter.name)
    ...
    async def _cleanup_ws(self) -> None:
        ...
        if ws is not None:
            try:
                ...
                logger.debug("[%s] WebSocket close error", adapter.name)
...
class StickerHandler(MediaSendHandler):
    ...
    def build_msg_body(self, upload_result, **kwargs):
        from prometheus.gateway.platforms.yuanbao_sticker import (
            build_face_msg_body,
...
class GroupQueryService:
    ...
    async def query_group_info_raw(self, group_code: str) -> dict | None:
        ...
        from prometheus.gateway.platforms.yuanbao_proto import decode_conn_msg as _decode
    ...
    async def query_session_members(
        ...
        if members and len(members) <= 10:
            names = [
                m.get("name_card") or m.get("nickname") or m.get("user_id", "") for m in members
            ]
    ...
    async def stop(self, chat_id: str, send_finish: bool = True) -> None:
        ...
        if send_finish:
            try:
                ...
                logger.debug("[%s] Typing indicator error", adapter.name)
    ...
    async def send_text(
        ...
        if self._on_send_finish:
            try:
                ...
                logger.debug("[%s] Slow mode error", adapter.name)
    ...
    def _build_msg_body_with_mentions(self, text: str, group_code: str) -> list:
        ...
        for match in self._AT_USER_RE.finditer(text):
            ...
            if entry:
                ...
                msg_body.append(
                    {
                        ...
                        "msg_content": {
                            "data": json.dumps(
                                {"elem_type": 1002, "text": f"@{real_nick}", "user_id": uid}
                            ),
                        },
                    }
                )
                ...
                msg_body.append(
                    {"msg_type": "TIMTextElem", "msg_content": {"text": f"@{nickname}"}}
                )
...
class YuanbaoAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs: Any) -> None:
        ...
        # ------------------------------------------------------------------
        dm_policy: str = (
            (_extra.get("dm_policy") or os.getenv("YUANBAO_DM_POLICY", "open")).strip().lower()
        )
        ...
        self._auto_sethome_done: bool = bool(_existing_home) and not _existing_home.startswith(
            "group:"
        )
    ...
    async def send_typing(self, chat_id: str, metadata: dict | None = None) -> None:
        ...
        try:
            ...
            logger.debug("[%s] send_typing error: %s", adapter.name, exc)
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        try:
            ...
            logger.debug("[%s] stop_typing error: %s", adapter.name, exc)
