...
def fetch_models_dev_catalog(force_refresh: bool = False) -> dict[str, Any]:
    ...
    try:
        request = urllib.request.Request(
            MODELS_DEV_URL, headers={"User-Agent": "Prometheus-Agent/1.0"}
        )
    ...
    if isinstance(provider_data, dict):
        for model_id, model_info in provider_data.items():
            if isinstance(model_info, dict):
                models.append({"id": model_id, "provider": provider, **model_info})
        ...
        if isinstance(provider_data, dict):
            for mid, model_info in provider_data.items():
                if mid == model_id and isinstance(model_info, dict):
                    return {"id": model_id, "provider": provider, **model_info}
            ...
            for model_id, model_info in provider_data.items():
                if isinstance(model_info, dict):
                    ...
                    if query_lower in searchable:
                        results.append({"id": model_id, "provider": provider, **model_info})

    return results


def merge_with_local_catalog(
    local_catalog: dict[str, Any], dev_catalog: dict[str, Any]
) -> dict[str, Any]:
    merged = {}
...
def get_all_providers() -> list[str]:
    catalog = fetch_models_dev_catalog()
    return [p for p in catalog if not p.startswith("_")]
...
def get_popular_models(limit: int = 20) -> list[dict[str, Any]]:
    ...
    for provider, provider_data in catalog.items():
        ...
        if isinstance(provider_data, dict):
            for model_id, model_info in provider_data.items():
                if isinstance(model_info, dict):
                    all_models.append({"id": model_id, "provider": provider, **model_info})
