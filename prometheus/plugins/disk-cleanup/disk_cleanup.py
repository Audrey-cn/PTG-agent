"""disk_cleanup — ephemeral file cleanup for Prometheus Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under PROMETHEUS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly PROMETHEUS_HOME and /tmp/prometheus-*
Never touches: ~/.prometheus/logs/ or any system directory.
"""
...
try:
    ...
    def get_prometheus_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("PROMETHEUS_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".prometheus").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$PROMETHEUS_HOME/logs/``."""
    return get_prometheus_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$PROMETHEUS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under PROMETHEUS_HOME or ``/tmp/prometheus-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    prometheus_home = get_prometheus_home()
    try:
        path.resolve().relative_to(prometheus_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/prometheus-* explicitly
    parts = path.parts
    return bool(len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("prometheus-"))
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside PROMETHEUS_HOME)")
        return False
...
def quick() -> dict[str, Any]:
    ...
    for item in tracked:
        ...
        should_delete = (
            cat == "test" or (cat == "temp" and age > 7) or (cat == "cron-output" and age > 14)
        )
    ...
    # Remove empty dirs under PROMETHEUS_HOME (but leave PROMETHEUS_HOME itself and
    ...
    empty_removed = 0
    try:
        for dirpath in sorted(prometheus_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == prometheus_home:
                continue
            try:
                rel_parts = dirpath.relative_to(prometheus_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> str | None:
    ...
    prometheus_home = get_prometheus_home()
    try:
        rel = path.resolve().relative_to(prometheus_home)
        ...
        # Path isn't under PROMETHEUS_HOME (e.g. /tmp/prometheus-*) — fall through.
        pass
