...
Auto-tracks and cleans up ephemeral files created during Prometheus Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$PROMETHEUS_HOME` and `/tmp/prometheus-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `PROMETHEUS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under PROMETHEUS_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `PROMETHEUS_HOME` or `/tmp/prometheus-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$PROMETHEUS_HOME/disk-cleanup/` is itself excluded
- `$PROMETHEUS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
