"""disk-cleanup plugin — auto-cleanup of ephemeral Prometheus session files.
...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``PROMETHEUS_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$PROMETHEUS_HOME/disk-cleanup/cleanup.log``.
...
def _extract_paths_from_terminal(args: dict[str, Any], result: str) -> set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command — catches `touch /tmp/prometheus-x/test_foo.py`
        try:
...
All operations are scoped to PROMETHEUS_HOME and /tmp/prometheus-*.
...
def _handle_slash(raw_args: str) -> str | None:
    ...
    if sub == "dry-run":
        ...
        lines.append(f"  Needs prompt: {len(prompt)} files ({dg.fmt_size(prompt_size)})")
        for item in prompt:
            lines.append(f"    [{item['category']}] {item['path']}")
    ...
    if sub == "deep":
        ...
        if prompt_items:
            ...
            lines.append(f"\n{len(prompt_items)} item(s) need confirmation ({dg.fmt_size(size)}):")
    ...
    if sub == "track":
        ...
        return f"Not tracked (already present, missing, or outside PROMETHEUS_HOME): {path_arg}"
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral Prometheus session files.",
    )
