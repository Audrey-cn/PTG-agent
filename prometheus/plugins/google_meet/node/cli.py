"""`prometheus meet node ...` subcommand tree.

Wired into the existing ``prometheus meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``prometheus meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="prometheus-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``prometheus meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             prometheus meet node approve <name> ws://<host>:{args.port} {token}")
        try:
