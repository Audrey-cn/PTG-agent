...
def _resolve_node_client(node: str | None):
    ...
    if entry is None:
        raise RuntimeError(
            ...
            "run `prometheus meet node approve <name> <url> <token>` first"
        )
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "guest_name": {
                "type": "string",
                "description": (
                    "Display name to use when joining as guest. Defaults to 'Prometheus Agent'."
                ),
            },
            "duration": {
                "type": "string",
            ...
            "headed": {
                "type": "boolean",
                "description": (
                    "Run Chromium headed instead of headless (debug only). Default false."
                ),
            },
            "node": {
                "type": "string",
                "description": (
                    ...
                    "approved via `prometheus meet node approve`."
                ),
            },
        },
        "required": ["url"],
    ...
    if client is not None:
        ...
        try:
            res = client.start_bot(
                url=url,
                guest_name=str(args.get("guest_name") or "Prometheus Agent"),
