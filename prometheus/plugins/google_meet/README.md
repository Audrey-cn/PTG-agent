...
Let the prometheus agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where prometheus runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `prometheus meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `prometheus meet` CLI |
...
| `cli.py` | `prometheus meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$PROMETHEUS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `prometheus meet node {run,list,approve,remove,status,ping}` |
...
prometheus meet install                                      # pip + Chromium
prometheus meet setup                                        # preflight
prometheus meet auth                                         # optional
prometheus meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
prometheus meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.prometheus/.env
prometheus meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
prometheus meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
prometheus meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.prometheus/.env
prometheus meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, prometheus will **not** switch your system audio input automatically — the
...
prometheus plugins enable google_meet
prometheus meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
prometheus meet node ping my-mac
