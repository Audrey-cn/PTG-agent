...
``_check_spotify_available()`` — when the user has not run ``prometheus auth
spotify``, the tools remain registered (so they appear in ``prometheus
...
The Spotify auth flow (``prometheus auth spotify``), CLI plumbing, and docs
