"""Native Spotify tools for Prometheus (registered via plugins/spotify)."""
...
from prometheus.cli.auth import get_auth_status
from prometheus.tools.registry import tool_error, tool_result
...
def _handle_spotify_search(args: dict, **kw) -> str:
    ...
    search_types = [
        ...
        if value.lower() in {"album", "artist", "playlist", "track", "show", "episode", "audiobook"}
    ]
    if not search_types:
        return tool_error(
