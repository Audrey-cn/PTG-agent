...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "prometheus" else ""
...
def cmd_sync(args) -> None:
    ...
    try:
        from prometheus.cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'prometheus honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'prometheus honcho setup' first.\n")
        return
    ...
    for p in profiles:
        ...
        if clone_honcho_for_profile(p.name):
            print(f"  + {p.name} -> prometheus.{p.name}")
            created += 1
        else:
            skipped += 1
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Prometheus profile."""
...
def _local_config_path() -> Path:
    ...
    return get_prometheus_home() / "honcho.json"
...
def _resolve_api_key(cfg: dict) -> str:
    ...
    if not key:
        base_url = (
            cfg.get("baseUrl") or cfg.get("base_url") or os.environ.get("HONCHO_BASE_URL", "")
        )
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Prometheus persistent cross-session memory.")
    ...
    prometheus_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'prometheus honcho setup' again once you have a key.\n")
            return

    current_peer = prometheus_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        prometheus_host["peerName"] = new_peer

    current_ai = prometheus_host.get("aiPeer") or cfg.get("aiPeer", "prometheus")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        prometheus_host["aiPeer"] = new_ai

    current_workspace = prometheus_host.get("workspace") or cfg.get("workspace", "prometheus")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        prometheus_host["workspace"] = new_workspace

    current_obs = prometheus_host.get("observationMode") or cfg.get(
    ...
    if new_obs in ("unified", "directional"):
        prometheus_host["observationMode"] = new_obs
    else:
        prometheus_host["observationMode"] = "directional"

    current_wf = str(prometheus_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        prometheus_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        prometheus_host["writeFrequency"] = (
            new_wf if new_wf in ("async", "turn", "session") else "async"
        )

    _raw_recall = prometheus_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        prometheus_host["recallMode"] = new_recall

    current_ctx_tokens = prometheus_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in ("none", "uncapped", "no limit"):
        prometheus_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                prometheus_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass

    current_dialectic = str(
        prometheus_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2"
    )
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            prometheus_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        prometheus_host["dialecticCadence"] = 2

    current_reasoning = (
        prometheus_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in ("minimal", "low", "medium", "high", "max"):
        prometheus_host["dialecticReasoningLevel"] = new_reasoning
    else:
        prometheus_host["dialecticReasoningLevel"] = "low"

    current_strat = prometheus_host.get("sessionStrategy") or cfg.get(
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        prometheus_host["sessionStrategy"] = new_strat

    prometheus_host["enabled"] = True
    prometheus_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        print("  Run: prometheus config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    prometheus honcho status     -- show full config")
    print("    prometheus honcho mode       -- change recall/observation mode")
    print("    prometheus honcho tokens     -- tune context and dialectic budgets")
    print("    prometheus honcho peer       -- update peer names")
    print("    prometheus honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Prometheus profile name."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: prometheus honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'prometheus honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: prometheus honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        hosts = cfg.get("hosts", {})
        prometheus = hosts.get(_host_key(), {})
        user = prometheus.get("peerName") or cfg.get("peerName") or "(not set)"
        ai = prometheus.get("aiPeer") or cfg.get("aiPeer") or _host_key()
        lvl = (
            prometheus.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        )
        max_chars = prometheus.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Prometheus' identity in Honcho. Seed with 'prometheus honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: prometheus honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(
            "\n  Set with: prometheus honcho strategy [per-session|per-directory|per-repo|global]\n"
        )
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = (
            prometheus.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        )
        d_chars = prometheus.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = (
            prometheus.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        )
        print("\nHoncho budgets\n" + "─" * 40)
        ...
        print("    AI-to-AI inference. Prometheus asks Honcho's AI peer a question")
        ...
        print("\n  Set with: prometheus honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'prometheus honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'prometheus honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    prometheus honcho identity --show        — show both peer representations")
        print(
            "    prometheus honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n"
        )
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: native memory → Prometheus + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: native memory → Prometheus\n" + "─" * 50)
    print()
    print("  Prometheus native memory stores context in local markdown files")
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Prometheus persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  prometheus honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'prometheus honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'prometheus honcho setup' when ready, then re-run this walkthrough.")

    print()
    print("Step 2  Detected native memory files")
    print()
    if user_files or agent_files:
        ...
        print("  No native memory files found in cwd or ~/.openclaw/.")
        print("  If your files are elsewhere, copy them here before continuing,")
        print("  or seed them manually:  prometheus honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'prometheus'")
        print("  with Honcho configured and no prior session history.")
        print("  (Prometheus calls migrate_memory_files() on first session init.)")
        print()
        if has_key:
            ...
            print("  Run 'prometheus honcho setup' first, then re-run this step.")
    else:
    ...
    print("  agent's character, capabilities, and behavioral rules.")
    print()
    print("  In Prometheus, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'prometheus honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    prometheus honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  prometheus honcho identity <path/to/SOUL.md>")

    print()
    print("Step 5  What changes vs. native memory")
    print()
    print("  Storage")
    print("    Native: markdown files on disk, searched via QMD at prompt-build time.")
    print("    Prometheus + Honcho: cloud-backed Honcho peers. Files can stay on disk as source")
    print("              of truth; Honcho holds the live representation.")
    print()
    print("  Context injection")
    print("    Native: file excerpts injected synchronously before each LLM call.")
    print("    Prometheus + Honcho: Honcho context fetched async at turn end, injected next turn.")
    print("              First turn has no Honcho context; subsequent turns are loaded.")
    print()
    print("  Memory growth")
    print("    Native: you edit files manually to update memory.")
    print("    Prometheus + Honcho: Honcho observes every message and updates representations")
    ...
    print("    Native: no persistent session concept — files are global.")
    print("    Prometheus + Honcho: per-session by default — each run gets its own session")
    print("              Map a custom name:  prometheus honcho map <session-name>")

    print()
    print("Step 6  Next steps")
    print()
    if not has_key:
        print("  1. prometheus honcho setup              — configure API key (required)")
        print("  2. prometheus honcho migrate            — re-run this walkthrough")
    else:
        print("  1. prometheus honcho status             — verify Honcho connection")
        print("  2. prometheus                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. prometheus honcho identity --show    — verify AI peer representation")
        print("  4. prometheus honcho tokens             — tune context and dialectic budgets")
        print("  5. prometheus honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'prometheus memory setup'...\n")
        from prometheus.cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``prometheus honcho`` argparse subcommand tree."""
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to prometheus memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide to Prometheus Honcho",
    )
