...
from prometheus.agent.memory_manager import sanitize_context
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._dialectic_cadence = 1
        self._dialectic_depth = 1
        self._dialectic_depth_levels: list[str] | None = None
        self._reasoning_heuristic: bool = True
        self._reasoning_level_cap: str = "high"
    ...
    def save_config(self, values, prometheus_home):
        """Write config to $PROMETHEUS_HOME/honcho.json (Honcho SDK native format)."""
        import json
        from pathlib import Path

        config_path = Path(prometheus_home) / "honcho.json"
        existing = {}
    ...
    def post_setup(self, prometheus_home: str, config: dict) -> None:
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_prometheus_home() / "memories")
                self._manager.migrate_memory_files(self._session_key, mem_dir)
                logger.debug(
                    "Honcho memory file migration attempted for new session: %s", self._session_key
                )
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "prometheus-default",
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        if dialectic_result and fired_at >= 0 and (self._turn_count - fired_at) > stale_limit:
            logger.debug(
                "Honcho pending dialectic discarded as stale: fired_at=%d, turn=%d, limit=%d",
                fired_at,
                self._turn_count,
                stale_limit,
            )
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        self._prefetch_thread_started_at = time.monotonic()
        self._prefetch_thread = threading.Thread(target=_run, daemon=True, name="honcho-prefetch")
        self._prefetch_thread.start()
    ...
    def _thread_is_live(self) -> bool:
        """Thread-alive guard that treats threads older than the stale threshold as dead."""
        ...
        if age > timeout * self._STALE_THREAD_MULTIPLIER:
            logger.debug(
                "Honcho prefetch thread age %.1fs exceeds stale threshold %.1fs — treating as dead",
                age,
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        self._sync_thread = threading.Thread(target=_sync, daemon=True, name="honcho-sync")
        ...
        try:
            if tool_name == "honcho_profile":
                ...
                if card_update:
                    ...
                    return json.dumps(
                        {"result": f"Peer card updated ({len(result)} facts).", "card": result}
                    )
