...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/prometheus>
...
```bash
prometheus honcho setup    # full interactive wizard (cloud or local)
prometheus memory setup    # generic picker, also works
```

Or manually:
```bash
prometheus config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.prometheus/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$PROMETHEUS_HOME/honcho.json` | Profile-local (isolated Prometheus instances) |
| 2 | `~/.prometheus/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Prometheus profile: `prometheus` (default) or `prometheus.<profile>`.
...
| 4 | `per-session` strategy | Prometheus session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`prometheus-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`prometheus`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-prometheus-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening prometheus in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Prometheus session ID (timestamp + hex). Every `prometheus` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Prometheus profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`prometheus`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Prometheus profile: `prometheus` (default) or `prometheus.<profile>` (e.g. `prometheus -p coder` → host key `prometheus.coder`).
...
| `PROMETHEUS_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `prometheus honcho setup` | Full interactive setup wizard |
| `prometheus honcho status` | Show resolved config for active profile |
| `prometheus honcho enable` / `disable` | Toggle Honcho for active profile |
| `prometheus honcho mode <mode>` | Change recall or observation mode |
| `prometheus honcho peer --user <name>` | Update user peer name |
| `prometheus honcho peer --ai <name>` | Update AI peer name |
| `prometheus honcho tokens --context <N>` | Set context token budget |
| `prometheus honcho tokens --dialectic <N>` | Set dialectic max chars |
| `prometheus honcho map <name>` | Map current directory to a session name |
| `prometheus honcho sync` | Create host blocks for all Prometheus profiles |
