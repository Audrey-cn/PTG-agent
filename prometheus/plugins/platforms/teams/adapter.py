"""
Microsoft Teams platform adapter for Prometheus Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Prometheus"}),
            )
    ...
    async def _on_card_action(
        ...
        from prometheus.tools.approval import has_blocking_approval, resolve_gateway_approval

        action = ctx.activity.value.action
        data = action.data or {}
        prometheus_action = data.get("prometheus_action", "")
        session_key = data.get("session_key", "")

        if not prometheus_action or not session_key:
        ...
        choice = choice_map.get(prometheus_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        if not has_blocking_approval(session_key):
            return InvokeResponse(
                ...
                body=AdaptiveCardActionCardResponse(
                    ...
                    .with_version("1.4")
                    .with_body(
                        [TextBlock(text="⚠️ Approval already resolved or expired.", wrap=True)]
                    )
                ),
            )
        ...
        card = (
            ...
            .with_actions(
                [
                    ExecuteAction(
                        title="Allow Once",
                        verb="prometheus_approve",
                        data={**btn_data_base, "prometheus_action": "approve_once"},
                        style="positive",
                    ),
                    ExecuteAction(
                        title="Allow Session",
                        verb="prometheus_approve",
                        data={**btn_data_base, "prometheus_action": "approve_session"},
                    ),
                    ExecuteAction(
                        title="Always Allow",
                        verb="prometheus_approve",
                        data={**btn_data_base, "prometheus_action": "approve_always"},
                    ),
                    ExecuteAction(
                        title="Deny",
                        verb="prometheus_approve",
                        data={**btn_data_base, "prometheus_action": "deny"},
                        style="destructive",
                    ),
                ]
            )
...
def interactive_setup() -> None:
    ...
    print_info('  teams app create --name "Prometheus" --endpoint "https://<tunnel>/api/messages"')
    print()
    ...
    client_secret = prompt(
        "Client secret", default=get_env_value("TEAMS_CLIENT_SECRET") or "", password=True
    )
    ...
    print()
    print_success("Teams configuration saved to ~/.prometheus/.env")
    print_info("Install the app in Teams:  teams app install --id <teamsAppId>")
    print_info("Restart the gateway:       prometheus gateway restart")
