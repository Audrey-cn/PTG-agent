...
"""
IRC Platform Adapter for Prometheus Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Prometheus agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "prometheus-bot")
    ...
    async def connect(self) -> bool:
        ...
        try:
            from prometheus.gateway.status import acquire_scoped_lock, release_scoped_lock

            lock_key = f"{self.server}:{self.nickname}"
            if not acquire_scoped_lock("irc", lock_key):
                logger.error(
                    "IRC: %s@%s already in use by another profile", self.nickname, self.server
                )
                self._set_fatal_error(
                    "lock_conflict", "IRC identity in use by another profile", retryable=False
                )
                return False
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Prometheus Agent")
        ...
        try:
            ...
            self._set_fatal_error(
                "registration_timeout", "IRC server did not send RPL_WELCOME", retryable=True
            )
            return False
    ...
    async def disconnect(self) -> None:
        ...
        if getattr(self, "_lock_key", None):
            try:
                from prometheus.gateway.status import release_scoped_lock
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Prometheus Agent shutting down")
    ...
    async def _receive_loop(self) -> None:
        ...
        try:
            ...
            if self.is_connected:
                ...
                self._set_fatal_error(
                    "connection_lost", "IRC connection closed unexpectedly", retryable=True
                )
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: prometheus_, prometheus_1, prometheus_2...
...
def check_requirements() -> bool:
    ...
    # prometheus setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `prometheus gateway setup` flow for the IRC platform.

    Lazy-imports ``prometheus.cli.setup`` helpers so the plugin stays importable
    ...
    print_info(
        "Connect Prometheus to an IRC network. Uses Python stdlib — no extra packages needed."
    )
    ...
    nickname = prompt(
        "Bot nickname (e.g. prometheus-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #prometheus — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.prometheus/.env")
    print_info("Restart the gateway for changes to take effect: prometheus gateway restart")
...
def register(ctx):
    """Plugin entry point — called by the Prometheus plugin system."""
