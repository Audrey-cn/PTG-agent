"""langfuse — Prometheus plugin for Langfuse observability.

Traces Prometheus conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Prometheus plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``prometheus plugins enable
observability/langfuse`` or ``prometheus tools → Langfuse Observability``). At
...
def _get_langfuse() -> Langfuse | None:
    ...
    Activation of this plugin is controlled by the Prometheus plugin system —
...
def _safe_value(
    value: Any, *, max_chars: int | None = None, depth: int = 0, parse_json_strings: bool = False
) -> Any:
    max_chars = (
        max_chars
    ...
    if isinstance(value, dict):
        ...
        if normalized is not value:
            return _safe_value(
                normalized, max_chars=max_chars, depth=depth, parse_json_strings=parse_json_strings
            )
        return {
            str(k): _safe_value(
                v, max_chars=max_chars, depth=depth + 1, parse_json_strings=parse_json_strings
            )
    ...
    if hasattr(value, "__dict__"):
        return _safe_value(
            vars(value), max_chars=max_chars, depth=depth + 1, parse_json_strings=parse_json_strings
        )
...
def _usage_and_cost(
    ...
    try:
        from prometheus.agent.usage_pricing import estimate_usage_cost, normalize_usage
        ...
        if cost.amount_usd is not None:
            ...
            try:
                ...
                from prometheus.agent.usage_pricing import get_pricing_entry
                ...
                if entry:
                    if entry.input_cost_per_million is not None and canonical.input_tokens:
                        cost_details["input"] = float(
                            Decimal(canonical.input_tokens) * entry.input_cost_per_million / _ONE_M
                        )
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Prometheus turn",
                tags=["prometheus", "langfuse"],
            ):
                root_ctx = client.start_as_current_observation(
                    trace_context=trace_ctx,
                    name="Prometheus turn",
                    as_type="chain",
                    input=trace_input,
                    metadata=metadata,
                    end_on_exit=False,
...
def on_pre_llm_call(
    ...
    **_: Any,
    ...
    # Older Prometheus branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Prometheus also has a turn-scoped
    ...
    # messages is a list only for legacy Prometheus branches that fired
    # pre_llm_call with API messages directly. Current Prometheus fires
        ...
        try:
            ...
            if entry:
                ...
                if entry.cache_read_cost_per_million is not None and _cache_read:
                    cost_details["cache_read_input_tokens"] = float(
                        Decimal(_cache_read) * entry.cache_read_cost_per_million / _ONE_M
                    )
                if entry.cache_write_cost_per_million is not None and _cache_write:
                    cost_details["cache_creation_input_tokens"] = float(
                        Decimal(_cache_write) * entry.cache_write_cost_per_million / _ONE_M
                    )
            else:
...
def register(ctx) -> None:
    ...
    # Prometheus versions.  pre_api_request / post_api_request fire per API
