...
``prometheus.cli.kanban_db`` or a direct SQL query. Writes use the same code
...
bridge in ``prometheus.cli/web_server.py``. If you run the dashboard with
...
from prometheus.cli import kanban_db
...
def _check_ws_token(provided: str | None) -> bool:
    ...
    if not provided:
        return False
    try:
        from prometheus.cli import web_server as _ws
    except Exception:
...
def create_task(payload: CreateTaskBody):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from prometheus.cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``prometheus kanban
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.prometheus/config.yaml.
...
def get_assignees():
    ...
    Returns the union of ``~/.prometheus/profiles/*`` on disk and every
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in prometheus.cli/web_server.py.
