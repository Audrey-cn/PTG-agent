...
Output is saved as PNG under ``$PROMETHEUS_HOME/cache/images/``.
"""
...
from prometheus.agent.image_gen_provider import (
...
def _read_codex_access_token() -> str | None:
    ...
    """
    try:
        from prometheus.agent.auxiliary_client import _read_codex_access_token as _reader
...
def _build_codex_client():
    ...
    try:
        ...
        from prometheus.agent.auxiliary_client import _codex_cloudflare_headers
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `prometheus auth codex` (or `prometheus setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`prometheus auth codex` (or `prometheus setup` → Codex) to sign in."
                ),
