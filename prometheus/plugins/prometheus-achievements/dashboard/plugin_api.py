"""Prometheus Achievements dashboard plugin backend.

Mounted at /api/plugins/prometheus-achievements/ by Prometheus dashboard.
"""
...
SUCCESS_RE = re.compile(
    r"\b(success|passed|built|compiled|done|exit_code[\"']?\s*[:=]\s*0|verified|ok)\b", re.I
)
FILE_RE = re.compile(
...
def tiers(values: list[int]) -> list[dict[str, Any]]:
    return [
        ...
        for name, threshold in zip(TIER_NAMES, values, strict=False)
    ]
...
ACHIEVEMENTS: list[dict[str, Any]] = [
    # Agent Autonomy — mostly best-session feats
    {
        "id": "let_him_cook",
        "name": "Let Him Cook",
        "description": "Let Prometheus run a serious autonomous tool chain in one session.",
    ...
    },
    {
        ...
        "description": "Accumulate a lifetime avalanche of Prometheus tool calls across sessions.",
        "category": "Agent Autonomy",
        "kind": "lifetime",
        "icon": "avalanche",
    ...
    },
    {
        ...
        "description": "Use a wide spread of distinct Prometheus tools in one session.",
        "category": "Agent Autonomy",
        "kind": "best_session",
        "icon": "nodes",
    ...
    },
    {
        ...
        "kind": "multi_condition",
        "icon": "package_skull",
        "requirements": [req("install_error_events", 25), req("install_success_events", 10)],
    },
    {
        ...
        "kind": "multi_condition",
        "icon": "restart",
        "requirements": [req("restart_after_error_events", 50), req("total_errors", 4000)],
    },
    {
    ...
    },
    {
        ...
        "icon": "needle",
        "secret": True,
        "requirements": [req("tiny_patch_after_errors_events", 5), req("total_errors", 4000)],
    },
    # Prometheus Native
    {
        "id": "skillsmith",
        "name": "Skillsmith",
        "description": "Work with Prometheus skills enough to leave fingerprints.",
        "category": "Prometheus Native",
        "kind": "lifetime",
        "icon": "hammer_scroll",
    ...
    },
    {
        ...
        "description": "Live through gateway-connected Prometheus workflows.",
        "category": "Prometheus Native",
        "kind": "lifetime",
        "icon": "antenna",
    ...
    },
    {
        ...
        "description": "Use a real spread of distinct model names across Prometheus history.",
        "category": "Model Lore",
        "kind": "lifetime",
        "icon": "prism",
    ...
    },
    {
        ...
        "description": "Use models from multiple providers across Prometheus history.",
        "category": "Model Lore",
        "kind": "lifetime",
        "icon": "swap",
    ...
    },
    {
        ...
        "description": "Actually chat with local/open-weight models through Prometheus session metadata.",
        "category": "Model Lore",
        "kind": "lifetime",
        "icon": "terminal",
    ...
    # Workflow Intelligence
    {
        ...
        "description": "Navigate Prometheus toolsets deliberately instead of treating tools as a blur.",
        "category": "Prometheus Native",
        "kind": "lifetime",
        "icon": "compass",
    ...
    # Lifestyle
    {
        ...
        "description": "Accumulate a serious number of Prometheus sessions.",
        "category": "Lifestyle",
        "kind": "lifetime",
        "icon": "marathon",
    ...
    },
    {
        ...
        "description": "Run Prometheus on weekends enough times to make it a lifestyle.",
        "category": "Lifestyle",
        "kind": "lifetime",
        "icon": "calendar",
...
def state_path() -> Path:
    return Path.home() / ".prometheus" / "plugins" / "prometheus-achievements" / "state.json"


def snapshot_path() -> Path:
    return (
        Path.home() / ".prometheus" / "plugins" / "prometheus-achievements" / "scan_snapshot.json"
    )


def checkpoint_path() -> Path:
    return (
        Path.home() / ".prometheus" / "plugins" / "prometheus-achievements" / "scan_checkpoint.json"
    )
...
def analyze_messages(session_id: str, title: str, messages: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "log_read_events": len(
            re.findall(r"gateway\.log|errors\.log|agent\.log|/api/logs|\blogs\b", full_text, re.I)
        ),
        "permission_denied_events": len(
            re.findall(r"permission denied|eacces|operation not permitted", full_text, re.I)
        ),
        ...
        "yaml_error_events": len(re.findall(r"yaml|yml|colon|parse error", full_text, re.I))
        ...
        "git_events": len(
            re.findall(r"\bgit\s+(commit|push|merge|rebase|status|diff)", full_text, re.I)
        ),
        "tiny_patch_after_errors_events": 1
        if error_count >= 5 and re.search(r"one character|single character|typo", full_text, re.I)
        else 0,
        ...
        "plugin_events": len(
            re.findall(r"plugin|dashboard-plugins|__PROMETHEUS_PLUGIN|manifest\.json", full_text, re.I)
        ),
        ...
        "release_events": len(
            re.findall(
                r"\bgit\s+tag|release|version bump|changelog|publish|pushed? tag", full_text, re.I
            )
        ),
...
METRIC_LABELS = {
    ...
    "max_distinct_tools_in_session": "distinct Prometheus tools used in one session",
    ...
    "skill_events": "Prometheus skill mentions or tool use",
    ...
    "total_tool_calls": "lifetime Prometheus tool calls",
    ...
    "local_model_chat_sessions": "Prometheus sessions whose model metadata is local/open-weight",
    ...
    "session_count": "Prometheus sessions",
...
def criteria_for(definition: dict[str, Any]) -> str:
    if definition.get("secret") and definition.get("state") == "secret":
        return "Secret: exact requirement hidden until Prometheus sees the first matching signal. Keep using Prometheus across debugging, tools, memory, skills, plugins, and model workflows to reveal it."
    secret_prefix = ""
    if "threshold_metric" in definition:
        tiers_list = sorted(definition.get("tiers", []), key=lambda t: t["threshold"])
        if not tiers_list:
            return secret_prefix + "Requirement: use Prometheus in the matching workflow."
    ...
    return secret_prefix + "Requirement: complete the matching Prometheus behavior."
...
def display_achievement(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if clean.get("state") == "secret":
        return {
            **clean,
            "name": "???",
            "description": "Secret achievement: hidden until Prometheus detects the first relevant behavior in your session history.",
...
def scan_sessions(
    ...
    """Scan Prometheus sessions and build per-session achievement stats.
    ...
    previous_sessions = (
        checkpoint.get("sessions") if isinstance(checkpoint.get("sessions"), dict) else {}
    )
    reused = 0
    rescanned = 0
        ...
        for idx, meta in enumerate(sessions_meta, start=1):
            ...
            if isinstance(cached_stats, dict) and cached_fp == fp:
                ...
                stats = analyze_messages(
                    sid, meta.get("title") or meta.get("preview") or "Untitled", messages
                )
                rescanned += 1

            stats["session_id"] = sid
            stats["title"] = (
                meta.get("title") or meta.get("preview") or stats.get("title") or "Untitled"
            )
...
def _compute_from_scan(scan: dict[str, Any], *, is_partial: bool = False) -> dict[str, Any]:
    ...
    for definition in ACHIEVEMENTS:
        ...
        if result["unlocked"]:
            ...
            item["evidence"] = unlocks.get(unlock_id, {}).get("evidence") or evidence_for(
                definition, scan.get("sessions", [])
            )
...
def _start_background_scan() -> None:
    ...
    with _BACKGROUND_SCAN_LOCK:
        ...
        thread = threading.Thread(
            ...
            name="prometheus-achievements-scan",
            daemon=True,
        )
