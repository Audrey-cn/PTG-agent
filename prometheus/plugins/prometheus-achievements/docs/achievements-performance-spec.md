# Prometheus Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: prometheus-achievements plugin
...
### `GET /api/plugins/prometheus-achievements/achievements`
...
### `POST /api/plugins/prometheus-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/prometheus-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/prometheus-achievements/overview`
...
- Persist plugin data under:
  - `~/.prometheus/plugins/prometheus-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.prometheus/plugins/prometheus-achievements/scan_checkpoint.json`
...
- `~/.prometheus/plugins/prometheus-achievements/`
