...
1. **Theme** — copy the theme YAML into your Prometheus home:

   ```
   cp theme/strike-freedom.yaml ~/.prometheus/dashboard-themes/
   ```
...
2. **Plugin** — the `dashboard/` directory gets auto-discovered because
   ...
   whole plugin directory into `~/.prometheus/plugins/`:

   ```
   cp -r . ~/.prometheus/plugins/strike-freedom-cockpit
   ```
