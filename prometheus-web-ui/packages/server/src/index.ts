...
import { setupTerminalWebSocket } from './routes/prometheus/terminal'
import { startVersionCheck } from './routes/health'
import { registerRoutes } from './routes'
import { setGroupChatServer } from './routes/prometheus/group-chat'
import { setChatRunServer } from './routes/prometheus/chat-run'
import { GroupChatServer } from './services/prometheus/group-chat'
import { ChatRunSocket } from './services/prometheus/chat-run-socket'
...
export async function bootstrap() {
  console.log(`prometheus-web-ui v${APP_VERSION} starting...`)
  ...
  const { initAllStores } = await import('./db/prometheus/init')
  initAllStores()
  console.log('[bootstrap] all stores initialized')

  // Sync Prometheus sessions from all profiles (only if local DB is empty)
  const { syncAllPrometheusSessionsOnStartup } = await import('./services/prometheus/session-sync')
  await syncAllPrometheusSessionsOnStartup()
  console.log('[bootstrap] Prometheus session sync completed')
  ...
  const { SessionDeleter } = await import('./services/prometheus/session-deleter')
  ...
  server.on('upgrade', (req: any, socket: any) => {
    ...
    if (url.pathname !== '/api/prometheus/terminal' && !url.pathname.startsWith('/socket.io/')) {
      socket.destroy()
    }
  })
  ...
  server.on('listening', () => {
    ...
    console.log(`Log: ~/.prometheus-web-ui/logs/server.log`)
