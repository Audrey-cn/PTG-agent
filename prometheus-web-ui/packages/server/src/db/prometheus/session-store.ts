/**
 * Self-built session database — completely replaces Prometheus CLI dependency.
...
// Tables are created automatically on bootstrap via initAllPrometheusTables()
...
function mapSessionRow(row: Record<string, unknown>): PrometheusSessionRow {
...
export function getSession(id: string): PrometheusSessionRow | null {
...
export function updateSession(id: string, data: Partial<Omit<PrometheusSessionRow, 'id' | 'profile'>>): void {
...
export function listSessions(profile: string, source?: string, limit = 2000): PrometheusSessionRow[] {
  if (!isSqliteAvailable()) return []
  const db = getDb()!
...
export function searchSessions(profile: string, query: string, limit = 20): PrometheusSessionSearchRow[] {
  if (!isSqliteAvailable()) return []
...
export function getSessionDetail(id: string): PrometheusSessionDetailRow | null {
  if (!isSqliteAvailable()) return null
  const db = getDb()!
