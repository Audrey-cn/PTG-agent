import { getActiveProfileDir, getProfileDir } from '../../services/prometheus/prometheus-profile'
...
export interface PrometheusSessionRow {
  id: string
  source: string
  user_id: string | null
  model: string
...
export interface PrometheusSessionSearchRow extends PrometheusSessionRow {
  matched_message_id: number | null
  snippet: string
  rank: number
}

export interface PrometheusMessageRow {
  id: number | string
  session_id: string
  role: string
  content: string
...
export interface PrometheusSessionDetailRow extends PrometheusSessionRow {
  messages: PrometheusMessageRow[]
  thread_session_count: number
}

interface PrometheusSessionInternalRow extends PrometheusSessionRow {
...
function mapRow(row: Record<string, unknown>): PrometheusSessionRow {
...
function mapSearchRow(row: Record<string, unknown>): PrometheusSessionSearchRow {
  return {
    ...mapRow(row),
...
function mapInternalSessionRow(row: Record<string, unknown>): PrometheusSessionInternalRow {
  return {
    ...mapRow(row),
...
function mapMessageRow(row: Record<string, unknown>): PrometheusMessageRow {
...
function isCompressionEnded(session: PrometheusSessionInternalRow | undefined): boolean {
  return !!session && COMPRESSION_END_REASONS.has(String(session.end_reason || ''))
}

function isCompressionContinuation(parent: PrometheusSessionInternalRow | undefined, child: PrometheusSessionInternalRow | undefined): boolean {
...
function latestSessionInChain(chain: PrometheusSessionInternalRow[]): PrometheusSessionInternalRow {
...
function projectSessionSummary(root: PrometheusSessionInternalRow, chain: PrometheusSessionInternalRow[]): PrometheusSessionRow {
...
interface SessionIndex {
  byId: Map<string, PrometheusSessionInternalRow>
...
function getLatestContinuationChild(
  parent: PrometheusSessionInternalRow,
  idx: SessionIndex,
): PrometheusSessionInternalRow | null {
  ...
  const candidates = (idx.childrenByParent.get(parent.id) || [])
    ...
    .filter((c): c is PrometheusSessionInternalRow => !!c)
...
function collectCompressionPath(
  session: PrometheusSessionInternalRow,
  idx: SessionIndex,
): PrometheusSessionInternalRow[] {
  const reversed: PrometheusSessionInternalRow[] = [session]
  const seen = new Set<string>()
  let current: PrometheusSessionInternalRow | null = session
...
function extendCompressionChain(
  chain: PrometheusSessionInternalRow[],
  idx: SessionIndex,
): PrometheusSessionInternalRow[] {
  const result = [...chain]
  const seen = new Set(result.map(s => s.id))
  let current: PrometheusSessionInternalRow | null = result[result.length - 1] || null
...
function collectSessionChain(
  root: PrometheusSessionInternalRow,
  idx: SessionIndex,
): PrometheusSessionInternalRow[] {
...
function projectSearchRow(
  ...
  idx: SessionIndex,
  source?: string,
): PrometheusSessionSearchRow | null {
 ...
 * without chain traversal. Used by syncFromPrometheus for ephemeral sessions.
 */
export async function getSessionMessagesFromDb(sessionId: string): Promise<{
  messages: PrometheusMessageRow[]
  session: PrometheusSessionRow | null
} | null> {
...
export async function getSessionDetailFromDb(sessionId: string): Promise<PrometheusSessionDetailRow | null> {
  const db = await openSessionDb()
  try {
...
export async function getSessionDetailFromDbWithProfile(sessionId: string, profile: string): Promise<PrometheusSessionDetailRow | null> {
  const { DatabaseSync } = await import('node:sqlite')
...
export interface PrometheusUsageStats extends LocalUsageStats {
  cost: number
...
): Promise<PrometheusUsageStats> {
  const empty: PrometheusUsageStats = {
    input_tokens: 0,
    output_tokens: 0,
    cache_read_tokens: 0,
...
export async function listSessionSummaries(source?: string, limit = 2000, profile?: string): Promise<PrometheusSessionRow[]> {
  if (!SQLITE_AVAILABLE) {
    throw new Error(`node:sqlite requires Node >= 22.5, current: ${process.versions.node}`)
  }
...
export async function searchSessionSummaries(
  ...
  source?: string,
  limit = 20,
): Promise<PrometheusSessionSearchRow[]> {
  if (!SQLITE_AVAILABLE) {
    throw new Error(`node:sqlite requires Node >= 22.5, current: ${process.versions.node}`)
  }
  ...
  try {
    ...
    const idx = loadAllSessions(db)
    const merged = new Map<string, PrometheusSessionSearchRow>()
