...
export async function checkLatestVersion(): Promise<void> {
  try {
    const packageName = PACKAGE_INFO?.name || 'prometheus-web-ui'
...
export async function healthCheck(ctx: any) {
  const raw = await prometheusCli.getVersion()
  const prometheusVersion = raw.split('\n')[0].replace('Prometheus Agent ', '') || ''
  let gatewayOk = false
  try {
