...
function getCliBin() {
  return join(getNodeBinDir(), process.platform === 'win32' ? 'prometheus-web-ui.cmd' : 'prometheus-web-ui')
}
...
function runUpdateInstall() {
  if (process.platform === 'win32') {
    return execFileSync(getWindowsShell(), ['/d', '/s', '/c', `${quoteForWindowsCommand(getNpmBin())} install -g prometheus-web-ui@latest`], {
      encoding: 'utf-8',
      timeout: 120000,
  ...
  return execFileSync(getNpmBin(), ['install', '-g', 'prometheus-web-ui@latest'], {
    encoding: 'utf-8',
    timeout: 120000,
    stdio: ['pipe', 'pipe', 'pipe'],
  })
