import * as prometheusCli from '../../services/prometheus/prometheus-cli'
import { listConversationSummaries, getConversationDetail } from '../../services/prometheus/conversations'
import { listConversationSummariesFromDb, getConversationDetailFromDb } from '../../db/prometheus/conversations-db'
import { listSessionSummaries, searchSessionSummaries, getUsageStatsFromDb } from '../../db/prometheus/sessions-db'
import {
...
} from '../../db/prometheus/session-store'
import { deleteUsage, getUsage, getUsageBatch, getLocalUsageStats } from '../../db/prometheus/usage-store'
import type { LocalUsageStats, UsageStatsModelRow, UsageStatsDailyRow } from '../../db/prometheus/usage-store'
import { getModelContextLength } from '../../services/prometheus/model-context'
import { getActiveProfileName } from '../../services/prometheus/prometheus-profile'
import { getGroupChatServer } from '../../routes/prometheus/group-chat'
import { logger } from '../../services/logger'
import type { ConversationSummary } from '../../services/prometheus/conversations'
...
export async function listConversations(ctx: any) {
  ...
  try {
    ...
    logger.warn(err, 'Prometheus Conversation DB: summary query failed, falling back to CLI export')
  }
...
export async function getConversationMessages(ctx: any) {
  ...
  try {
    ...
    logger.warn(err, 'Prometheus Conversation DB: detail query failed, falling back to CLI export')
  }
...
export async function list(ctx: any) {
  ...
  try {
    ...
    logger.warn(err, 'Prometheus Session DB: summary query failed, falling back to CLI')
  }

  const sessions = await prometheusCli.listSessions(source, limit)
  ctx.body = { sessions: filterPendingDeletedSessions(sessions) }
}

/**
 * List Prometheus sessions only (exclude api_server source)
 * GET /api/prometheus/sessions/prometheus?source=&limit=
 */
export async function listPrometheusSessions(ctx: any) {
  const source = (ctx.query.source as string) || undefined
  const limit = ctx.query.limit ? parseInt(ctx.query.limit as string, 10) : undefined
...
export async function search(ctx: any) {
  ...
  try {
    ...
    logger.error(err, 'Prometheus Session DB: search failed')
    ctx.status = 500
...
export async function get(ctx: any) {
  ...
  const session = await prometheusCli.getSession(ctx.params.id)
  if (!session) {
    ctx.status = 404
    ctx.body = { error: 'Session not found' }
    return
...
/**
 * Get Prometheus session detail only (exclude api_server source)
 * GET /api/prometheus/sessions/prometheus/:id
 */
export async function getPrometheusSession(ctx: any) {
...
export async function remove(ctx: any) {
  ...
  const sessionId = ctx.params.id
  const ok = await prometheusCli.deleteSession(sessionId)
  if (!ok) {
    ctx.status = 500
    ctx.body = { error: 'Failed to delete session' }
    return
...
export async function rename(ctx: any) {
  ...
  const ok = await prometheusCli.renameSession(ctx.params.id, title.trim())
  if (!ok) {
    ctx.status = 500
    ctx.body = { error: 'Failed to rename session' }
    return
...
export async function setWorkspace(ctx: any) {
  ...
  if (useLocalSessionStore()) {
    const { updateSession, getSession, createSession } = await import('../../db/prometheus/session-store')
    const { getActiveProfileName } = await import('../../services/prometheus/prometheus-profile')
...
export async function usageStats(ctx: any) {
  ...
  // with Prometheus' native state.db analytics for the same period.
  ...
  try {
    prometheus = await getUsageStatsFromDb(days)
  } catch (err) {
    logger.warn(err, 'usageStats: failed to load Prometheus usage analytics from state.db')
  }

  const totalInput = local.input_tokens + prometheus.input_tokens
  const totalOutput = local.output_tokens + prometheus.output_tokens
  const totalCacheRead = local.cache_read_tokens + prometheus.cache_read_tokens
  const totalCacheWrite = local.cache_write_tokens + prometheus.cache_write_tokens
  const totalReasoning = local.reasoning_tokens + prometheus.reasoning_tokens
  const totalSessions = local.sessions + prometheus.sessions

  const modelMap = new Map<string, UsageStatsModelRow>()
  for (const m of [...local.by_model, ...prometheus.by_model].filter(m => m.model)) {
  ...
  for (const d of [...local.by_day, ...prometheus.by_day]) {
  ...
  ctx.body = {
    ...
    total_cost: prometheus.cost,
    total_api_calls: prometheus.total_api_calls,
    period_days: days,
 ...
 * GET /api/prometheus/workspace/folders?path=<relative_path>
...
export async function getConversationMessagesPaginated(ctx: any) {
  ...
  if (useLocalSessionStore()) {
    const { getSessionDetailPaginated } = await import('../../db/prometheus/session-store')
