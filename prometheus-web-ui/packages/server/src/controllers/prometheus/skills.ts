...
import {
  ...
  safeReadFile, extractDescription, listFilesRecursive, getPrometheusDir,
} from '../../services/config-helpers'
import { pinSkill } from '../../services/prometheus/prometheus-cli'

/** Read bundled manifest as a name→hash map from ~/.prometheus/skills/.bundled_manifest */
...
/** Read hub-installed skill names from ~/.prometheus/skills/.hub/lock.json */
...
/** Compute md5 hash of all files in a directory (mirrors Prometheus _dir_hash), with in-memory cache */
...
export async function list(ctx: any) {
  const skillsDir = join(getPrometheusDir(), 'skills')
  try {
...
export async function listFiles(ctx: any) {
  ...
  const skillDir = join(getPrometheusDir(), 'skills', category, skill)
  try {
