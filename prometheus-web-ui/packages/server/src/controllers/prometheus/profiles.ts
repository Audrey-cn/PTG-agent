...
import { SessionDeleter } from '../../services/prometheus/session-deleter'
import { getGatewayManagerInstance } from '../../services/gateway-bootstrap'
import { logger } from '../../services/logger'
import { smartCloneCleanup } from '../../services/prometheus/profile-credentials'

export async function list(ctx: any) {
  try {
    const profiles = await prometheusCli.listProfiles()
    ctx.body = { profiles }
  } catch (err: any) {
    ctx.status = 500
...
export async function create(ctx: any) {
  ...
  try {
    const output = await prometheusCli.createProfile(name, clone)
...
export async function get(ctx: any) {
  try {
    const profile = await prometheusCli.getProfile(ctx.params.name)
    ctx.body = { profile }
  } catch (err: any) {
...
export async function remove(ctx: any) {
  ...
  try {
    ...
    const ok = await prometheusCli.deleteProfile(name)
    if (ok) {
...
export async function rename(ctx: any) {
  ...
  try {
    const ok = await prometheusCli.renameProfile(ctx.params.name, new_name)
    if (ok) {
      ctx.body = { success: true }
    } else {
      ctx.status = 500
...
export async function switchProfile(ctx: any) {
  ...
  try {
    const output = await prometheusCli.useProfile(name)
    await new Promise(r => setTimeout(r, 1000))
    const mgr = getGatewayManagerInstance()
    if (mgr) { mgr.setActiveProfile(name) }
    try {
      const detail = await prometheusCli.getProfile(name)
      logger.debug('Profile detail.path = %s', detail.path)
      if (!existsSync(join(detail.path, 'config.yaml'))) {
        try { await prometheusCli.setupReset() } catch { }
      }
      const profileEnv = join(detail.path, '.env')
      if (!existsSync(profileEnv)) {
        writeFileSync(profileEnv, '# Prometheus Agent Environment Configuration\n', 'utf-8')
...
export async function exportProfile(ctx: any) {
  const { name } = ctx.params
  const outputPath = join(tmpdir(), `prometheus-profile-${name}.tar.gz`)
  try {
    await prometheusCli.exportProfile(name, outputPath)
...
export async function importProfile(ctx: any) {
  ...
  const tmpDir = join(tmpdir(), 'prometheus-import')
  ...
  try {
    const result = await prometheusCli.importProfile(archivePath)
