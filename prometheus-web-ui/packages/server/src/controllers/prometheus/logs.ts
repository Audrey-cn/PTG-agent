...
const WEBUI_LOG_FILE = join(homedir(), '.prometheus-web-ui', 'logs', 'server.log')
...
export async function list(ctx: any) {
  const files = await prometheusCli.listLogFiles()
...
export async function read(ctx: any) {
  ...
  try {
    const content = await prometheusCli.readLogs(logName, lines, level, session, since)
    const rawLines = content.split('\n')
