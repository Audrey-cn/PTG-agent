...
import { getActiveEnvPath, getActiveAuthPath } from '../../services/prometheus/prometheus-profile'
...
import { getCopilotModelsDetailed, resolveCopilotOAuthToken, type CopilotModelMeta } from '../../services/prometheus/copilot-models'
...
export async function getAvailable(ctx: any) {
  try {
    ...
    if (typeof modelSection === 'object' && modelSection !== null) {
      ...
      // When prometheus CLI sets provider: custom, resolve to custom:name
    ...
    // 时也不返回。避免误把 VS Code/gh CLI 用户的全局凭证当作 prometheus provider。
