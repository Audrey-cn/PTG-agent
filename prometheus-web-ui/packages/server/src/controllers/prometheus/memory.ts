...
import { safeReadFile, safeStat, getPrometheusDir } from '../../services/config-helpers'
...
export async function save(ctx: any) {
  ...
  let filePath: string
  if (section === 'soul') {
    filePath = join(getPrometheusDir(), 'SOUL.md')
  } else {
    const fileName = section === 'memory' ? 'MEMORY.md' : 'USER.md'
    filePath = join(getPrometheusDir(), 'memories', fileName)
  }
  try {
