...
const PROMETHEUS_BASE = process.env.PROMETHEUS_HOME || resolve(homedir(), '.prometheus')

/**
 * Get the active profile's home directory.
 * default → ~/.prometheus/
 * other   → ~/.prometheus/profiles/{name}/
 */
export function getActiveProfileDir(): string {
  const activeFile = join(PROMETHEUS_BASE, 'active_profile')
  try {
    const name = readFileSync(activeFile, 'utf-8').trim()
    if (name && name !== 'default') {
      const dir = join(PROMETHEUS_BASE, 'profiles', name)
...
export function getProfileDir(name: string): string {
  if (!name || name === 'default') return PROMETHEUS_BASE
  const dir = join(PROMETHEUS_BASE, 'profiles', name)
  return existsSync(dir) ? dir : PROMETHEUS_BASE
}
