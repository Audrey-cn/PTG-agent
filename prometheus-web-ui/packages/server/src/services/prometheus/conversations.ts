import { exportSessionsRaw, type PrometheusSessionFull } from './prometheus-cli'
...
type ConversationSession = PrometheusSessionFull & {
...
type CachedExport = {
  ...
  sessions: PrometheusSessionFull[]
}
...
function visibleHumanMessage(message: PrometheusMessageLike): boolean {
...
function firstVisibleHumanText(messages: PrometheusMessageLike[]): string {
  const firstVisible = messages.find(visibleHumanMessage)
  return firstVisible ? textFromContent(firstVisible.content).trim() : ''
}

function maxMessageTimestamp(messages: PrometheusMessageLike[]): number {
...
function enrichSession(session: PrometheusSessionFull, nowSeconds: number): ConversationSession {
...
function sessionMessages(session: PrometheusSessionFull): PrometheusMessageLike[] {
  return Array.isArray(session.messages) ? session.messages as PrometheusMessageLike[] : []
}

function normalizeVisibleMessage(message: PrometheusMessageLike, session: PrometheusSessionFull, index: number): ConversationMessage | null {
...
function visibleMessagesForSessions(sessions: PrometheusSessionFull[]): ConversationMessage[] {
  return sessions
...
function hasVisibleHumanMessages(sessions: PrometheusSessionFull[]): boolean {
