 ...
 *
 * 启动模式：
 *   - 正常系统（macOS/Linux）：prometheus gateway start/stop（系统服务管理）
 *   - WSL / Docker：prometheus gateway run（detached 子进程，手动 kill）
 */
...
const PROMETHEUS_BIN = process.env.PROMETHEUS_BIN?.trim() || 'prometheus'
...
export class GatewayManager {
  ...
  private profileDir(name: string): string {
    if (name === 'default') return PROMETHEUS_BASE
    return join(PROMETHEUS_BASE, 'profiles', name)
  }
   ...
   *         port: <port>
   *         host: <host>
   * 同时清理旧的顶层 port/host（避免 Prometheus 读取错误）
   */
  private writeProfilePort(name: string, port: number, host: string): void {
    ...
    try {
      ...
      // 清理旧的顶层 port/host，Prometheus 只从 extra 读取
  ...
  /** 列出所有已知 profile 名称（通过 prometheus CLI 或文件系统扫描） */
  async listProfiles(): Promise<string[]> {
    try {
      ...
      const profilesDir = join(PROMETHEUS_BASE, 'profiles')
  ...
  async start(name: string): Promise<GatewayStatus> {
    ...
    const prometheusHome = this.profileDir(name)
    const url = `http://${host}:${port}`

    if (needsRunMode) {
      // WSL / Docker：无 systemd/launchd，用 "gateway run" 作为 detached 子进程
      return new Promise((resolve, reject) => {
        const env = { ...process.env, PROMETHEUS_HOME: prometheusHome }
        const child = spawn(PROMETHEUS_BIN, ['gateway', 'run', '--replace'], {
          detached: true,
          stdio: 'ignore',
          windowsHide: true,
          env,
    ...
    const env = { ...process.env, PROMETHEUS_HOME: prometheusHome }
    try {
      const { stdout } = await execFileAsync(PROMETHEUS_BIN, ['gateway', 'start'], {
        timeout: 30000,
        env,
        windowsHide: true,
      })
      ...
      try {
        const { stdout } = await execFileAsync(PROMETHEUS_BIN, ['gateway', 'restart'], {
          timeout: 30000,
          env,
          windowsHide: true,
        })
  ...
  async stop(name: string, timeoutMs = 10000): Promise<void> {
    ...
    if (!needsRunMode) {
      // 正常系统：通过 prometheus CLI 停止系统服务
      try {
        const prometheusHome = this.profileDir(name)
        const env = { ...process.env, PROMETHEUS_HOME: prometheusHome }
        await execFileAsync(PROMETHEUS_BIN, ['gateway', 'stop'], {
          timeout: 10000,
          env,
          windowsHide: true,
        })
  ...
  async startAll(): Promise<void> {
    ...
    for (const name of profiles) {
      ...
      // Skip remote profiles — local prometheus command cannot start remote gateways
