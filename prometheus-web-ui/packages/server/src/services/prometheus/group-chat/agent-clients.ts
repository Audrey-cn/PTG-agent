...
import { deleteSession as prometheusDeleteSession } from '../prometheus-cli'
import { getActiveProfileName } from '../prometheus-profile'
import { logger } from '../../../services/logger'
import { updateUsage } from '../../../db/prometheus/usage-store'
import { getSessionDetailFromDbWithProfile } from '../../../db/prometheus/sessions-db'
...
class AgentClient {
    ...
    private async deleteSession(sessionId: string): Promise<void> {
        try {
            ...
            const ok = await prometheusDeleteSession(sessionId)
            if (ok) {
    ...
    // ─── Prometheus Gateway Integration ────────────────────────────
    ...
    async replyToMention(
        ...
        try {
            ...
            // Start a run on Prometheus gateway
            ...
            source.onmessage = async (e: any) => {
                try {
                    ...
                    if (parsed.event === 'run.completed') {
                        // Record usage data from Prometheus state.db BEFORE closing source
