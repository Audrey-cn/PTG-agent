...
import { setRunSession } from '../../routes/prometheus/proxy-handler'
import { updateUsage } from '../../db/prometheus/usage-store'
import {
  getSession,
  getSessionDetail,
...
import { getSessionDetailFromDb } from '../../db/prometheus/sessions-db'
...
import { getCompressionSnapshot } from '../../db/prometheus/compression-snapshot'
...
function convertToAnthropicFormat(messages: any[]): any[] {
  ...
  for (const m of messages) {
    ...
    if (role === 'tool') {
      ...
      // Follow Prometheus official format: content is a string (not array)
...
export class ChatRunSocket {
  ...
  private prometheusSessionIds = new Map<string, any>()
    ...
    try {
      _messages = messages
        ...
        .map((m, idx, arr) => {
          ...
          // Check if content is a stringified array (Prometheus Gateway behavior) - only for assistant messages
  ...
  private async handleRun(
    ...
    // Generate ephemeral session ID for Prometheus (fresh session per run)
    const prometheusSessionId = session_id
    ...
    if (session_id) {
      ...
      this.prometheusSessionIds.set(session_id, prometheusSessionId)
    ...
    try {
      ...
      if (prometheusSessionId) body.session_id = prometheusSessionId
      ...
      source.onmessage = (event: MessageEvent) => {
        try {
          ...
          // Usage will be calculated after syncFromPrometheus completes (in markCompleted)
  ...
  private markCompleted(socket: Socket, sessionId: string, _info: { event: string; run_id?: string }) {
    ...
    if (state) {
      ...
      state.events = []
      // Sync messages from Prometheus ephemeral session to local DB
      if (useLocalSessionStore() && this.prometheusSessionIds.get(sessionId)) {
        const prometheusId = this.prometheusSessionIds.get(sessionId)
        const prof = state.profile
        this.prometheusSessionIds.delete(sessionId)
        state.profile = undefined
        this.syncFromPrometheus(socket, sessionId, prometheusId, prof)
      }
    }
  }
  ...
  /**
   * Read complete messages from Prometheus state.db for the ephemeral session
   ...
   */
  private syncFromPrometheus(socket: Socket, localSessionId: string, prometheusSessionId: string, profile?: string) {
    getSessionDetailFromDb(prometheusSessionId)
      .then((detail) => {
        if (!detail || !detail.messages?.length) {
          logger.warn('[chat-run-socket] syncFromPrometheus: no data for Prometheus session %s', prometheusSessionId)
          return
        }
        ...
        // (Prometheus stores tool_name as NULL, name lives inside tool_calls JSON)
        ...
        if (toInsert.length > 0) {
          ...
          logger.info('[chat-run-socket] syncFromPrometheus: memory has %d messages, DB has %d messages',
            memoryMessages.length, toInsert.length)

          // Match messages by order since Prometheus DB and memory should have same sequence
          let memoryIdx = 0
          let mergedCount = 0
          for (let i = 0; i < toInsert.length && memoryIdx < memoryMessages.length; i++) {
            ...
            if (dbMsg.role === memoryMsg.role) {
              ...
              if (!dbMsg.reasoning && memoryMsg.reasoning) {
                ...
                mergedCount++
                logger.info('[chat-run-socket] syncFromPrometheus: merged reasoning from memory to DB for %s message at index %d',
                  dbMsg.role, i)
              }
            }
            memoryIdx++
          ...
          if (mergedCount > 0) {
            logger.info('[chat-run-socket] syncFromPrometheus: merged reasoning for %d messages', mergedCount)
          }
          ...
          logger.info('[chat-run-socket] syncFromPrometheus: synced %d messages to local session %s', toInsert.length, localSessionId)
        }

        updateSessionStats(localSessionId)

        // Record usage from Prometheus session
        ...
        if (state) {
          ...
          if (messages.length > 0) {
            this.replaceByPrometheusSessionId(localSessionId, prometheusSessionId, messages)
          }
          const emit = (event: string, payload: any) => {
        ...
        this.enqueueEphemeralDelete(prometheusSessionId, profile)
      })
      .catch((err: any) => {
        logger.warn(err, '[chat-run-socket] syncFromPrometheus failed for session %s (prometheusId: %s, profile: %s)', localSessionId, prometheusSessionId, profile || 'default')
      })
  }
  private replaceByPrometheusSessionId(session_id: string, prometheusSessionId: string, newItems: SessionMessage[]) {
    let start = -1
    let end = -1
    const state = this.sessionMap.get(session_id)
    ...
    for (let i = 0; i < msg.length; i++) {
      if (msg[i].prometheusSessionId === prometheusSessionId) {
        if (start === -1) start = i
        end = i
      } else if (start !== -1) {
        // 已经找到一段，后面断了就可以结束
  ...
  /** Enqueue an ephemeral Prometheus session for deferred deletion */
  private enqueueEphemeralDelete(prometheusSessionId: string, profile?: string) {
    try {
      const db = getDb()
      if (!db) return
      const now = Date.now()
      ...
      ).run(prometheusSessionId, profile || 'default', now, now, now)
      logger.info('[chat-run-socket] enqueued ephemeral session %s for deletion', prometheusSessionId)
    } catch { /* best-effort */ }
  }
