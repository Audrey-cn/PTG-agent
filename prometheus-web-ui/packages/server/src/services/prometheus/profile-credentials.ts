 ...
 * 智能克隆 Profile 凭据管理
 *
 * 背景：`prometheus profile create --clone` 会完整复制源 profile 的 .env + config.yaml，
 * 包括各平台的独占凭据（Weixin / Telegram / Slack / ...）。
 * 这导致多个 profile 同时持有同一个 bot token，prometheus-agent 内部的 token 互斥机制
...
const PROMETHEUS_BASE = join(homedir(), '.prometheus')

/**
 * 已知"独占型"平台的环境变量前缀正则
 *
 * 这些平台的凭据本质上是"一对一身份绑定"：一个 token / app_id 对应唯一一个机器人或账号。
 * 多个 profile 共享同一凭据会触发 prometheus-agent 的 token 互斥机制 → 启动失败。
 *
 * 不在此列表的（模型 provider API key、工具调试开关等）视为可安全共享。
 *
 * **来源（不要凭主观推测扩展）**：与 prometheus-agent `gateway/platforms/` 中实际调用
 ...
 *
 * @param profileName profile 名称（'default' → ~/.prometheus/，其他 → ~/.prometheus/profiles/<name>/）
 */
export function smartCloneCleanup(profileName: string): SmartCloneCleanup {
  const profileDir = profileName === 'default'
    ? PROMETHEUS_BASE
    : join(PROMETHEUS_BASE, 'profiles', profileName)
