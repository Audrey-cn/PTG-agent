...
import { getActiveProfileDir, getActiveEnvPath } from './prometheus-profile'
...
export interface FileEntry {
  name: string
  path: string       // relative to prometheus home
  isDir: boolean
  size: number
...
/**
 * Resolve a relative path to an absolute path under the prometheus home directory.
 * Validates path safety (no traversal).
 */
export function resolvePrometheusPath(relativePath: string): string {
...
/**
 * Read terminal config from prometheus config.yaml.
 */
...
/**
 * Read SSH env vars from prometheus .env file.
 */
...
async function resolveDockerContainer(cfg: TerminalConfig): Promise<string> {
  ...
  throw Object.assign(
    new Error('Cannot determine Docker container. Set terminal.docker_container_name in prometheus config.'),
    { code: 'backend_error' },
  )
}
...
/**
 * Create a FileProvider based on the active prometheus terminal config.
