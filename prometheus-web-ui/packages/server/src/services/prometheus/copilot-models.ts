...
// 缓存按 oauth token 隔离：避免切换 prometheus profile（不同 .env / 不同 Copilot 账号）
...
// prometheus-agent copilot_auth.py and skip them so callers fall through.
 ...
 * 跳过 classic PAT (ghp_)，与上游 prometheus-agent copilot_auth.py 行为对齐。
...
async function fetchModelsList(copilotToken: string): Promise<CopilotModelMeta[]> {
  ...
  // 与上游 prometheus-agent prometheus_cli/models.py 对齐：只过滤 chat type 且 supports
