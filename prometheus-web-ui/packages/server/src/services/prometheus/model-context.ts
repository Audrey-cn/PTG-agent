...
const MODELS_DEV_CACHE = resolve(PROMETHEUS_BASE, 'models_dev_cache.json')
...
function getProfileDir(profile?: string): string {
  if (!profile || profile === 'default') return PROMETHEUS_BASE
  const dir = join(PROMETHEUS_BASE, 'profiles', profile)
