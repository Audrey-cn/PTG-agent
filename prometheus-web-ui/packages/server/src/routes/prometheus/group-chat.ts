...
import type { GroupChatServer } from '../../services/prometheus/group-chat'
...
// Create room
groupChatRoutes.post('/api/prometheus/group-chat/rooms', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
...
groupChatRoutes.get('/api/prometheus/group-chat/rooms/:roomId', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
// List rooms
groupChatRoutes.get('/api/prometheus/group-chat/rooms', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.get('/api/prometheus/group-chat/rooms/join/:code', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.put('/api/prometheus/group-chat/rooms/:roomId/invite-code', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
// Add agent to room
groupChatRoutes.post('/api/prometheus/group-chat/rooms/:roomId/agents', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
// List agents in room
groupChatRoutes.get('/api/prometheus/group-chat/rooms/:roomId/agents', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.delete('/api/prometheus/group-chat/rooms/:roomId/agents/:agentId', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
// Delete room
groupChatRoutes.delete('/api/prometheus/group-chat/rooms/:roomId', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.put('/api/prometheus/group-chat/rooms/:roomId/config', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.post('/api/prometheus/group-chat/rooms/:roomId/compress', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
