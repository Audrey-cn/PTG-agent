...
function resolveProfile(ctx: Context): string {
  return ctx.get('x-prometheus-profile') || (ctx.query.profile as string) || 'default'
}
...
export async function proxy(ctx: Context) {
  ...
  const upstreamPath = ctx.path.replace(/^\/api\/prometheus\/v1/, '/v1').replace(/^\/api\/prometheus/, '/api')
