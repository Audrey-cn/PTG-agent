export default {
  ...
  sidebar: {
    ...
    updateTip: 'Ejecuta "prometheus-web-ui update" en la terminal para actualizar',
  ...
  chat: {
    ...
    emptyState: 'Inicia una conversacion con Prometheus Agent',
    ...
    prometheusHistory: 'Historial de Prometheus',
    historyScopeHint: 'Sesiones del historial de Prometheus, de solo lectura y agrupadas por origen.',
...
jobTriggered: 'Job ejecutado',
  ...
  models: {
    ...
    copilotAddDetected: 'Se detectó un token OAuth de GitHub Copilot en este equipo. Haz clic en Agregar para habilitar Copilot en Prometheus.',
    copilotAddSourceEnv: 'Origen: ~/.prometheus/.env (COPILOT_GITHUB_TOKEN)',
    copilotAddSourceGhCli: 'Origen: gh CLI (gh auth token)',
    copilotAddSourceAppsJson: 'Origen: extensión Copilot de VS Code (apps.json)',
    copilotDeleteHintEnv: 'Esto borrará COPILOT_GITHUB_TOKEN en ~/.prometheus/.env. Otras herramientas no se verán afectadas.',
    copilotDeleteHintGhCli: 'Copilot se ocultará de Prometheus. Tu sesión de gh CLI no se verá afectada — `gh auth status` seguirá mostrando que estás conectado.',
    copilotDeleteHintAppsJson: 'Copilot se ocultará de Prometheus. La extensión Copilot de VS Code seguirá conectada.',
  ...
  platform: {
    ...
    weixinTokenHint: 'Desde el inicio de sesion QR de la CLI de weixin (prometheus weixin)',
  ...
  changelog: {
    ...
    new_0_5_5_2: 'Añadida página de historial para sesiones Prometheus',
