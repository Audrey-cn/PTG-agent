export default {
  ...
  sidebar: {
    ...
    connected: '接続済み',
    disconnected: '未接続',
    updateTip: 'ターミナルで "prometheus-web-ui update" を実行して更新してください',
  ...
  chat: {
    contextRemaining: '残り',
    emptyState: 'Prometheus Agent と会話を開始しましょう',
    ...
    openHistory: '履歴を開く',
    prometheusHistory: 'Prometheus 履歴',
    historyScopeHint: 'ソース別にグループ化された Prometheus 履歴セッションを読み取り専用で表示します。',
  ...
  models: {
    ...
    copilotAddDetected: 'このマシンで GitHub Copilot OAuth トークンを検出しました。「追加」をクリックして Prometheus で Copilot を有効化します。',
    copilotAddSourceEnv: 'ソース: ~/.prometheus/.env (COPILOT_GITHUB_TOKEN)',
    copilotAddSourceGhCli: 'ソース: gh CLI (gh auth token)',
    copilotAddSourceAppsJson: 'ソース: VS Code Copilot 拡張機能 (apps.json)',
    copilotDeleteHintEnv: 'この操作で ~/.prometheus/.env の COPILOT_GITHUB_TOKEN を消去します。他のツールには影響しません。',
    copilotDeleteHintGhCli: 'Copilot は Prometheus 上で非表示になります。gh CLI のログインには影響しません — `gh auth status` は引き続きログイン状態を表示します。',
    copilotDeleteHintAppsJson: 'Copilot は Prometheus 上で非表示になります。VS Code Copilot 拡張機能のログインには影響しません。',
    customBadge: 'カスタム',
    previewBadge: 'プレビュー',
    disabledBadge: '利用不可',
  ...
  platform: {
    ...
    weixinTokenHint: 'weixin CLI の QR ログインから取得 (prometheus weixin)',
  ...
  changelog: {
    ...
    new_0_5_5_2: 'Prometheusセッション履歴ページを追加',
