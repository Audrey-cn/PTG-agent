...
import type { AvailableModelGroup } from '@/api/prometheus/system'
import { useModelsStore } from '@/stores/prometheus/models'
import { useAppStore } from '@/stores/prometheus/app'
import { useChatStore } from '@/stores/prometheus/chat'
import { checkCopilotToken, disableCopilot } from '@/api/prometheus/copilot-auth'
    ...
    onPositiveClick: async () => {
      ...
      try {
        if (isCopilot.value) {
          ...
          // 仅当 token 来自 ~/.prometheus/.env 时才清掉，gh-cli / apps.json 不动。
        ...
        // "无默认模型"的尴尬态。与 prometheus CLI `model` 子命令的隐含行为对齐。
