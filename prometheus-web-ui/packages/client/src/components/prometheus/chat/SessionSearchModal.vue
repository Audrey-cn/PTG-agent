...
import { fetchSessions, searchSessions, type SessionSearchResult, type SessionSummary } from '@/api/prometheus/sessions'
...
async function openItem(item: SearchItem) {
  ...
  if (router.currentRoute.value.name !== 'prometheus.chat') {
    await router.push({ name: 'prometheus.chat' })
  }
}
