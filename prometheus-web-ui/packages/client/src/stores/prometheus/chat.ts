import { startRunViaSocket, resumeSession, registerSessionHandlers, unregisterSessionHandlers, type RunEvent } from '@/api/prometheus/chat'
import { deleteSession as deleteSessionApi, fetchSession, fetchSessions, type PrometheusMessage, type SessionSummary } from '@/api/prometheus/sessions'
...
export interface Message {
  ...
  // 思考/推理文本。两条来源：
  //   1) 历史消息：来自 PrometheusMessage.reasoning 字段
...
async function uploadFiles(attachments: Attachment[]): Promise<{ name: string; path: string }[]> {
  ...
  const token = localStorage.getItem('prometheus_api_key') || ''
...
function mapPrometheusMessages(msgs: PrometheusMessage[]): Message[] {
...
function mapPrometheusSession(s: SessionSummary): Session {
  return {
    id: s.id,
    title: s.title || '',
...
const STORAGE_KEY_PREFIX = 'prometheus_active_session_'
const LEGACY_STORAGE_KEY = 'prometheus_active_session'
...
function inFlightKey(sid: string): string { return `prometheus_in_flight_v1_${getProfileName()}_${sid}` }
function legacyInFlightKey(sid: string): string | null { return getProfileName() === 'default' ? `prometheus_in_flight_v1_${sid}` : null }
...
function recoverStorageQuota() {
  try {
    // 清理所有会话相关的旧缓存（已完全废弃）
    const prefixes = [
      'prometheus_sessions_cache_v1_',
      'prometheus_session_msgs_v1_',
      'prometheus_session_pins_v1_',
      'prometheus_human_only_v1_',
    ]
...
export const useChatStore = defineStore('chat', () => {
  ...
  async function loadSessions() {
    ...
    try {
      ...
      const fresh = list.map(mapPrometheusSession)
  ...
  async function refreshActiveSession(): Promise<boolean> {
    ...
    try {
      ...
      const mapped = mapPrometheusMessages(detail.messages || [])
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        resumeSession(sessionId, (data) => {
          ...
          if (data.messages?.length) {
            activeSession.value!.messages = mapPrometheusMessages(data.messages as any[])
          }
  ...
  async function switchSessionModel(modelId: string, provider?: string) {
    ...
    // If provider changed, update global config too (Prometheus requires it)
    if (provider) {
    ...
    try {
      ...
      if (attachments && attachments.length > 0) {
        ...
        const urlMap = new Map(uploaded.map(f => {
          const base = `/api/prometheus/download?path=${encodeURIComponent(f.path)}&name=${encodeURIComponent(f.name)}`
      ...
      // prometheus-agent occasionally emits run.completed with empty output and no
        ...
        (evt: RunEvent) => {
          switch (evt.event) {
            ...
            case 'run.completed': {
              ...
              // Workaround for upstream prometheus-agent bug: when the agent
              ...
              if (swallowedError) {
                addMessage(sid, {
                  id: uid(),
                  role: 'system',
                  content: 'Error: Agent returned no output. The model call may have failed (e.g. invalid API key, model not supported by provider, or context exceeded). Check the prometheus-agent logs for details.',
                  timestamp: Date.now(),
                })
              }
