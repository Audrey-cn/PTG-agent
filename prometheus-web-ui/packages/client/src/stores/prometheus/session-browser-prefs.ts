...
const PIN_KEY_PREFIX = 'prometheus_session_pins_v1_'
const HUMAN_ONLY_KEY_PREFIX = 'prometheus_human_only_v1_'

function currentProfileName(): string {
  try {
    return useProfilesStore().activeProfileName || localStorage.getItem('prometheus_active_profile_name') || 'default'
  } catch {
    return localStorage.getItem('prometheus_active_profile_name') || 'default'
  }
}
