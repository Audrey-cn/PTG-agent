...
const STORAGE_KEY_THEME = "prometheus_terminal_theme";
...
function buildWsUrl(): string {
  ...
  if (base) {
    return `${wsProtocol}//${new URL(base).host}/api/prometheus/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
  }
  ...
  const host = import.meta.env.DEV
    ...
    : location.host;
  return `${wsProtocol}//${host}/api/prometheus/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
}
