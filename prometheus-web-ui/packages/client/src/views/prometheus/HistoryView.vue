...
import { useChatStore, type Session } from '@/stores/prometheus/chat'
import { useAppStore } from '@/stores/prometheus/app'
import { useProfilesStore } from '@/stores/prometheus/profiles'
import { useSessionBrowserPrefsStore } from '@/stores/prometheus/session-browser-prefs'
import { NButton, NDropdown, NInput, NModal, NTooltip, useMessage } from 'naive-ui'
import { useI18n } from 'vue-i18n'
import { getSourceLabel } from '@/shared/session-display'
import { copyToClipboard } from '@/utils/clipboard'
import FolderPicker from '@/components/prometheus/chat/FolderPicker.vue'
import HistoryMessageList from '@/components/prometheus/chat/HistoryMessageList.vue'
import SessionListItem from '@/components/prometheus/chat/SessionListItem.vue'
import { renameSession, setSessionWorkspace, fetchPrometheusSessions, fetchPrometheusSession, type SessionSummary } from '@/api/prometheus/sessions'
...
// Prometheus history sessions (exclude api_server)
const prometheusSessions = ref<SessionSummary[]>([])
const prometheusSessionsLoading = ref(false)
const prometheusSessionsLoaded = ref(false)
...
async function loadPrometheusSessions() {
  if (prometheusSessionsLoading.value) return
  prometheusSessionsLoading.value = true
  try {
    prometheusSessions.value = await fetchPrometheusSessions()
    prometheusSessionsLoaded.value = true
  } catch (err) {
    console.error('Failed to load Prometheus sessions:', err)
  } finally {
    prometheusSessionsLoading.value = false
  }
}
...
async function handleSessionClick(sessionId: string) {
  // First, fetch the Prometheus session detail
  const sessionDetail = await fetchPrometheusSession(sessionId)
  if (!sessionDetail) {
...
const collapsedGroups = ref<Set<string>>(new Set(JSON.parse(localStorage.getItem('prometheus_collapsed_groups') || '[]')))
...
// Computed sessions from Prometheus API
const historySessions = computed<Session[]>(() =>
  prometheusSessions.value.map(sessionSummaryToSession)
)
...
function toggleGroup(source: string) {
  ...
  localStorage.setItem('prometheus_collapsed_groups', JSON.stringify([...collapsedGroups.value]))
}

watch(groupedSessions, groups => {
  if (localStorage.getItem('prometheus_collapsed_groups') !== null) {
...
// Auto-load first CLI session when Prometheus sessions are loaded
watch(prometheusSessionsLoaded, (loaded) => {
  if (loaded && prometheusSessions.value.length > 0) {
    // Only auto-load if no session is currently active
    if (!historySessionId.value || !prometheusSessions.value.find(s => s.id === historySessionId.value)) {
      // Find first CLI session
      const firstCliSession = prometheusSessions.value.find(s => s.source === 'cli')
      if (firstCliSession) {
...
async function handleRenameConfirm() {
  ...
  if (ok) {
    // Reload Prometheus sessions to get updated title
    await loadPrometheusSessions()
...
async function handleWorkspaceConfirm() {
  ...
  if (ok) {
    // Reload Prometheus sessions to get updated workspace
    await loadPrometheusSessions()
  ...
  <div class="history-panel">
    ...
    <aside class="session-list" :class="{ collapsed: !showSessions }">
      <div class="session-list-header">
        <span v-if="showSessions" class="session-list-title">{{ t('chat.prometheusHistory') }}</span>
      ...
      <div v-if="showSessions" class="session-items">
        <div v-if="prometheusSessionsLoading && prometheusSessions.length === 0" class="session-loading">{{ t('common.loading') }}</div>
        <div v-else-if="prometheusSessions.length === 0" class="session-empty">{{ t('chat.noSessions') }}</div>
