...
export async function startCopilotLogin(): Promise<CopilotStartResult> {
  return request<CopilotStartResult>('/api/prometheus/auth/copilot/start', { method: 'POST' })
}

export async function pollCopilotLogin(sessionId: string): Promise<CopilotPollResult> {
  return request<CopilotPollResult>(`/api/prometheus/auth/copilot/poll/${sessionId}`)
}

export async function checkCopilotToken(): Promise<CopilotCheckTokenResult> {
  return request<CopilotCheckTokenResult>('/api/prometheus/auth/copilot/check-token')
}

export async function enableCopilot(): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>('/api/prometheus/auth/copilot/enable', { method: 'POST' })
}

export async function disableCopilot(): Promise<{ ok: boolean; cleared_env: boolean; cleared_default?: boolean }> {
  return request<{ ok: boolean; cleared_env: boolean; cleared_default?: boolean }>('/api/prometheus/auth/copilot/disable', { method: 'POST' })
}
