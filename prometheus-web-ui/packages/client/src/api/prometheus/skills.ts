...
export async function fetchSkills(): Promise<SkillsData> {
  const res = await request<SkillListResponse>('/api/prometheus/skills')
...
export async function fetchSkillContent(skillPath: string): Promise<string> {
  const res = await request<{ content: string }>(`/api/prometheus/skills/${skillPath}`)
  return res.content
}

export async function fetchSkillFiles(category: string, skill: string): Promise<SkillFileEntry[]> {
  const res = await request<{ files: SkillFileEntry[] }>(`/api/prometheus/skills/${category}/${skill}/files`)
  return res.files
}

export async function fetchMemory(): Promise<MemoryData> {
  return request<MemoryData>('/api/prometheus/memory')
}

export async function saveMemory(section: 'memory' | 'user' | 'soul', content: string): Promise<void> {
  await request('/api/prometheus/memory', {
    method: 'POST',
...
export async function toggleSkill(name: string, enabled: boolean): Promise<void> {
  await request('/api/prometheus/skills/toggle', {
    method: 'PUT',
...
export async function pinSkillApi(name: string, pinned: boolean): Promise<void> {
  await request('/api/prometheus/skills/pin', {
    method: 'PUT',
