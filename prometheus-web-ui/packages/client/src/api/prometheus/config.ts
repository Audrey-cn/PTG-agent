...
export async function fetchConfig(sections?: string[]): Promise<AppConfig> {
  ...
  return request<AppConfig>(`/api/prometheus/config${query}`)
}
...
export async function updateConfigSection(
  ...
  await request('/api/prometheus/config', {
    method: 'PUT',
...
export async function saveCredentials(
  ...
  await request('/api/prometheus/config/credentials', {
    method: 'PUT',
...
export async function fetchWeixinQrCode(): Promise<WeixinQrCode> {
  return request<WeixinQrCode>('/api/prometheus/weixin/qrcode')
}

export async function pollWeixinQrStatus(qrcode: string): Promise<WeixinQrStatus> {
  return request<WeixinQrStatus>(`/api/prometheus/weixin/qrcode/status?qrcode=${encodeURIComponent(qrcode)}`)
}
...
export async function saveWeixinCredentials(data: {
  ...
  await request('/api/prometheus/weixin/save', {
    method: 'POST',
